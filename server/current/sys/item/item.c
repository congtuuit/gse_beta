...
#define ITEM_ATTRIBUTE "sys/item/itemAttribute.c"
#define EQUIP "sys/item/equip.c"
...
string get_pawn_string(object me, string prop)
{
	...
	switch (name->get_item_type())
	{
		...
		if (sscanf(prop, "%s;%d;%d;%d;%s;[%s]%d,%d", name, size, use, maxuse, prop, id, locktime, lockutil) != 8)
		{
			return "Các mặt hàng có vấn đề, sự cứu chuộc sẽ khiến vật phẩm biến mất. Nếu bạn có bất kỳ câu hỏi nào, xin vui lòng thông báo ＧＭ.";
		}
		...
		switch (name->get_item_color())
		{
			...
			case 6:	color = PPM;  break;
			case 10: color = HIG;  break;
		}
	...
	case ITEM_TYPE_TALISMAN:	
		if (sscanf(prop, "%s;%d;%d;%d;%d;[%s]%d,%d", name, size, use, maxuse, point, id, locktime, lockutil) != 8)
		{
			return "Các mặt hàng có vấn đề, sự cứu chuộc sẽ khiến vật phẩm biến mất. Nếu bạn có bất kỳ câu hỏi nào, xin vui lòng thông báo ＧＭ.";
		}
...
// 函数：获取道具描述(所有)
// Hieu thi chi tiet trang bi
varargs string get_look_string(object who, object item, int flag)
{
	object map, fileobj;
	string name, file, loai, color, mautc, result, product, bind, forge, sao, mau, max = "";
	int level, point, lockutil, total, amount, hole;
	int i, size, dk;
	bind = "";
	...
	case 5:
		...
		break;
	case 6:
		bind = HIC"(Nhận vật phẩm sẽ Phong Ấn)"NOR;
		break;
	}

	hole = EQUIP->get_hole_amount(item);
	if (hole > 0) {
		if (!is_player(who))
			result = sprintf("%s%s[%s]%s%x#"NOR"(%d lỗ)%s\n%s", item->get_name(), item->get_item_make_desc(),
				item->get_user_id(), item->is_locked() > 0 ? "[Khóa]" : "", getoid(item),hole, bind, get_desc_string(who, item));
		else    result = sprintf("%s%s"NOR"(%d lỗ)%s\n%s", item->get_name(), item->get_item_make_desc(), hole, bind, get_desc_string(who, item));
	}
	else {
		if (!is_player(who))
	...
	case ITEM_TYPE_EQUIP:
		level = item->get_level() - who->get_equip_less_level() - item->get("-1-");
		if (level < 0) level = 0;
		if (item->is_thanbinh() && (item->is_thanbinh_thien() < 1 && item->is_thanbinh_dia() < 1))
		{
		...
		if (item->get("hole")) {
			result += EQUIP->get_hole_string(item) + "\n";
		}

		if (point = item->get("stoneHP+"))  result += ITEM_EQUIP_D->get_chinese_prop_string_3(file, "stoneHP+", point, color);
		if (point = item->get("stoneAP+"))  result += ITEM_EQUIP_D->get_chinese_prop_string_3(file, "stoneAP+", point, color);
		if (point = item->get("stoneCP+"))  result += ITEM_EQUIP_D->get_chinese_prop_string_3(file, "stoneCP+", point, color);
		if (point = item->get("stoneDP+"))  result += ITEM_EQUIP_D->get_chinese_prop_string_3(file, "stoneDP+", point, color);
		if (point = item->get("stonePP+"))  result += ITEM_EQUIP_D->get_chinese_prop_string_3(file, "stonePP+", point, color);
		...
		if (point = item->get("aps%")) result += ITEM_EQUIP_D->get_chinese_prop_string_3(file, "aps%", point, color);
		...
		if (point = item->get("cps%")) result += ITEM_EQUIP_D->get_chinese_prop_string_3(file, "cps%", point, color);
		...
		if (point = item->get("-anti_hurt%")) result += ITEM_EQUIP_D->get_chinese_prop_string_3(file, "-anti_hurt%", point, color);
		...
		//Dat vang
		if (item->get("gold_time") - time() > 0) {

			item->set_weapon_color(YELLOW);

			if (point = item->get("goldAP+"))  result += ITEM_EQUIP_D->get_chinese_prop_string_3(file, "goldAP+", point, color);
			if (point = item->get("goldCP+"))  result += ITEM_EQUIP_D->get_chinese_prop_string_3(file, "goldCP+", point, color);
			if (point = item->get("goldDP+"))  result += ITEM_EQUIP_D->get_chinese_prop_string_3(file, "goldDP+", point, color);
			if (point = item->get("goldPP+"))  result += ITEM_EQUIP_D->get_chinese_prop_string_3(file, "goldPP+", point, color);

			result += sprintf(HIC"(Thời gian hiệu lưc còn lại: %s)\n", "cmd/base/time"->get_time_string_3(item->get("gold_time") - time()));
		...
		{
			if (size == 1) mau = HIW;
			else if (size == 2) mau = HIC;
			else if (size == 3) mau = HIY;
			else if (size == 4) mau = PPM;
			else mau = HIR;
			result += mau;
			result += "→";
			for (i = 0; i < size; i++)
		...
		if (item->get("sao")) {
			switch (strlen(item->get("sao")))
			{
			case 1: loai = "Sơ Cấp"; mautc = NOR; break;
			case 2: loai = "Trung Cấp"; mautc = HIC; break;
			case 3: loai = "Cao Cấp"; mautc = HIY; break;
			case 4: loai = "Xuất Sắc"; mautc = PPM; break;
			case 5: loai = "Hoàn Mỹ"; mautc = HIR; break;
			}

			result += sprintf("%s →Tư Chất Trang Bị: %s←\n ", mautc, loai);

			if (point = item->get("ap3"))  result += ITEM_EQUIP_D->get_tuchat_string(file, "ap3", point, color);
			if (point = item->get("cp3"))  result += ITEM_EQUIP_D->get_tuchat_string(file, "cp3", point, color);
			if (point = item->get("dp3"))  result += ITEM_EQUIP_D->get_tuchat_string(file, "dp3", point, color);
			if (point = item->get("pp3"))  result += ITEM_EQUIP_D->get_tuchat_string(file, "pp3", point, color);
			if (point = item->get("hp3"))  result += ITEM_EQUIP_D->get_tuchat_string(file, "hp3", point, color);
			if (point = item->get("mp3"))  result += ITEM_EQUIP_D->get_tuchat_string(file, "mp3", point, color);
			if (point = item->get("sp3"))  result += ITEM_EQUIP_D->get_tuchat_string(file, "sp3", point, color);
		}*/
		...
		/*if (item->get("hole")) {
			hole = EQUIP->get_hole_amount(item);
			size = EQUIP->get_hole_used_amount(item);
		...
		//Hien thi ngu hanh trang bi
		if (item->get_nh()) {
			result += ITEM_ATTRIBUTE->string_4_nguhanh(item->get_nh() - 1, item->get_equip_type());
		}
	...
	if (!flag && (point = item->get_value()) > 0)
	{
		...
		if (item->get_no_give()) {
			result += sprintf(HIR"Vật phẩm không thể giao dịch\n");
		}

		if (item->get_no_drop()) {
			result += sprintf(HIR"Vật phẩm không thể vứt bỏ\n");
		}

		if (item->get_no_sell()) {
			result += sprintf(HIR"Vật phẩm không thể bán\n");
		}
		else {
	...
	if ((point = item->get_save("DMP")) > 0 && flag == 2)
	{
		result += sprintf(HIC "Đơn giá: "HIY"%d "HIR"Điểm Môn Phái\n", point);
	}
