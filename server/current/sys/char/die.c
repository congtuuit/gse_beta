...
//1 liên sát 
//2 chấm dứt liên sát
void tich_luy_vst(object me, int point, string type) {
    string dateString;
    string* data;

    data = explode(short_time(), " ");
    data = explode(data[0], "-");
    dateString = sprintf("%s%s", data[2], data[1]);

    /*if (me->get_save("vst.chain") != 2) {
        me->set_save("vst.chain", 1);
    }*/

    if (type == "add") {
        if (me->get_save("vst.today") != dateString) {

            me->set_save("vst.today", dateString);
            me->set_save("vst.point", point);
            me->set_save("vst.chain", 1);

            if (me->get_save("vst.pointTotal") > 0) {
                me->add_save("vst.pointTotal", point);
            }
            else {
                me->set_save("vst.pointTotal", point);
            }

        }
        else {
            me->add_save("vst.point", point);
            me->add_save("vst.pointTotal", point);
        }

        if (me->get_save("vst.point") >= 3 && me->get_save("vst.chain") == 1) {
            USER_D->user_channel(sprintf(HIC"Bằng hữu "HIG"%s "HIC"đang có "HIR"%d liên sát"HIC" !", me->get_name(), me->get_save("vst.point")));
        }

        tell_user(me, sprintf(HIC"Bạn đã tiêu diệt %d kẻ địch.", me->get_save("vst.point")));
    }
    else {

        me->set_save("vst.today", dateString);

        if (me->get_save("vst.point") > 0) {
            //tell_user(me, sprintf(HIR"Bạn bị chấm dứt %d liên sát.", me->get_save("vst.point")));
        }

        me->set_save("vst.chain", 2);
    }
}
...
//                if ( me->get_enemy()==0) me->set_enemy(who);
        ...
        //Nhiem vu halloween
        if (objectp(map = get_map_object(get_z(me))) && !map->is_virtual_scene()) {
            if (me->get_save_2("halloween.status") == 1 && who->get_level() >= me->get_level() - 10) {
                me->set_save_2("halloween.task", me->get_save_2("halloween.task") + 1);

                if (me->get_save_2("halloween.task") >= 100) {
                    me->set_save_2("halloween.status", 2);
                    notify(HIY"Hoàn thành nhiệm vụ Hồn Ma Yêu Tinh");
                }
                else {
                    notify(sprintf("Hồn ma yêu tinh %d/100", me->get_save_2("halloween.task")));
                }
            }
        }
        ...
        //Nhiem vu TET
        if (objectp(map = get_map_object(get_z(me))) && !map->is_virtual_scene()) {
            if (me->get_save_2("task.TetStatus") == 1 && who->get_level() >= me->get_level() - 10) {
                me->set_save_2("task.TetPoint", me->get_save_2("task.TetPoint") + 1);

                if (me->get_save_2("task.TetPoint") >= 50) {
                    me->set_save_2("task.TetStatus", 2);
                    notify(HIY"Hoàn thành nhiệm vụ!");
                }
                else {
                    notify(sprintf("Tiêu diệt yêu tinh %d/50", me->get_save_2("task.TetPoint")));
                }
            }
        }
    ...
    else    // 玩家处理
    {
        ...
        /*	if ( me && who->get("tythyben")==me->get_number() )
            {
            who->stop_fight();
            me->stop_fight();
            me->delete("tythyben");
        ...
        //PVP
        if (map && map->is_virtual_scene() && map->get_name() == "Đấu Trường PVP"){

            who->set_login_flag(2);
            set_invisible(who, 1);
            set_effect(who, EFFECT_USER_LOGIN, 4);
            who->add_to_scene(80, 254 + random(10), 212 + random(10));

            me->set_login_flag(2);
            set_invisible(me, 1);
            set_effect(me, EFFECT_USER_LOGIN, 4);
            me->add_to_scene(80, 254 + random(10), 212 + random(10));

            tell_user(me, HIY">> Trận đấu kết thúc, chiến thắng thuộc về bạn <<");
            send_user(CHAT_D->get_chat(), "%c%s", 0xA3, HIY + sprintf("Chúc mừng, chiến thắng thuộc về "HIR"%s", me->get_name()));

            "sys/party/pvp"->finished_match_p1win(me, who);
            return;
        }

        if (map && map->is_virtual_scene() && map->is_tower())
        {
            who->set_hp(1);
            who->set_login_flag(2);
            set_invisible(who, 1);
            ...
            who->delete_save_2("tower_105.startAt");
        ...
        if (z < 885 || map->is_org_map() || map->is_copy_scene() || map->is_pvp())
            {
                ...
                if (objectp(map = get_map_object(get_z(who))))
                {
                    ...
                    if (who->is_user() && me->is_user()) {
                        CHAT_D->rumor_channel(0, sprintf(HIM "Nghe nói "HIY"%s "HIR"bị giết "HIM"bởi "HIR"%s.", who->get_name(), me->get_name()));
                    }

                    if (me->get_pk() >= 1000) {
                        me->set_save("gietnguoi", 1);
                        me->set_save("vaotu", 7200);
                        me->set_save("vaotuluc", time());
                        me->set_login_flag(2);
                        set_invisible(me, 1);
                        set_effect(me, EFFECT_USER_LOGIN, 4);
                        me->add_to_scene(704, 35, 25);
                        send_user(me, "%c%c%w%s", ':', 3, 9963, "Giám Ngục Quan :\nNgươi tàn sát quá nhìu người, đây là nơi dành cho ngươi xám hối !\n"
                            ESC "Xác nhận\nCLOSE\n");
                    }
                }
                if (me)
                ...
                ///me->add_pk(100);
                write_user(me, HIC "Bạn đã giết người và tăng thêm 100 điểm PK");
                write_user(me, HIC "Điểm PK hiện tại: "HIR"%d", me->get_pk());
            ...
            else if (map->is_wushuang())
            {
                ...
                if (who->get_level() >= 80) {
                    tich_luy_vst(me, 1, "add");
                }
                
                tich_luy_vst(who, 0, "subPoint");

                //who->set_save("vst.chain",0);
                
                //	me->add_save("tichluyvst",1);
                //	me->add_save("danhhieuvstben",1);
                
                if (who->get_save("vst.point") >= 3 && who->get_save("vst.chain") == 2) {
                    USER_D->user_channel(sprintf("Nghe nói "HIR"%s "NOR"đã bị "HIG"%s"NOR" chấm dứt liên sát!", who->get_name(), me->get_name()));
                }
            ...
            else if (map->is_wushuang_palace())
            {
                ...
                tich_luy_vst(who, 0, "die");
                tich_luy_vst(me, 1, "add");

                if (who->get_save("vst.point") >= 3 && who->get_save("vst.chain") == 1) {
                    USER_D->user_channel(sprintf("Nghe nói "HIR"%s "NOR"đã bị "HIG"%s"NOR" chấm dứt liên sát!", who->get_name(), me->get_name()));
                }
            ...
            else if (map->get_client_id() == 893 || map->get_client_id() == 885 || map->get_client_id() == 891)//|| map->is_wushuang() )    // 比武场
            {
                ...
                if (map->get_client_id() == 893) {
                    USER_D->user_channel(sprintf(HIY "Trận đấu giữa "HIR"%s (%d)"HIY" vs "HIR"%s (%d)"HIY". Phần thắng thuộc về "HIG"%s "HIY".", who->get_name(), who->get_number(), me->get_name(), me->get_number(), me->get_name()));
