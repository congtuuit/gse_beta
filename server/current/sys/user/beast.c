...
void show_beast_perform(object me, object pet)
{
	...
	if (check_show_beast(me, pet) != 1)
		return;
	send_user(me, "%c%c%w%s", 0x5a, 0, 4, "Gọi Bảo Thú……");
	me->set_2("conjure.type", 9999);
	me->set_2("conjure.pet", pet);
	set_effect(me, EFFECT_CONJURE, 4);
}
// 宠物
...
int use_shenshou_item(object who, object item)
{
	...
	if (item->get_spitem() == 1)
	{
		...
		// Ngoai cong
		pet->set_skill(74216, 1);
		pet->set_skill(74217, 1);
		pet->set_skill(74218, 1);
		pet->set_skill(74219, 1);
		pet->set_skill(74220, 1);
		pet->add_max_hp(1000);
		pet->add_hp(1000);
	}
