...
void do_gift(object me, object who, string arg)
{
	...
	switch(flag)
	{
		...
		case 100:
			...
			{
				send_user(who,"%c%c%w%s",':',3,me->get_char_picid(),me->get_name()+":\n Kim bảo của ngươi không đủ con số 150, không thể mua thêm thời gian Song Tu !");
				return 0;
			}
			"sys/sys/test_db"->add_yuanbao(who, -1500);
			who->add_save_2("songtu.time",1800);
			send_user(who,"%c%s",';', "Bạn đã mua thêm "HIR "30 phút Song Tu" NOR ", bạn bị trừ đi 150 Kim bảo !");
			break;
		case 101:
			if ( who->get_level()<20 ) 
			{
