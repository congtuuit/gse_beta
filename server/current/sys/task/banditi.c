...
//	if ( !who->get_legend(TASK_41, 1) || (who->get_legend(TASK_41, 2) && !who->get_legend(TASK_41, 3)) )
	...
	if ( who->get_legend(TASK_48, 20) && !who->get_legend(TASK_48, 21))
		tmp += sprintf(ESC HIY "Bệnh nan y(1)\ntalk %x# welcome.11\n",getoid(me));
...
void gold_task_bonus(object who,object me)
{
	object item;
	string id, id2;
	int level, level2, exp, exp2, title, p;
	int i, size;
	object item_bonus;
...
//	exp = 35*(100+40*level+random(10*level));
	...
	item_bonus = new("item/test2/TuiHoangKimThanhY");
	if (item_bonus)
	{
		p = item_bonus->move2(who);
		item_bonus->add_to_user(p);
	}

	send_user( who, "%c%w", 0x0d, MUSIC_TASK );
	write_user( who, ECHO "Bạn hoàn thành nhiệm vụ Vệ Đội Trưởng Hoàng Kim Thánh Y, kinh nghiệm %+d, Kim Nguyên Bảo +1", who->get("warrior_exp") );
...
//		"sys/sys/test_db"->add_yuanbao(who,20);
		send_user( who, "%c%s", ':', me->get_name() + sprintf(":\n    Ngươi có được Hoàng Kim Thánh Y, đem đến cho ta để đổi %d kinh nghiệm\n",exp)+
