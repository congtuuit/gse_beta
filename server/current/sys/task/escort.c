...
void accept_task(object who, object me)
{
	...
	default: if (sizeof_inventory(who, 1, MAX_CARRY) >= MAX_CARRY)
	{
		...
		return;
		   ...
		   who->log_money("Giao thông vận tải phi tiêu", -gold);
	...
	//	//询问队友是否加入
	//        if( size == 2 && inside_screen_2( team[0], team[1] ) )    // 组队Hoàn thành
	//        {
	//        	team -= ({who});
...
void do_task_start(object who, object me, string arg)
{
	...
	{
		send_user(who, "%c%s", '!', "Bạn đang chạy thử phi tiêu nó");
		return;
	}
	flag = to_int(arg);
	switch (flag)
...
void do_task_finish(object who, object me)
{
		...
		&& who->get_quest("escort.city") + " Quốc" == me->get_city_name())    // 找对人了
	{
		...
		if ((id = who->get_quest("escort.id"))
			...
			&& inside_screen_2(me, member) && member->get_quest("escort.time") == who->get_quest("escort.time"))    // 组队Hoàn thành
		{
			clear_task(who);
			...
			{
				gold = escortgold * 2;
				if (mixTime[TIME_HOUR] > 19 && mixTime[TIME_HOUR] < 21) {
					if (mixTime[TIME_MIN] <= 30) {
						gold += gold;
					}
				}
				gold = gold * member->get_online_rate() / 100;
		...
		"/sys/sys/count"->add_task("Áp Tiêu", 1);
		SAY_CMD->say(who, "#80");
	}
}
...
void send_task_list(object who)
{
	...
	if (iFlag = who->get_quest("escort.flag"))
	{
		...
		{
			send_user(who, "%c%c%c%w%s", 0x51, 1, 1, TID_ESCORT, "Nhiệm vụ áp tiêu");
			send_user(who, "%c%c%c%w%w%s", 0x51, 1, 2, TID_ESCORT, 3, "Cuộc chiến áp tiêu");
		}
	}
}
...
void do_task_list(object who, object me)
{
	...
	if (who->get_legend(TASK_49, 13) && !who->get_legend(TASK_49, 14))
	{
		send_user(who, "%c%s", '!', "Bạn đang thí điểm Áp Tiêu呢");
		return;
	}
	id = getoid(me);
	if (level >= 30)
...
void biaowang(object me, object who, int flag)
{
	...
	case 11:
		...
		{
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(),
				sprintf("%s:\n   tốc độ cạnh tranh và may mắn, trí tuệ và cạnh tranh công nghệ! Bạn được chào đón để tham gia vào Superman nhỏ này"
					"杯“Phiêu vương bá chiến”, Giải thưởng vẫy tay gọi cho bạn, danh dự của bạn khoanh tròn trên cao! Chiến Quốc không phải là một vị vua phi tiêu thực, chỉ"
					"Tôi đã cố gắng để biết.\n    Bạn chỉ cần càng nhanh càng tốt để phi tiêu gửi Wang Superman% s% s tại, và bạn có thể nhận được phần thưởng, thành"
					"Ngắn hơn thời gian vào những phần thưởng hấp dẫn hơn, để dịch vụ trọn gói trong ba người đầu tiên, sau đó có những bất ngờ chờ bạn!\n    bạn đã sẵn sàng chưa？"
					"\n"ESC"Bắt đầu hộ tống\ntalk %x# welcome.12\n"ESC"điều tra kết quả\n" +
	...
	case 12:
		...
		{
			send_user(who, "%c%s", '!', "Bây giờ bạn có một thời gian trò chơi không lành mạnh, Không thể làm điều đó được！");
			return;
		}
		level = who->get_level();
		if (level < 30)
		{
			send_user(who, "%c%s", '!', "trình độ của bạn là không đủ 30级");
			return;
		}
	...
	case 15:
		...
		send_user(who, "%c%s", ':', "Các bảng xếp hạng giải đấu" + ":\n"
			+ cmd +
			ESC "Rời khỏi\nCLOSE\n");
		break;
	case 21:
	...
	case 22:
		...
		if (npc)
		{
			if (!inside_screen_2(me, npc))
			{
				send_user(who, "%c%s", '!', "người hộ tống của bạn làm？！");
				return;
			}
			npc->remove_from_scene();
			destruct(npc);
		...
		send_user(who, "%c%s", ';', sprintf("您参与Phiêu vương bá chiến, Ngân lượng %+d.", gold));
		send_user(who, "%c%c%c%w%w%s", 0x51, 1, 2, TID_ESCORT, 3, "");
		log_file("biaowang.txt", sprintf("%s %s(%d) Hoàn thành %d秒\n", short_time(), who->get_name(), who->get_number(), iTime));
		break;
	}
}
...
void check_time()
{
	...
	else if (iFlag == 3)
	{
		...
		mxTmp = localtime(iFinish2);
		CHAT_D->sys_channel(0, sprintf(HIY"本届小超人杯“Phiêu vương bá chiến”今天活动到此结束, 已经接受了任务的玩家可以继续Hoàn thành, 但成绩不会进入镖王排名计算, %d:%d将会给出今天Phiêu vương bá chiến结果, 请大家Chờ và xem.", mxTmp[TIME_HOUR], mxTmp[TIME_MIN]));
		set_game_flag(4);
		return;
	}
