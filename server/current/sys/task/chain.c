...
void give_chain_task(object who, object me, int type)
{
	...
	else if ( type == 5 )	//索要宝宝
	{
		if ( catch_animal(who,me) == 1 )
			cTmp = sprintf("%s:\n    不久前，"HIR"%s"NOR"丢失了一只"HIR"%s（宝宝）"NOR"，现在妖魔横行，我担心它被捉去被训练成妖怪出来害人，为了阻止此事的发生，你快去将它寻回来交给"HIR"%s"NOR"，如果你能Hoàn thành此事，定是苍生之福.\n"ESC"确定",me->get_name(),npcname,who->get_save_2("chain.pet"),npcname);
		else
		{
			clear_chain(who);	
			return ;
...
void send_task_list(object who, int type)
{
	...
	switch(type)
	{
		...
		case 2:
			...
			break;	
		case 3:
			send_user( who, "%c%c%c%w%w%s", 0x51, 1, 2,TID_CHAIN,type,"帮助"+who->get_save_2("chain.npcname")+"Thoát khỏi kẻ bắt nạt" );
			break;
		case 4:
...
void send_task_intro(object who)
{
	...
	case 4:
		...
		break;	
	case 5:
		result = sprintf(BLK"任务目的:\n"BLK"    寻一只%s（宝宝）给%s(第%d环，"HIR"%s)\n"BLK"任务描述:\n"BLK"    %s丢失了一只%s（宝宝），现在妖魔横行，%s担心它被捉去被训练成妖怪出来害人，为了阻止此事的发生，你快去将它寻回来交给%s，如果你能Hoàn thành此事，定是苍生之福.据说"HIR"%s"NOR".",cPet=who->get_save_2("chain.pet"),cName=who->get_save_2("chain.npcname"),who->get_save_2("chain.ring"),who->get_save_2("chain.save")?"已保存":"还有%d分钟",cName,cPet,cName,cName,who->get_save_2("chain.petinfo"));
		break;
	}
	if ( stringp(result) )
	{
...
string get_chain_menu(object who,object me)
{
	...
	case 3:
		if ( who->get_save_2("chain.status") != 99 )
			cTmp = sprintf(ESC HIG"Thoát khỏi kẻ bắt nạt%s\nchain %x# 31\n",who->get_save_2("chain.bossname"),id);
		else
			cTmp = sprintf(ESC HIG"Thoát khỏi kẻ bắt nạt%s(Hoàn thành任务)\nchain %x# %d\n",who->get_save_2("chain.bossname"),id,type);
		break;
	case 4:
...
void finish_task(object who,object me,string arg)
{
	...
	send_user(who,"%c%s",';',sprintf("任务链第%d环 经验 %d tiềm năng %d",ring,exp,pot));
	//任务链的相关处理
	clear_chain(who);
	send_user( who, "%c%c%c%w%w%s", 0x51, 1, 2,TID_CHAIN,type,"" );
...
int accept_object(object me,object who,object item)
{
	...
	finish_task(me,who,"");	//Hoàn thành任务
	return 1;	
}
//打传说，获得装备
