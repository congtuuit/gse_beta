...
//	mixed* mixTime, item;
//
//	iStart = mktime(2020, 9, 1, 8, 0, 0);
//	iFinish = mktime(2020, 9, 15, 23, 0, 0);
//	iHour = time();
//
//	if (iHour > iFinish)
//		return;
...
void do_look(object who, object me)
{
	...
	if (who->get_legend(TASK_2_00, 12) && !who->get_legend(TASK_2_00, 13))
		result += sprintf(ESC HIY "Đến viếng thăm\ntalk %x# visit.1\n", getoid(me));
	if (who->get_legend(TASK_2_00, 13) && !who->get_legend(TASK_2_00, 14))
		result += sprintf(ESC HIY "Cậu chủ lần Yuchi cũ\ntalk %x# visit.3\n", getoid(me));
	result += ESC "Rời khỏi\nCLOSE\n";
	if (stringp(result))
...
void do_task_start(object who, object me)
{
	...
	{
		send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), me->get_name() + ":\n    Nhận nhiệm vụ cần ít nhất là 1 người");
		return;
	}
	name = "";
...
//        &&    ( npc = find_char(robber) ) && npc->is_robber() )
//        {
//                send_user( who, "%c%s", '!', "请先Hoàn thành手上的剿匪任务吧." );
//                return;
//        }
...
void do_task_cancel(object who, object me)
{
	...
	//team->set_task("robber#", -1);    // 任务Hoàn thành数
...
void task_bonus(object who, object me)
{
	...
	//tip
	send_user(who, "%c%s", ';', sprintf("Ngươi hoàn thành %s nhiệm vụ (lượt thứ %d), Kinh nghiệm %+d, Tiềm năng %+d, Ngân lượng +500.", name, level2 + 1, exp2, pot2));
	clear_task_desc(who);
}
...
void task_giveup(object who, int iKind)
{
	...
	if (gone_time(who->get_task("robber.time")) < 180)
	{
		...
		if (item && item->get_item_number() == 10001078)
		{
			send_user(who, "%c%c%s", 0x59, 1, sprintf("Bạn có muốn loại bỏ khoảng thời gian nhiệm vụ của bạn?\n"
				ESC "%c\n"ESC"use %x# robber2\n"
				, 1, getoid(item)));
			return;
		}
...
void do_visit(object who, object npc, string arg)
{
	...
	if (who->get_legend(TASK_2_00, 12) && !who->get_legend(TASK_2_00, 13))
	{
		...
		else if (i == 2)
		{
			...
			who->add_cash(money);
			send_user(who, "%c%s", ';', sprintf("Hoàn thành nhiêm vụ bạn nhận được %d kinh nghiệm, %d tiềm năng, %d lượng", exp, pot, money));
	...
	if (who->get_legend(TASK_2_00, 13) && !who->get_legend(TASK_2_00, 14))
	{
		...
		else if (i == 4)
		{
			...
			send_user(who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"拜访尉迟老镖头");
		}
	}
