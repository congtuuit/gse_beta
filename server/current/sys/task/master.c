...
void event_bonus(object npc, object who) {
	...
	iStart = mktime(year, 11, 19, 8, 0, 0);
	iFinish = mktime(year, 11, 30, 23, 0, 0);
	iHour = time();
	...
	if (iHour < iFinish)
	{
		...
		item2 = new("item/event/2011/trong");
		if (item2) {
			p = item2->move(who, -1);
			if (p)
			{
...
void do_look(object player, object master)
{
	...
	if (player->get_save_2("master_hide"))
	{
		...
		if (!player->get_save("kjh_enhance_sk"))
		{
			...
			{
				result += sprintf(ESC HIY"Người xin cấp tăng cường các kỹ năng võ thuật\ntalk %x# apprentice.15\n", getoid(master));
			}
		}
	...
	if (!player->get_save("kjh_enhance_sk"))
	{
		if (player->get_level() >= 60
			&& (present("Thiên Ân Lệnh", player, 1, MAX_CARRY * 4)))
		{
			result += sprintf(ESC"Người xin cấp tăng cường các kỹ năng võ thuật\ntalk %x# apprentice.15\n", getoid(master));
		}
	}
	result += ESC"Quay về hướng của sư phụ mời nhấn\nCLOSE";
	send_user(player, "%c%s", ':', result);
...
void check_enhance_sk(object player, object master)
{
	...
	if (player->get_save("kjh_enhance_sk"))
	{
		send_user(player, "%c%s", ':', "Bạn không cần phải học các kỹ năng võ thuật để tăng cường chưa!!\n" + ESC + "Rời khỏi\nCLOSE");
		return;
	}
	...
	{
		send_user(player, "%c%s", ':', "Để học thêm kỹ năng gia cường của Môn Phái, phải có những điều kiện sau đây:\n1. Trường của con phải tương ứng với trường dạy nghề con đã học；\n2. Cấp độ của người chơi phải là ≥60；\n3. Người chơi phải có vật kỷ niệm để rèn luyện kỹ năng học tập (giới hạn)!\nHiện tại bạn không thể đáp ứng những điều kiện trên!\n" + ESC + "Rời khỏi\nCLOSE");
		return;
	}
	else
		if (item->remove_from_user())
		{
			...
			send_user(player, "%c%s", ':', "Bạn có thể học kỹ năng thăng tiến trong trường kỹ năng Môn Phái của bạn! Bạn có thể đóng hộp thoại hiện thời được rồi，Nhấn lần nữa Học kĩ năng để xem thông tin!\n" + ESC"Rời khỏi\nCLOSE");
			send_user(player, "%c%s", '!', "Yêu cầu học thêm kỹ năng giáo phái đã được thông qua");
		}
}
...
void task_info(object player, object master)
{
	...
	if (status != 99)
	{
		if (type == 1)
		{
			...
			{
				if (player->get("mastertask.finish"))		//抓宠 nhiệm vụHoàn thành
				{
