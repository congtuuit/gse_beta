...
#define MAP "map/copy/1707"

#define CREP_1 "map/copy/1707"
#define CREP_2 "map/copy/1707"

#define BOSS "map/copy/1707"
...
object new_copy_tomb1(object me, object who)
{
	...
	if (!tId)
		return 0;
	z = new_copy_scene(who, me, MAP, ({ 334,258 }), ({ 165,107 }), COPY_TOMB1);
	if (!z)
		return 0;
	if (!objectp(map = get_map_object(z)) || !map->is_virtual_scene())
		return 0;
...
object new_copy_map(object me, object who, int gateNumber)
{
	int tId, z, x, y, * nP, i, size, j, count;
	mapping mpTmp;
	object map, npc, * nNpc;
	...
	if (!tId)
		return 0;
	z = new_copy_scene(who, me, MAP, ({ 334,258 }), ({ 165,107 }), gateNumber);
	if (!z)
		return 0;
	if (!objectp(map = get_map_object(z)) || !map->is_virtual_scene())
		return 0;
