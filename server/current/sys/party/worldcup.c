...
int do_look(object who,object npc,int id)
{
	...
	size = sizeof(index);
	buf = sprintf("%s:\n    Mỗi ngày có giải thưởng. Đây là một hoạt động đoán mò cổ điển mà những người hâm mộ chuyên nghiệp phải tham gia. Họ đã tích tụ rất nhiều kiến thức về bóng đá và World Cup. Bây giờ họ có thể trao đổi giải thưởng.。\n",npc->get_name());
	for(i=0;i<size;i++)
	{
	...
	if ( mpBonus[who->get_number()] )
		buf += sprintf(ESC"Nhận phần thưởng\ntalk %x# bonus\n",getoid(npc));
	buf += ESC"Rời khỏi\nclose\n";
...
void add_bonus(int idx1, int idx2)
{
	...
	for(i=0,size=sizeof(userid);i<size;i++)
	{
		...
		if ( type == 2 )
		{
			if ( !mapp(mpChampion) )
				mpChampion = ([]);
			mpChampion[userid[i]] = mpTmp1[userid[i]][win];
			if ( random(total) < mpTmp1[userid[i]][win] )
			{
				if ( !mapp(winners) )
					winners = ([]);	
				winners[userid[i]] = mpTmp1[userid[i]]["name"];
			}
		}
	}
	save();
...
void get_bonus(object who,object npc)
{
	...
	if ( pot )
		send_user( who, "%c%s", '>',sprintf("Bạn được thưởng: Kinh nghiệm %d Ngân lượng %d Tiềm năng %d",exp,cash,pot));
	else
		send_user( who, "%c%s", '>',sprintf("Bạn được thưởng: Kinh nghiệm %d Ngân lượng %d",exp,cash));
	...
	if ( item )
	{
		if( USER_INVENTORY_D->can_carry(who, item)  >=1 )
		{
			...
			i =item->move(who, -1);
			item->add_to_user(i);
			send_user( who, "%c%s", '>', sprintf("Nhận được %s",name) );
		}
		else
		{
			destruct(item);
...
void set_score(object who,string arg)
{
	...
	if ( sscanf(arg,"%d %d:%d", idx, score1,score2) != 3 )
	{
		if ( to_int(arg) == 999 )
		{
			...
			init_time = 0;
			save();
			tell_user(who,"Tất cả kết quả và phần thưởng đã bị loại bỏ.");
			check_time();	
			remove_call_out("check_time");
		}
		return ;	
