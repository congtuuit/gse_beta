/*------------------------------------------------------------------------------------------*\
	惊天大爆装的活动控制文件
	07-06-26 	由 Cjxx 创建
	07-06-27  18:46 调试Hoàn thành时间控制框架
	07-06-28  18:00 Hoàn thành时间轮寻控制框架（修复之前一个严重BUG）
\*------------------------------------------------------------------------------------------*/
#include <ansi.h>
#include <time.h>
#include <public.h>
...
private string active_name	= "Big Bang";		//	活动名称
...
void set_active_time(int from, int to) {
	...
	_DEBUG(sprintf("Thiết lập 时间 %d <--> %d",active_from,active_to));
	save();
}
...
void set_cycle_way(string way) {
	...
	save();
	_DEBUG(sprintf("Thiết lập 运转周期为 %s",cycle_way));	
}
...
void set_npc_status(object npc,int level,int *skills,int *other_stat) {
	int i,size;
	for(i=0,size=sizeof(skills);i<size;i++) {	//	Thiết lập 技能
		npc->set_skill(skills[i],level);
	}
	if(other_stat[0]) {				//	0 Thiết lập 行进速度
		npc->set_walk_speed(other_stat[0]);
	}
	if(other_stat[1]) {				//	1 Thiết lập 攻击速度
...
void on_active_playing(int callout_delay_time) {
	...
	for(i=0,count=1;i<100;i++) {
		...
		NPC_ENERGY_D->init_level(renzha,level);	//	给这个家伙Thiết lập 一下等级
		renzha->set_max_seek(8);
		rand_int = random(100);			//	100 几率去做一些通用Thiết lập 
...
void on_active_finish(int callout_delay_time) {
	...
	//	活动结束 -- Thiết lập 老人渣的消失状态 -- NPC 内部也会进行相应的清楚操作，因此这里有双保险
