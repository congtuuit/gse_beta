...
//	else*/
	{
		...
		else
		{
			...
			mixTime[TIME_HOUR] == 12 || mixTime[TIME_HOUR] == 0 )
				call_out("check_time",i+3600);
			else
			{
				generate_scum();
...
//		y = p % 1000;
		...
		npc = new("/npc/event/PhoBan/Boss60/Tang1");
		npc->add_to_scene(151,84,35);	
		Scum += ({ npc });
//	}

	CHAT_D->sys_channel(0,HIC "Tuyệt đại Cao Thủ "HIG"Song Nhi"HIC" đã xuất hiện tại "HIY "Kênh Trịnh Quốc 1 "HIC"(84,35).");
}
