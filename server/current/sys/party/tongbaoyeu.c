...
void drop_items(object me, object who)
{
	...
	if (rate < 3000) {
		...
		item = new(sprintf("/item/91/916%d", random(5)));
	}
	else if (rate < 5500) {
...
void win_bonus_callout(object me, object who)
{
	...
	if (who->get_save("qcamnang") == 101 && who->get_save("qcamnang100") <= 0)
	{
		who->add_save("qcamnang100", 1);
		if (who->get_save("qcamnang100") >= 1)
		{
			send_user(who, "%c%c%c%w%w%s", 0x51, 2, 2, TID_NEWITEMS, 101, "Tổng bảo tiểu yêu (Hoàn thành)");
		}

		send_user(who, "%c%s", '!', sprintf("Tổng bảo tiểu yêu ( Hoàn thành)"));

		USER_TASK_D->send_task_intro(who, 2, TID_NEWITEMS, 101);
	}
	...
	for (i = 0; i < size; i++) if (team[i])
	{
		...
		write_user(team[i], sprintf(ECHO "Bạn tiêu diệt Chiến Quốc Tống Bảo Yêu và nhận được %d Kinh nghiệm!", exp2));
