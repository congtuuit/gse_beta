...
void do_welcome( object me, string arg )
{
	...
	mixed *mixTime;
		...
		if ( who->get_save("tanciyutu") )
		{
			...
			item->add_to_user(i);
                	send_user(who, "%c%s", ';', "恭喜您！Bạn nhận được 了嫦娥赠送的中秋节礼物——"HIG+item->get_name());
        		if ( cTmp )
...
int exchange( object who,object me,object yuebing) 
{
	...
	if ( item )
	{
		...
		send_user(who,"%c%s",'!',"恭喜您！Bạn nhận được 了"+item->get_name());
		item->add_to_user(p);	
		if ( name )
	...
	{
		who->add_cash(cash);
		send_user(who,"%c%s",'!',sprintf("恭喜您！Bạn nhận được 了%d金",cash));
	}
	else if (  pot )
	{
		who->add_potential(pot);
		send_user(who,"%c%s",'!',sprintf("恭喜您！Bạn nhận được 了%d点tiềm năng",pot));
	}
	else if (  exp )
	{
		who->add_exp(exp);
		send_user(who,"%c%s",'!',sprintf("恭喜您！Bạn nhận được 了%d点经验",exp));
	}
	return 1;
}
