...
//	iTime = time();
	...
	mixTime = localtime(iTime);
	get_xigua_uncle();
	CHAT_D->sys_channel(0,HIY"最近大话战国里风调雨顺，农作物都有了很好的收成，尤其是水果，更是大丰收。各个国家农民伯伯都非常开心，但是最近Dưa hấu伯伯却遇到了一些问题。他发现自己没有时间去把自己的Dưa hấu采集回来，于是想委托大话战国的好心玩家帮忙，大家愿意帮助Dưa hấu伯伯吗？");
...
//	iTime = time();
	...
	if ( iTime > iFinish1 )
	{
		...
		destruct_xigua();
		destruct_bad_npc();
		CHAT_D->sys_channel(0,HIY"国庆的七天活动到此完全结束了，谢谢大家的参与，"HIR"Dưa hấu伯伯"HIY"也对大家的衷心帮忙表示感谢，最后祝大家国庆快乐，游戏更快乐。");
		log_file( "national.dat", sprintf( "%d月%d日 %d ：%d 国庆活动全部结束。\n",mixTime[TIME_MON]+1,mixTime[TIME_MDAY],mixTime[TIME_HOUR],mixTime[TIME_MIN]) );
		return ;
	}
	if ( mixTime[TIME_HOUR] >= 20 )
	{
		CHAT_D->sys_channel(0,HIY"今天的的国庆活动就到此了，"HIR"Dưa hấu伯伯"NOR"很感谢大家的帮忙，请大家明天再继续帮助他采集Dưa hấu吧。");
	...
	{
		get_bad_npc();
		CHAT_D->sys_channel(0,HIY"大事不好了，一些坏小猴、坏小猪和坏大熊出现在了"HIR"Dưa hấu伯伯"HIY"的Dưa hấu田里，它们一定是想抢走Dưa hấu，大家赶紧把它们捉起来，然后交给"HIR"Dưa hấu伯伯"HIY"对它们略惩小戒吧！");
	...
	{
		get_xigua();
		CHAT_D->sys_channel(0,HIY"大话战国的国庆活动已经开始了，请大家前往"HIR"周国的126，120"HIY"找"HIR"Dưa hấu伯伯"HIY"，帮助他采集Dưa hấu吧。");
		call_out("check_time3",60*(60-mixTime[TIME_MIN]));
		log_file( "national.dat", sprintf( "%d月%d日 %d ：%d 放入Dưa hấu。\n",mixTime[TIME_MON]+1,mixTime[TIME_MDAY],mixTime[TIME_HOUR],mixTime[TIME_MIN]) );
		return ;
	}
}
// 放入Dưa hấu伯伯
