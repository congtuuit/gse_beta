...
string* qixi_rose = ({ "Hoa hồng xanh Tanabata","Hoa hồng trắng Tanabata","Hoa hồng đỏ Tanabata","Hoa hồng tím Tanabata", });
void get_qixi_npc();
...
void create()
{
	...
	iStart_qixi = mktime(year - 2, 1, 1, 7, 50, 0);
	iFinish_qixi = mktime(year - 2 , 12, 30, 23, 0, 0);
...
void check_time()
{
	...
	CHAT_D->sys_channel(0, HIY"Đồng hồ Cowherd sẽ rơi vào thế giới nhộn nhịp, tìm kiếm sự giúp đỡ, mọi người sẽ đến Zhou Guo!");
}
...
void task_giveup(object who, int iKind)
{
	...
	case 10:
		break;
	case 12:
		if (objectp(item = present("Ống gan", who, 1, MAX_CARRY)))
		{
			item->remove_from_user();
			destruct(item);
		}
		if (objectp(item = present("Ống dẫn dầu", who, 1, MAX_CARRY)))
		{
			item->remove_from_user();
			destruct(item);
		}
	...
	for (i = 0; i < size; i++)
	{
		if (team[i]->get_save_2("qixi.together") == together)
			MAILBOX->sys_mail(team[i]->get_id(), team[i]->get_number(), HIY"你的有缘人已经放弃了七夕天赐奇缘任务了，你如果想Hoàn thành此任务，需要放弃任务后，和你的"HIR"有缘人"HIY"在牛郎织女那儿重新开始。");
		//			send_user( team[i],"%c%s",';',sprintf(HIY"你的有缘人已经放弃了七夕天赐奇缘任务了，你如果想Hoàn thành此任务，需要放弃任务后，和你的有缘人在牛郎织女那儿重新开始。"));
	}
}
...
void do_qixi(object me, object who, string arg)
{
	...
	case 19:
		...
		if (team[0]->get_gender() != team[1]->get_gender() && team[0]->get_save_2("qixi.flag") == 1 && team[1]->get_save_2("qixi.flag") == 1 && team[0]->get_save_2("qixi.together") == team[1]->get_save_2("qixi.together") && very_good == 1)
		{
			...
			send_user(team[0], "%c%s", '!', "Nhận nhiệm vụ  "HIY"七夕天赐奇缘（1）");
			send_user(team[1], "%c%s", '!', "Nhận nhiệm vụ  "HIY"七夕天赐奇缘（1）");
		}
		break;
		...
		if (team[0]->get_save_2("qixi.flag") == 8 && team[1]->get_save_2("qixi.flag") == 8 && team[0]->get_gender() != team[1]->get_gender() && team[0]->get_save_2("qixi.together") == team[1]->get_save_2("qixi.together"))
		{
			...
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(),
				sprintf("%s：\n    我知道你们要找王母娘娘，不过，王母娘娘要我为她收集一味灵药，只要你把灵药采来，我就告诉你王母娘娘的行踪。听说"HIR"%s（%d,%d）"NOR"有此灵药，如果你们能帮我采回来，我可以立刻给他们解除天河的限制。不过灵药都有强大的灵兽守护着，你可要考虑清楚。\n"
					ESC "我考虑清楚了\ntalk %x# qixi.3\n"
					ESC "Rời khỏi.", me->get_name(), map->get_name(), x, y, getoid(me)));
		}
		...
		if (team[0]->get_save_2("qixi.flag") == 8 && team[1]->get_save_2("qixi.flag") == 8 && team[0]->get_gender() != team[1]->get_gender() && team[0]->get_save_2("qixi.together") == team[1]->get_save_2("qixi.together"))
		{
			...
			send_user(team[1], "%c%c%w%s", ':', 3, me->get_char_picid(),
				sprintf("%s：\n    玩家%s决定为王母娘娘采集灵药，你是否接受？\n"
					ESC "接受\ntalk %x# qixi.4\n"
					ESC "Rời khỏi.", me->get_name(), team[0]->get_name(), getoid(me)));
		}
		break;
	case 4:
		if (!team[0]->get_save_2("qixi.z")) return;
