...
void create()
{
	...
	iStart_b = mktime(year,10,19,9,50,0);
	iFinish1 = mktime(year,10,20,22,0,0);
	iFinish_a = mktime(year,10,23,22,0,0);
...
//	iTime = time();
	...
	get_party_item();
	CHAT_D->sys_channel(0,HIY"nghẹt thở bốn kho báu rải rác trong làng non và ngoại ô của bảy quốc gia.，大家赶快去帮重阳老人收集啊！");
	log_file( "chongyang.dat", sprintf( "%d月%d日 %d ：%d 放入重阳四宝。\n",mixTime[TIME_MON]+1,mixTime[TIME_MDAY],mixTime[TIME_HOUR],mixTime[TIME_MIN]) );
