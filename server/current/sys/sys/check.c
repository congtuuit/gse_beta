...
#define CHECK           INI_DIR "check.ini"     // 检查Thiết lập 文件
...
int is_invalid_name(string name)
{
    ...
    for (i = 0; i < size; i++) {
        ...
        //ghi_file("tennhanvatxxx.txt", sprintf("%d 1\n", name[i]));
...
int is_legal_name( object me, string name )
{
        ...
        if( len < 2 || len > 15 )
        {
            send_user( me, "%c%s", '!', "Tên nhân vật phải từ 2 tới 15 ký tự." );
            send_user(me, "%c%c", 0xff, 0x11); 
            return 0;
        }

		line = explode(read_file("../current/tennhanvat.txt"), "\n");
		for (i = 0, size = sizeof(line); i < size; i++) 
		{
			data = explode(line[i], " ");
			if (name == data[0]) 
		...
		if (exist >= 1) {
			...
			//log ten nhan vat
			ghi_file("../current/tennhanvat.txt",sprintf("%s 1\n",name));				
        }
