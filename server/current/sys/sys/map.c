...
void load_npc_file(object map, string file)
{
        ...
        for (i = 0, size = sizeof(line); i < size; i++)
        {
                ...
                {
                        log_file(ERROR, sprintf("Lỗi: %s ở dòng %d do định nghĩa nhân vật sai.\n", file, i + 1));
                        continue;
                }
...
void reset_npc(int id)
{
        ...
        if (mapp(officer = map->get_xy_dbase()))
        {
                for (i = 0, size = sizeof(key = keys(officer)); i < size; i++)
                {
                        ...
                        npc->set_channel(CHANNEL_1);
                        npc->set_name(sprintf("%s - %d", npc->get_name(), CHANNEL_1));
                        ...
                        {
                                npc = new (officer[key[i]]);
                                npc->set_channel(CHANNEL_2);
                                npc->set_name(sprintf("%s - %d", npc->get_name(), CHANNEL_2));
                                npc->add_to_scene(z, p / 1000, p % 1000, d);
                                set_block(z, get_x(npc), get_y(npc), FLY_BLOCK);
                                npc->set_start_place(sprintf("%d", p));
                                map->add_npc(key[i], npc);
                        ...
                        {
                                npc = new (officer[key[i]]);
                                npc->set_channel(CHANNEL_3);
                                npc->set_name(sprintf("%s - %d", npc->get_name(), CHANNEL_3));
                                npc->add_to_scene(z, p / 1000, p % 1000, d);
                                set_block(z, get_x(npc), get_y(npc), FLY_BLOCK);
                                npc->set_start_place(sprintf("%d", p));
                                map->add_npc(key[i], npc);
                        ...
                        {
                                npc = new (officer[key[i]]);
                                npc->set_channel(CHANNEL_4);
                                npc->set_name(sprintf("%s - %d", npc->get_name(), CHANNEL_4));
                                npc->add_to_scene(z, p / 1000, p % 1000, d);
                                set_block(z, get_x(npc), get_y(npc), FLY_BLOCK);
                                npc->set_start_place(sprintf("%d", p));
                                map->add_npc(key[i], npc);
