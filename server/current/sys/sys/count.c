...
static int iMaxUser = 100;
static int iMaxUserDefault= 100;
...
void create() 
{
	...
	//VMP DHN TS CVQ DM MS CL VMC
	hp1 = ({30, 100, 75, 85, 55, 85, 70, 65, });
	hp2 = ({10, 10, 10, 10, 10, 10, 10, 10, });

	mp1 = ({10, 8, 56, 8, 32, 56, 70, 80, });
	mp2 = ({10, 10, 10, 10, 10, 10, 10, 10, });

	ap1 = ({5, 7, 12, 14, 8, 2, 1, 2, });        
	ap2 = ({10, 10, 10, 10, 10, 10, 10, 10, });

	dp1 = ({16, 20, 30, 23, 25, 15, 10, 10, });   
	dp2 = ({10, 10, 10, 10, 10, 10, 10, 10, });

	cp1 = ({1, 1, 11, 1, 1, 8, 10, 7, });         
	cp2 = ({10, 10, 10, 10, 10, 10, 10, 10, });  

	pp1 = ({15, 17, 20, 15, 17, 25, 23, 30, });
	pp2 = ({10, 10, 10, 10, 10, 10, 10, 10, });

	sp1 = ({13, 18, 13, 25, 30, 13, 15, 13, });
...
{
	if ( MAIN_D->get_absolute_random_number() == 0 ) return iMaxUserDefault;
	return iMaxUser;
}
...
int set_hefu_time2(object who, string arg )
{
	...
	send_user(who,"%c%s",'!',"Thiết lập thành công！");	
	hefu_ii=ii;
	save();
	return hefu_time ;
}
