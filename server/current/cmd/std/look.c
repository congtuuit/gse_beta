        ...
        else if( who->is_char() )
        {
	        ...
	        {        	
                ...
                {        	
                	if (me->get_save_2("orgtask2.type")==2 && me->get_save_2("orgtask2.name")==who->get_name())
                	{
                		...
                		if (objectp(npc))
                		{
                			...
                			if (me->get_save_2("orgtask2.status")==1)
                			{
                				...
                				result += sprintf(ESC"Hoàn thành任务\nlook %x#\n", getoid(who));
						result += sprintf(ESC"Rời đi\nCLOSE\n");		
						...
						me->add_cash(cash);
						me->add_exp(exp);
						result = sprintf("你获得经验%d，tiềm năng%d，Ngân lượng%d，帮派贡献度%d点，帮派的人气提升2点、安定提升2点。\n", exp, pot, cash, 1+order*2/9 );
					...
					orgnpc->add_favour(1);					
					me->add_exp(1000);
					result = sprintf("你获得经验%d，tiềm năng%d，Ngân lượng%d，帮派贡献度%d点，帮派的人气提升1点、安定提升1点。", exp, pot, cash, 1+order*2/9);
					send_user( me, "%c%s", ';', result );	
					me->delete_save_2("orgtask2");	
