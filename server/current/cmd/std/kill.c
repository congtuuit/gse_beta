...
int main(object me, string arg)
{
	...
	if (me->is_store_open())
	{
		notify("Bạn đang đặt trên một gian hàng，Anh không thể chiến đấu.。");
		return 1;
	}
	...
	{
		notify("Con thú triệu hồi của riêng bạn không thể chiến đấu.!");
		return 1;
	}
	if (me == who)
	{
		notify("Anh muốn tự cắt xuống.？");
		return 1;
	}
	if (who->is_user() && who->get_login_flag() < 3)
	{
		send_user(me, "%c", 0x01);    // Hủy bỏ跟随攻击
		return 1;
	}
	if (who->is_die())
	{
