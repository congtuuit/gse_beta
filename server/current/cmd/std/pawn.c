...
int do_list(object me, string name, int which)
{
	...
	if ((time() - me->get("pawn_time") > 400) && !inside_screen_2(me, who))
	{
...
int do_pawn(object me, string name, string what, int amount, int which3)
{
	...
	if (item->get_value() < 1
		|| item->get_no_drop() || item->get_no_get() || item->get_no_pawn())
	{
	...
	/*if( item->is_locked() )
	{
		notify(item->get_name() + " không thể cầm aa");
		send_user(me, "%c%d%c", 0x4e, getoid(item), item->get_amount());
		return 1;
	} */
	...
	{
		notify(item->get_name() + " đã được dát vàng, không thể cất giữ.");
		send_user(me, "%c%d%c", 0x4e, getoid(item), item->get_amount());
		return 1;
	}
...
int do_redeem(object me, string name, int which2, int amount, int which3)
{
	...
	if (item->get_amount())
		result = sprintf("Bạn lấy ra từ %s lấy ra %s .", who->get_name(), item->get_name());
	else    result = sprintf("Bạn lấy ra từ %s lấy ra %s .", who->get_name(), item->get_name());
	notify(result);
	write_user(me, ECHO + result);
	if (item->is_record())
	{
