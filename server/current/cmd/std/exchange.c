...
//                      notify( "您找不到这位玩家。" );
                ...
                if( !stringp(who->get_2( "exchange.mine" )) )
		{
                ...
                {
                        notify( "Đối phương ngân lượng đã đạt tới cao nhất" );    // 您想Giao dịch的金额太大。
                        return 1;
                }
		for(i=0;i<3;i++)
		{	
			...
			if ( (obj3=find_char(sprintf("%x#",pet[i]))) && obj3->is_pet() && obj3->get_owner() == me )
			{
                ...
                for( i = 0; i < 6; i ++ ) if( index2[i] > 0 )
                {
                        ...
                        {
                                send_user( who, "%c%s", '!', sprintf("%sKhoá càn khôn mất hiệu lực còn thưa %d giờ", obj2[i]->get_name(), obj2[i]->is_locked()/3600) );
                                return 1;
                        }
		...
		if (d>0 && who->get_bag3_time()>0 && time()>who->get_bag3_time()) d = 0;
	        ...
	        allcount = MAX_CARRY + b + c + d;   
                ...
                {
                        send_user( who, "%c%s", '!', "Đối phương ngân lượng đã đạt tới cao nhất" );    // 您想Giao dịch的金额太大。
                        return 1;
                }
...
//                me->log_money("Giao dịch "+who->get_id(), gold2 - gold);
                who->add_cash2(gold - gold2);  who->add_gold_log("exchange", gold2);  who->add_gold_log("exchange2", gold);
//		who->log_money("Giao dịch "+me->get_id(), gold - gold2);
		if (gold-gold2>0)
		{
		...
		if (gold2-gold>0)
		{
			legend = sprintf("Giao dịch cấp %s,%d,%s,%s %s,%d,%s,%s lượng 0 %d 0 %d_%d_%d", who->get_name(), who->get_number(), get_ip_name(who), who->get_id(), me->get_name(), me->get_number(), get_ip_name(me), me->get_id(), gold2-gold, get_z(me), get_x(me), get_y(me) );
                ...
                send_user( me, "%c%c", 0x26, 0 );    // 关闭Giao dịch窗口
                send_user( who, "%c%c", 0x26, 0 );    // 关闭Giao dịch窗口
		//宠物存盘
		for(i=0;i<3;i++)
                {
                	if ( pet[i] && oPet[i] )
...
//                      notify( "您找不到这位玩家。" );
		...
		pet = ({0,0,0});
                ...
                for( i = 0; i < 6; i ++ ) if( index[i] > 0 )
                {
                        ...
                        if( obj[i]->is_locked()>0 )
                        {
                                notify( sprintf("%sKhoá càn khôn mất hiệu lực còn thưa %d giờ", obj[i]->get_name(), obj[i]->is_locked()/3600) );
                                send_user( me, "%c%c", 0x26, 4 );    // 己方物品错误
                                return 1;
                        }
                        if( me->get_store( sprintf( "%02d", get_d( obj[i] ) ) ) )
...
//                      notify( "您找不到这位玩家。" );
        ...
        else if( sscanf( arg, "- %s", id ) )    // 关闭Giao dịch窗口
        {
                me->delete_2("exchange");
                send_user( me, "%c%c", 0x26, 0 );    // 关闭Giao dịch窗口
...
//                      notify( "您找不到这位玩家。" );
        ...
        else if( who = find_player(arg) )    // 打开Giao dịch窗口
        {
                ...
                if( who->get_2( "exchange.flag" ) && who->get_2( "exchange.name" ) == me->get_number() )
                {
	                ...
	                me->set_2( "exchange.name", who->get_number() );                	
                        send_user( me, "%c%c%d%s", 0x26, 1, who->get_number(), who->get_name() );    // 打开Giao dịch窗口
                        send_user( who, "%c%c%d%s", 0x26, 1, me->get_number(), me->get_name() );    // 打开Giao dịch窗口
