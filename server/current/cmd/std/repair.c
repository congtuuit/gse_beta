...
int main(object me, string arg) {
	...
	if (flag == 7 || flag == 8) {
		...
		case 2:
			...
			break;
		default:
			result = "Thật vinh hạng cho ta có thể nhìn thấy được trang bị này !";
			break;
		}
		send_user(me, "%c%c%w%s", ':', 3, who->get_char_picid(),
			sprintf(" %s :\n %s \n"
	...
	if (arg == "all") {
		...
		for (i = 0, size = sizeof(inv); i < size; i++) {
			if (objectp(inv[i])) {
				...
				if (item->get_equip_type() == HAND_TYPE && item->get_lasting() != item->get_max_lasting()) value = 150000;
				if (flag == 3 || flag == 5) value *= 8;      			
				value += value * tax / 100 + (value * 10 / 100);
