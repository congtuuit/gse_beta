...
int main(object me, string arg)
{
	...
	if (!arg || sscanf(arg, "%s %s", arg, id2) != 2)
		return 1;
	if (arg != "follow" && arg != "bet" && arg != "gain" && arg != "give" && !objectp(who = find_player(id2)))
		return 1;
	...
	case "++":
		...
		// channel feature
		if (me->get_channel() != who->get_channel())
		{
			notify(sprintf("Người chơi đang ở kênh %d!", who->get_channel()));
			return 1;
		}
		...
		if (arrayp(team)) // 本人有队伍
		{
			...
			if (!objectp(map1 = get_map_object(get_z(me))))
				return 1;
		...
		else if (arrayp(team2)) // 对方有队伍
		{
			...
			if (get_z(who) == 801 || get_z(who) == 802 || get_z(who) == 803 || get_z(who) == 805 ||
				get_z(me) == 801 || get_z(me) == 802 || get_z(me) == 803 || get_z(me) == 805)
			{
				notify("Người chơi đang ở hang động, không thể lập nhóm lúc này!");
				return 1;
			}

			if (!map1 || !map2)
			{
				notify("Không thể lập nhóm lúc này!");
				return 1;
			}
		...
		if (arrayp(team3 = who->get("pending_team")) && member_array(me, team3) != -1) // 回应
		{
			...
			send_user(team, "%c%c%d%s", 0x43, 7, 0, sprintf(WHT "%s(%d) ở %s mời bạn gia nhập nhóm", oJoiner->get_name(), oJoiner->get_number(), oLeader->get_name()));
	...
	case "x":
		...
		if (me != who)
			send_user(team, "%c%c%d%s", 0x43, 7, 0, sprintf(WHT "Trưởng nhóm %s ( %d ) khai trừ %s ( %d )ra khỏi nhóm .", me->get_name(), me->get_number(), who->get_name(), who->get_number()));
		else
			send_user(team, "%c%c%d%s", 0x43, 7, 0, sprintf(WHT "Trưởng nhóm %s ( %d )đã rời khỏi nhóm.", me->get_name(), me->get_number()));
		TEAM_D->remove_member(who->get_leader(), who);
		who->add_to_scene(get_z(who), get_x(who), get_y(who), get_d(who)); // 重进场景
		return 1;
	...
	case "-":
		...
		if (me != who)
			send_user(me, "%c%s", ':', "Bạn muốn khai trừ " + who->get_name() + " ra khỏi nhóm phải không ？\n" ESC "Chấp nhận.\n" + cmd + ESC "Ta suy nghĩ lại……\nCLOSE\n");
		else
			send_user(me, "%c%s", ':', "Trưởng nhóm có chắc rằng muốn rời khỏi nhóm không?\n" ESC "Chấp nhận.\n" + cmd + ESC "Ta suy nghĩ lại……\nCLOSE\n");
		return 1;
	...
	case "=":
		...
		send_user(team, "%c%c%d%s", 0x43, 7, 0, sprintf(WHT "Trưởng nhóm %s(%d) đã nhường chức cho %s(%d) ", me->get_name(), me->get_number(), who->get_name(), who->get_number()));
