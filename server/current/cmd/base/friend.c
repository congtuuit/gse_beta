...
int main(object me, string arg)
{
	...
	if (sscanf(arg, "+ %d", id))
	{
		...
		{
			notify("Người chơi đang ở kênh khác.");
			return 1;
		}
		...
		if (size >= 150)
		{
			notify("Số lượng bằng hữu đã đạt tối đa 150 người rồi.");
			return 1;
		}
		me->add_friend(who, 0);
	...
	else if (sscanf(arg, "* %d", id))
	{
		...
		if (size >= 100)
		{
			notify("Số lượng bằng hữu đã đạt tối đa 100 người rồi.");
			return 1;
		}
		me->add_friend(who, 1);
		send_user(me, "%c%c%d", 0x46, 9, who->get_number());
	...
	{
		send_user(me, "%c%s", ':', sprintf("Bạn xác định là phải từ bỏ quan hệ với " HIR "%s(%d)" NOR " phải không ?\n" ESC "Ta quyết định đoạn tuyệt\nfriend x %d\n" ESC "Ta phải suy nghĩ lại……\nCLOSE\n", me->get_friend_name(id), id, id));
	}
