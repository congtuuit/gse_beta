	...
	if( arg[0] == '#' && ( arg[1] < '0' || arg[1] > '9' ) )
	{
	        arg = arg[1..<1];
                ...
                if( !is_player(me) || random(5) )    // ＧＭ造谣不漏
                {
                        ...
                        CHAT_D->rumor_gm_channel( 0, sprintf( "%s %s ", "Một người nào đó", arg/*, me->get_name(), me->get_number()*/ ) );
                }
                else 
                {
                        number = me->get_number();
                        CHAT_D->rumor_player_channel( number, sprintf( CHAT "%s%s", name, arg ) );
                        CHAT_D->rumor_gm_channel( number, sprintf( "%s %s ", name, arg/*, me->get_name(), me->get_number()*/ ) );
                }
	}
	else if( arg[0] == '*' )
	{
	        arg = arg[1..<1];
                ...
                if( !is_player(me) || random(5) ) 
                {
                        ...
                        CHAT_D->rumor_gm_channel( 0, sprintf( CHAT "%s ", arg/*, me->get_name(), me->get_number()*/ ) );
                }
                else
                {
                        if( sscanf( arg, "%s %s", arg, id ) == 2 )
                        ...
                        CHAT_D->rumor_gm_channel( number, sprintf( CHAT "%s ", arg/*, me->get_name(), me->get_number()*/ ) );
                }
	}
	else
	{
                ...
                {
                        CHAT_D->rumor_player_channel( 0, sprintf( CHAT "%s: %s", "Một người nào đó", arg ) );
                        CHAT_D->rumor_gm_channel( 0, sprintf( CHAT "%s: %s ", "Một người nào đó", arg/*, me->get_name(), me->get_number()*/ ) );
                }
                else
                {
                        number = me->get_number();
                        CHAT_D->rumor_player_channel( number, sprintf( CHAT "%s: %s", name, arg ) );
                        CHAT_D->rumor_gm_channel( number, sprintf( CHAT "%s: %s", name, arg/*, me->get_name(), me->get_number()*/ ) );
                }
	}
