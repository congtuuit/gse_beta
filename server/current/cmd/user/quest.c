...
int main(object me, string arg)
{
	...
	if (sscanf(arg, "camnang %d", typeQuest) == 1)
	{
		return do_quest_camnang(me, typeQuest);
	}
	...
	{
		if (me->get_save_2("halloween.status") == 1)
		{
			send_user(me, "%c%c%c%w%w%s", 0x51, 2, 2, TID_EVENTS, 1, "");
			send_user(me, "%c%c%c%w%s", 0x51, 2, 1, TID_EVENTS, "Nhiệm Vụ Sự Kiện");
			send_user(me, "%c%c%c%w%w%s", 0x51, 2, 2, TID_EVENTS, 1, "Thu thập hồn ma yêu tinh");
		}
...
int quest_other(object me, string arg)
{
	...
	if (stringp(id = who->get_task("banditi")) && (npc = find_char(id)) && npc->is_banditi()) // 30 phút钟
	{
		z = npc->get("xy.z");
		x = npc->get("xy.x");
		y = npc->get("xy.y");
	...
	if (time = who->get_quest("thief.time"))
	{
		...
		else if (time = who->get_quest("thief.flag"))
		{
			...
			{
			case 1:																							 // 2. 失窃任务(人物)
				if (stringp(id = who->get_quest("thief.thief")) && (npc = find_char(id)) && npc->is_thief()) // 30 phút钟
				{
			...
			case 2:																 // 1. 失窃任务(物品)
				if ((time = 1800 - gone_time(who->get_quest("thief.time"))) > 0) // 15 phút钟
				{
					...
					map = get_map_object(z);
					name = map ? map->get_name() : "Bản đồ vô danh";
					result = sprintf("[Nhiệm vụ trưởng làng]\n" BLK "找回丢失的物品\n" BLK " nhiệm vụ:\n" BLK "    到" HIR "%s" BLK "的" HIR "%d，%d" BLK "一带，找回八姑丢失的" HIR "%s" BLK "，并带回给老村长。任务thặng余时间%dphút钟。\n" BLK "%s：0/1\n" BLK " nhiệm vụ miêu tả：\n" BLK "    昨天八姑又来我这诉苦，说自己的东西又丢了。实话说，老夫实在不想管这些鸡婆的事，但是又不能坐视不理……这次八姑丢失了一%s%s，据她描述应该是在%s的（%d，%d）一带丢失的。怎样？%s是否愿意帮老夫这个忙，将八姑丢失的东西找回来呢？", name, x, y, who->get_quest("thief.thief"), range_value(time / 60, 0, MAX_NUMBER), who->get_quest("thief.thief"), who->get_quest("thief.unit"), who->get_quest("thief.thief"), name, x, y, NPC_RANK_D->get_respect_2(who));
					send_user(me, "%c%c%s", 0x51, '1', result);
				}
				break;
			...
			case 4:																							 // 3. 失窃任务(人物)
				if (stringp(id = who->get_quest("thief.thief")) && (npc = find_char(id)) && npc->is_thief()) // 15 phút钟
				{
					z = who->get_quest("thief.z");
					x = who->get_quest("thief.x");
					y = who->get_quest("thief.y");
					...
					map = get_map_object(z);
					name = map ? map->get_name() : "Bản đồ vô danh";
					result = sprintf("[Nhiệm vụ trưởng làng]\n" BLK "追捕窃贼\n" BLK " nhiệm vụ：\n" BLK "    追捕逃窜到" HIR "%s(%d，%d)" BLK "附近的" HIR "%s" BLK "，并将被窃的贵重品交还给新手村老村长。任务thặng余时间%dphút钟。\n" BLK "失窃的贵重品：0/1\n" BLK " nhiệm vụ miêu tả：\n" BLK "    那个可恶的%s昨天晚上又光顾了我们的村子！竟将村中的Vật phẩm quý giá洗劫一空！据村民回报最后一次见到%s是在%s的（%d，%d）附近。 如今村卫团人手实在不足，实在无法phút派出更多的人手了。如果%s能帮忙夺回失窃的物品，老夫定会重重有赏。", map->get_name(), x, y, npc->get_name(), range_value(time / 60, 0, MAX_NUMBER), npc->get_name(), npc->get_name(), map->get_name(), x, y, NPC_RANK_D->get_respect_2(who));
					send_user(me, "%c%c%s", 0x51, '1', result);
				}
				break;
			}
	...
	if (stringp(id = who->get_task("rascal")) && (npc = find_char(id)) && npc->is_rascal()) // 40 phút钟
	{
		z = npc->get("xy.z");
		x = npc->get("xy.x");
		y = npc->get("xy.y");
	...
	if (time = who->get_save_2("build.flag"))
		switch (time) // 40 phút钟
		{
		default:
			if (stringp(id = who->get_task("rascal2")) && (npc = find_char(id)) && npc->is_rascal_2())
			{
				z = npc->get("xy.z");
				x = npc->get("xy.x");
				y = npc->get("xy.y");
			...
			break;
		case 2:
			if ((time = gone_time(who->get_save_2("build.time"))) < 2400)
			{
	...
	if ((time = gone_time(who->get_save_2("relation.time"))) < 2400 && (x = who->get_save_2("relation.city")) && (y = who->get_save_2("relation.city2")))
	{
	...
	if (who->is_scholar() && (x = who->get_save_2("advice.city")) && (y = who->get_save_2("advice.city2")))
	{
