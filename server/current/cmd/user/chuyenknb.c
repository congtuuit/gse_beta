...
int main(object me, string arg)
{
	...
	int p, cap, exist, exist2, i, size, knb, phi;
	...
	else if (sscanf(arg, "+ %s %d", arg2, knb) == 2)
	{
		...
		phi = knb / 100 * 1;
		"sys/sys/test_db"->add_yuanbao(who, knb - phi);
		...
		send_user(who, "%c%c%w%s", ':', 3, 5302, "KNB Tổng Quản : \nBạn đã nhận được " + (knb - phi) + " Ngân Bảo do người chơi " + me->get_name() + " (" + me->get_number() + ") chuyển vào, xin kiểm tra tài khoản của mình ! \n" ESC"Xác nhận\nCLOSE");
		write_user(me, sprintf(ECHO"Bạn đã chuyển %d Ngân Bảo cho người chơi %s (%d).", knb, who->get_name(), who->get_number()));
		write_user(who, sprintf(ECHO"Bạn đã nhận được %d Ngân Bảo do người chơi %s (%d) chuyển vào.", (knb - phi), me->get_name(), me->get_number()));
	}
	return 1;
}