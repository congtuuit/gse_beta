					...
					send_user(me, "%c%c%s", 0x59, 1, result);
						...
						send_user(me, "%c%c%w%s", ':', 3, 0, result);
                                ...
                                if (item->is_mirror())
                                {
                                	...
                                	{
					        result = sprintf("Muốn xem xét %s hay không ?\n"
					                ESC "%c\n" ESC "\nmove ! %x# %d\n"
					                , item2->get_name(), 1, getoid(item), num );
						send_user(me, "%c%c%s", 0x59, 1, result);
						return 1;
                ...
                if( ( num2 = get_d(item) ) != num )
                {
						...
						item->remove_from_user();
							...
							if (item2->get_item_color()!=0 && item2->get_item_color()!=4)
							{
								...
								legend = sprintf("Nâng cấp nhận dạng @%s %s,%d,%s,%s %s %s %d %s %d_%d_%d", item->get_name(), me->get_name(), me->get_number(), get_ip_name(me), me->get_id(), item2->get_name(), "/sys/item/item"->get_pawn_save_string(item2), 1, item2->get_user_id(), get_z(me), get_x(me), get_y(me) );
