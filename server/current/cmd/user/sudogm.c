...
int main(object me, string arg)
{
	...
	if (arg) {
		...
		//Go to map
		if (sscanf(arg, "go %d %d %d", z, x, y) == 3) {
			p = get_valid_xy(z, x, y, IS_CHAR_BLOCK);
			type = -1;
		}
		...
		if (type == -1) {
			if (p)
			{
				...
				tell_user(me, ECHO"Đã di chuyển tới: %d (%d, %d)", z, x, y);
				return 1;
			}
			else
			{
		...
		if (arg == "go") {
			tell_user(me, "Map id: %d", z);
			send_user(me, "%c%c%d%s", '?', 16, 12, "Nhap id map toa do：\n" ESC"sudogm go %s\n");
			return 1;
		}
				...
				if (type == 100) {
					...
					if (have_scene_object(z, x, y, USER_TYPE, 1))
					{
						...
						for (i = 0, size = sizeof(item); i < size; i++)
						{
							if (who = item[i])
							{
								tell_user(me, ECHO"USER: %s (%d, %d)", who->get_name(), get_x(who), get_y(who));
							}
						}
					}
				}

				if (type == CHAR_TYPE || type == USER_TYPE) {
					if (have_scene_object(z, x, y, type, 1))
					{
						item = get_scene_object(z, x, y, type, 1);
						for (i = 0, size = sizeof(item); i < size; i++)
						{
							if (who = item[i])
							{
								tell_user(me, ECHO"%s (%d, %d)", who->get_name(), get_x(who), get_y(who));
							}
						}
					}
				}
