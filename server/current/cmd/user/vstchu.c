...
int main(object me, string arg)
{
	...
	if (sscanf(arg, "+ %s", arg2) == 1)
	{
		...
		{
			if (mixTime[TIME_HOUR] >= 18 && mixTime[TIME_HOUR] <= 21) {
		...
		if ("sys/party/vst4"->check_revive(me) > 0)
		{
			"sys/party/vst4"->add_revive(me, 1);
			who->set_chblk("chat", 16 * 60);
			who->set_chblk("rumor", 16 * 60);
			who->set_chblk("family", 16 * 60);
			who->set_chblk("say", 16 * 60);
			who->set_chblk("city", 16 * 60);
			who->set_chblk("vipchat", 16 * 60);
			who->set_chblk("time", time());
			who->set_save("nomail", time() + 16 * 60);
	...
	else if (sscanf(arg, "- %s", arg2) == 1)
	{
		...
		//Nhot 1 nguoi 1 lan 1 ngay
		/*if (who->get_save("vstchuToday") ==  (mixTime[TIME_WDAY] + 1)) {
			send_user(me, "%c%c%w%s", ':', 3, 5502, "Đặc quyền Vô Song : \nNgười này hôm nay đã bị nhốt rồi ! \n"ESC"Xác nhận");
			return 1;
		}*/

		if (mixTime[TIME_HOUR] >= 18 && mixTime[TIME_HOUR] < 21) {
			write_user(me, ECHO "Khung giờ này tạm thời không thể sử dụng !");
			send_user(me, "%c%c%w%s", ':', 3, 5502, "Đặc quyền Vô Song : \nKhung giờ này tạm thời không thể sử dụng ! \n"ESC"Xác nhận");
			return 1;
		}
		...
		if ("sys/party/vst4"->check_revive2(me) > 0)
		{
			if (gone_time(me->get_save_2("nhottu.timentuben")) < 900)
			{
			...
			if (get_z(who) != 704)
			{
				if (get_effect(who, EFFECT_TRAVEL)) {
					write_user(me, ECHO "Đối phương đang bận, thử lại sau !");
					return 1;
				}
				
				if (get_effect(who, EFFECT_USER_LOGIN)) {
					write_user(me, ECHO "Đối phương đang bận, thử lại sau !");
					return 1;
				}

				me->set_save_2("nhottu.timentuben", time());
				...
				who->set_save("vaotu", 16 * 60);
				who->set_save("vaotuluc", time());
				who->set_save("vstchuToday", (mixTime[TIME_WDAY] + 1));
				...
				send_user(who, "%c%c%w%s", ':', 3, 9963, "Giám Ngục Quan :\nNgươi đã bị Vô Song Thành Chủ nhốt vào tù trong thời gian 15 phút !\n"
					ESC "Xác nhận\nCLOSE\n");
				send_user(me, "%c%s", ';', sprintf("%s (%d) đã bị Nhốt vào tù và Phong bế kênh Chat trong 15 phút", who->get_name(), who->get_number()));
	...
	{
		if ("sys/party/vst4"->check_revive3(me) > 0)
		{
	...
	else if (sscanf(arg, "? %s", arg2) == 1)
	{
		...
		if ("sys/party/vst4"->check_revive4(me) > 0)
		{
			"sys/party/vst4"->add_revive4(me, 1);
			who->set_save("havecamchat", 1);
			who->set_save("timecamchat", time());
