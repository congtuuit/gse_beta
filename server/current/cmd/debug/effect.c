...
int main(object me, string arg)
{
	...
	if (what->is_npc())
		tell_user(me, "Triệu chứng：%s", what->get_name());
	else    tell_user(me, "Triệu chứng：%s(%d)", what->get_name(), what->get_number());
	...
	if (effect3 = get_effect_flag(what, 3))    // 简单征状标志３(３２位)
	{
		...
		if (effect3 & 0xffff0000)
		{
			if (effect3 & 0x00ff0000)    // 80
			{
				...
				if (effect3 & 0x00010000) effect += ({ sprintf("PK: %d", me->get_pk()) });
				if (effect3 & 0x00020000) effect += ({ sprintf("PK_RED: %d", me->get_pk()) });
