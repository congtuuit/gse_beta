...
int main(object me, string arg2)
{
	...
	if (!(who = find_player(arg)))
	{
		...
		*/
		...
		if (!who->restore())
		{
			send_user(me, "%c%c%s", 0xa1, 3, "Người dùng này không tồn tại！");
			destruct(who);
			return 1;
		}
		...
		{
			destruct(who);
			send_user(me, "%c%s", '!', "Thành viên này đang trực tuyến！");
			return 1;
		}
		else
			flag = 1;
	...
	{
		send_user(me, "%c%c%s", 0xa1, 3, "Thành viên này đang trực tuyến！");
		return 1;
	}
	arg = ids[0];
	if (sizeof(ids) == 1)
	{
		...
		if (stringp(result = who->get_start_place()) && sscanf(result, "%d:(%d,%d)%d", z, x, y, i) == 4)
		{
	...
	case 37:
		...
		break;
	default:
		send_user(me, "%c%c%s", 0xa1, 3, "Bạn không thể sửa đổi giá trị của mặt hàng tương ứng");
		if (flag == 1)
			destruct(who);
		return 1;
	}
	log_file("gm/op.dat", sprintf("%s %s(%d) Sửa %s(%d) %s %s\n", short_time(), me->get_id(), me->get_number(), who->get_id(), who->get_number(), ids[1], ids[2]));
