...
void win_bonus_callout( object me, object who )
{
        ...
        who->add_exp(20);
        send_user( who, "%c%s", ']', "Bạn nhận được " HIY "经验" NOR " 20 点！");
        who->add_potential(2);
        send_user( who, "%c%s", ']', "Bạn nhận được " HIY "tiềm năng" NOR " 2 点！");        
        if( status >= 6 && !who->get_legend(TASK_NEWBIE_00, 7) )
        {
                ...
                send_user(who, "%c%s", '!', sprintf("还剩下%d个目标没有Hoàn thành。", 6 - status ));                
