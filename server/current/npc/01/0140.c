	...
	string cmd;
        ...
        case 1:
        	if (who->get_legend(TASK_NEWBIE_00, 17) && !who->get_legend(TASK_NEWBIE_00, 18))
        	{
                	...
                	send_user( who, "%c%s", ':', get_name() + "：\n    真是稀奇，竟然有人会帮奇怪的稻草人的忙？可见少侠真是个善心之人。请稍候片刻，我给你弄一个。\n    这块Đá Tinh Linh是我以前从长白山的峰顶长白天池深处采集回来的，汇集了天地间的灵气，或许能派上用场，你就把它给稻草人装上吧。\n" 
                        	ESC "索取Đá Tinh Linh\n" + cmd + ESC"Rời khỏi." );        		
        	}
        	break;
        case 2:
        	if (who->get_legend(TASK_NEWBIE_00, 17) && !who->get_legend(TASK_NEWBIE_00, 18))
        	{
					...
					who->set_legend(TASK_NEWBIE_00, 18);
	                                send_user( who, "%c%s", '>', "Bạn nhận được " HIY "Đá Tinh Linh" NOR " 1 块！");    
	                                send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_NEWBIE_01,19,"" );
	                                send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_NEWBIE_01,19,"稻草人的委托(已经Hoàn thành)" );
	                        }                            
	                        else
	                        {
	                                destruct(item); 
