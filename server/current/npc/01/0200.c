...
void do_look(object who)
{
	...
	+ ESC"400金收购小蚂蚁一只\ntalk %x# welcome.4\n"
	+ ESC"600金收购斑点蛙一只\ntalk %x# welcome.5\n"
	+ESC"Rời khỏi.",me->get_name(),id,id,id,id,id,));
}
...
void do_visit2( object me, string arg )
{
	...
	case 12:
	        if( who->get_legend(TASK_49, 1) && !who->get_legend(TASK_49, 2) )
		{
			...
			who->add_cash(500);
			send_user(who,"%c%s",';',"Đau đầu（1） 经验 650 tiềm năng 20 Ngân lượng 500");
...
//			send_user(who,"%c%s",';',"你得到"HIY"10根乾坤绳");
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_NEWBIE_01,114,"Đau đầu（2）" );
...
//			who->add_cash(500);
			send_user(who,"%c%s",';',"Đau đầu（2） 经验 550 tiềm năng 30 灵芝草 1");
			TASK_LEGEND_D->task_finish(who);
