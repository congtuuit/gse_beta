...
void do_welcome2( object me, string arg )
{
        ...
        case 6:
                cmd1 = sprintf("talk %x# welcome.4", getoid(me));
                result = sprintf("%s：\n    不错，这么快就看到重点了。在往后的游戏中，当你达到 10 级以后，便有资格拜入门派学习更高深的武艺。如今天下一共有七大门派，分别是：‘" HIR "Hoa đào源、禁卫军、蜀山、唐门、茅山、昆仑山和云梦泽" NOR "’。七个门派各有特色，并分散在天下各地。到时你可以去找驻留在" HIR "新手村的门派向导" NOR "，他会送你到门派去的。\n"
                        ESC "返回\n%s\n", me->get_name(), cmd1 );                
                send_user( who, "%c%c%w%s", ':', 4, 4153, result );
                break;                                                                                                
        case 7:
        ...
        case 8:
                cmd1 = sprintf("talk %x# welcome.4", getoid(me));
                result = sprintf("%s：\n    这很简单，每次你消灭怪物后，自然会得到一定的tiềm năng奖励，但是这个奖励会随着你与怪物之间的等级差距有所修正。你与怪物之间的等级差距越大，所能获得的tiềm năng会越少。一些特殊的药品也能增加你的tiềm năng值，只不过这种药品相当罕见。\n"
                        ESC "返回\n%s\n", me->get_name(), cmd1 );                
                send_user( who, "%c%c%w%s", ':', 4, 4153, result );
                break;                                                                                                    
        case 9:
                cmd1 = sprintf("talk %x# welcome.10", getoid(me));
                result = sprintf("%s：\n    好，继续！谈到战斗，就离不开战斗时所运用的各种武功招式。要学会这些招式，就必须先从相应的" HIR "技能修炼" NOR "开始，只要将某个技能修炼到一定的等级，自然可以领会其包含的" HIR "武功" NOR "。\n    所有的武功技能在每个门派的师傅处都能学到，不过相应的代价就是要消耗定量的" HIR "tiềm năng" NOR "与Ngân lượng。随着修炼技能等级的越高，这个消耗便会越大。\n    除了门派武功之外，你加入帮派后还可以学习到一些特殊的帮派技能武功。\n"
                        ESC "继续\n%s\n"
                        ESC "关闭\nCLOSE\n", me->get_name(), cmd1 );                
                send_user( who, "%c%c%w%s", ':', 4, 4153, result );
                break;       
        case 10:
                result = sprintf("%s：\n    能教你的我都教了，以后你也许还会面对更多的难关。一切都得看自己的造化。还有什么不懂的地方就去拜访其他几位老师。如果要Rời khỏi.仙灵岛的话，你找百合仙子帮忙就可以了。\n"
                        ESC "关闭\nCLOSE\n", me->get_name(), cmd1 );                
                send_user( who, "%c%c%w%s", ':', 4, 4153, result );
                break;                                    
        }
