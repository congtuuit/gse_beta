...
void do_look( object who ) 
{
	...
	if(who->get_save_2("chuyensinh.score")==0) { knb = 400; cap = 150; }
	else if(who->get_save_2("chuyensinh.score")==1) { knb = 800; cap = 145; }
	else if(who->get_save_2("chuyensinh.score")==2) { knb = 1200; cap = 140; }
	else if(who->get_save_2("chuyensinh.score")==3) { knb = 1600; cap = 135; }
	else if(who->get_save_2("chuyensinh.score")==4) { knb = 2000; cap = 130; }
	else if(who->get_save_2("chuyensinh.score")==5) { knb = 2400; cap = 125; }
	else if(who->get_save_2("chuyensinh.score")==6) { knb = 2800; cap = 120; }
	else if(who->get_save_2("chuyensinh.score")==7) { knb = 3200; cap = 115; }
	else if(who->get_save_2("chuyensinh.score")==8) { knb = 3600; cap = 110; }
	else if(who->get_save_2("chuyensinh.score")==9) { knb = 4000; cap = 105; }
	...
	else if ( who->get_save_2("chuyensinh.score") == 1 )
	{
	...
	else if ( who->get_save_2("chuyensinh.score") == 2 )
	{
	...
	else if ( who->get_save_2("chuyensinh.score") == 3 )
	{
	...
	else if ( who->get_save_2("chuyensinh.score") == 4 )
	{
	...
	else if ( who->get_save_2("chuyensinh.score") == 5 )
	{
	...
	else if ( who->get_save_2("chuyensinh.score") == 6 )
	{
	...
	else if ( who->get_save_2("chuyensinh.score") == 7 )
	{
	...
	else if ( who->get_save_2("chuyensinh.score") == 8 )
	{
	...
	else if ( who->get_save_2("chuyensinh.score") == 9 )
	{
	...
	string *nTmp,cFile,cTmp;
        ...
        switch(flag)
        {
       	case 1:
			break;
		case 2:
			if ( who->get_save_2("chuyensinh.score")>=10 )
				{
			...
			if(who->get_save_2("chuyensinh.score")==1)
			{	
				if ( who->get_level() < 145 )
				{
				send_user(who,"%c%s",':',me->get_name()+":\n    Cấp độ chưa đạt tới cấp 145, không thể Chuyển Sinh Lần 2 !!!\n"ESC"Rời khỏi.");
				return;
				}
				else if ( "sys/sys/test_db"->get_yuanbao(who) < 8000 )
				{
				send_user(who,"%c%s",':',me->get_name()+":\n    Không đủ 800 Kim Nguyên Bảo, không thể Chuyển Sinh Lần 2 !!!\n"ESC"Rời khỏi.");
				return;
				}
			...
			USER_D->user_channel(sprintf(HIG "Chúc mừng "HIC "%s" HIG " đã Chuyển Sinh lần thứ "HIC " %d  "HIG " . ", who->get_name(),who->get_save_2("chuyensinh.score") ));
			who->set_level(who->get_level()-135);
			who->count_gift();
			who->add_gift_point(250);
			return 0;
			}
			if(who->get_save_2("chuyensinh.score")==2)
			{	
				if ( who->get_level() < 140 )
				{
				send_user(who,"%c%s",':',me->get_name()+":\n    Cấp độ chưa đạt tới cấp 140, không thể Chuyển Sinh Lần 3 !!!\n"ESC"Rời khỏi.");
				return;
				}
				else if ( "sys/sys/test_db"->get_yuanbao(who) < 12000 )
				{
				send_user(who,"%c%s",':',me->get_name()+":\n    Không đủ 1200 Kim Nguyên Bảo, không thể Chuyển Sinh Lần 3 !!!\n"ESC"Rời khỏi.");
				return;
				}
			...
			USER_D->user_channel(sprintf(HIG "Chúc mừng "HIC "%s" HIG " đã Chuyển Sinh lần thứ "HIC " %d  "HIG " . ", who->get_name(),who->get_save_2("chuyensinh.score") ));
			who->set_level(who->get_level()-130);
			who->count_gift();
			who->add_gift_point(250);
			return 0;
			}
			if(who->get_save_2("chuyensinh.score")==3)
			{	
				if ( who->get_level() < 135 )
				{
				send_user(who,"%c%s",':',me->get_name()+":\n    Cấp độ chưa đạt tới cấp 135, không thể Chuyển Sinh Lần 4 !!!\n"ESC"Rời khỏi.");
				return;
				}
				else if ( "sys/sys/test_db"->get_yuanbao(who) < 16000 )
				{
				send_user(who,"%c%s",':',me->get_name()+":\n    Không đủ 1600 Kim Nguyên Bảo, không thể Chuyển Sinh Lần 4 !!!\n"ESC"Rời khỏi.");
				return;
				}
			...
			USER_D->user_channel(sprintf(HIG "Chúc mừng "HIC "%s" HIG " đã Chuyển Sinh lần thứ "HIC " %d  "HIG " . ", who->get_name(),who->get_save_2("chuyensinh.score") ));
			who->set_level(who->get_level()-125);
			who->count_gift();
			who->add_gift_point(250);
			...
			if(who->get_save_2("chuyensinh.score")==4)
			{	
				if ( who->get_level() < 130 )
				{
				send_user(who,"%c%s",':',me->get_name()+":\n    Cấp độ chưa đạt tới cấp 130, không thể Chuyển Sinh Lần 5 !!!\n"ESC"Rời khỏi.");
				return;
				}
				else if ( "sys/sys/test_db"->get_yuanbao(who) < 20000 )
				{
				send_user(who,"%c%s",':',me->get_name()+":\n    Không đủ 2000 Kim Nguyên Bảo, không thể Chuyển Sinh Lần 5 !!!\n"ESC"Rời khỏi.");
				return;
				}
			...
			USER_D->user_channel(sprintf(HIG "Chúc mừng "HIC "%s" HIG " đã Chuyển Sinh lần thứ "HIC " %d  "HIG " . ", who->get_name(),who->get_save_2("chuyensinh.score") ));
			who->set_level(who->get_level()-120);
			who->count_gift();
			who->add_gift_point(250);
			return 0;
			}
			if(who->get_save_2("chuyensinh.score")==5)
			{	
				if ( who->get_level() < 125 )
				{
				send_user(who,"%c%s",':',me->get_name()+":\n    Cấp độ chưa đạt tới cấp 125, không thể Chuyển Sinh Lần 6 !!!\n"ESC"Rời khỏi.");
				return;
				}
				else if ( "sys/sys/test_db"->get_yuanbao(who) < 24000 )
				{
				send_user(who,"%c%s",':',me->get_name()+":\n    Không đủ 2400 Kim Nguyên Bảo, không thể Chuyển Sinh Lần 6 !!!\n"ESC"Rời khỏi.");
				return;
				}
			...
			USER_D->user_channel(sprintf(HIG "Chúc mừng "HIC "%s" HIG " đã Chuyển Sinh lần thứ "HIC " %d  "HIG " . ", who->get_name(),who->get_save_2("chuyensinh.score") ));
			who->set_level(who->get_level()-115);
			who->count_gift();
			who->add_gift_point(250);
			return 0;
			}
			if(who->get_save_2("chuyensinh.score")==6)
			{	
				if ( who->get_level() < 120 )
				{
				send_user(who,"%c%s",':',me->get_name()+":\n    Cấp độ chưa đạt tới cấp 120, không thể Chuyển Sinh Lần 7 !!!\n"ESC"Rời khỏi.");
				return;
				}
				else if ( "sys/sys/test_db"->get_yuanbao(who) < 28000 )
				{
				send_user(who,"%c%s",':',me->get_name()+":\n    Không đủ 2800 Kim Nguyên Bảo, không thể Chuyển Sinh Lần 7 !!!\n"ESC"Rời khỏi.");
				return;
				}
			"sys/sys/test_db"->add_yuanbao(who,-28000);
			who->add_save_2("chuyensinh.score",1);	
			send_user(who,"%c%s",';',HIY"Bạn đã Chuyển Sinh lần 7 !!!");
			USER_D->user_channel(sprintf(HIG "Chúc mừng "HIC "%s" HIG " đã Chuyển Sinh lần thứ "HIC " %d  "HIG " . ", who->get_name(),who->get_save_2("chuyensinh.score") ));
			who->set_level(who->get_level()-110);
			who->count_gift();
			who->add_gift_point(250);
			...
			if(who->get_save_2("chuyensinh.score")==7)
			{	
				if ( who->get_level() < 115 )
				{
				send_user(who,"%c%s",':',me->get_name()+":\n    Cấp độ chưa đạt tới cấp 115, không thể Chuyển Sinh Lần 8 !!!\n"ESC"Rời khỏi.");
				return;
				}
				else if ( "sys/sys/test_db"->get_yuanbao(who) < 32000 )
				{
				send_user(who,"%c%s",':',me->get_name()+":\n    Không đủ 3200 Kim Nguyên Bảo, không thể Chuyển Sinh Lần 8 !!!\n"ESC"Rời khỏi.");
				return;
				}
			...
			USER_D->user_channel(sprintf(HIG "Chúc mừng "HIC "%s" HIG " đã Chuyển Sinh lần thứ "HIC " %d  "HIG " . ", who->get_name(),who->get_save_2("chuyensinh.score") ));
			who->set_level(who->get_level()-105);
			who->count_gift();
			who->add_gift_point(250);
			return 0;
			}
			if(who->get_save_2("chuyensinh.score")==8)
			{	
				if ( who->get_level() < 110 )
				{
				send_user(who,"%c%s",':',me->get_name()+":\n    Cấp độ chưa đạt tới cấp 110, không thể Chuyển Sinh Lần 9 !!!\n"ESC"Rời khỏi.");
				return;
				}
				else if ( "sys/sys/test_db"->get_yuanbao(who) < 36000 )
				{
				send_user(who,"%c%s",':',me->get_name()+":\n    Không đủ 3600 Kim Nguyên Bảo, không thể Chuyển Sinh Lần 9 !!!\n"ESC"Rời khỏi.");
				return;
				}
			"sys/sys/test_db"->add_yuanbao(who,-36000);
			who->add_save_2("chuyensinh.score",1);	
			send_user(who,"%c%s",';',HIY"Bạn đã Chuyển Sinh lần 9 !!!");
			USER_D->user_channel(sprintf(HIG "Chúc mừng "HIC "%s" HIG " đã Chuyển Sinh lần thứ "HIC " %d  "HIG " . ", who->get_name(),who->get_save_2("chuyensinh.score") ));
			who->set_level(who->get_level()-100);
			who->count_gift();
			who->add_gift_point(250);
			return 0;
			}
			if(who->get_save_2("chuyensinh.score")==9)
			{	
				if ( who->get_level() < 105 )
				{
				send_user(who,"%c%s",':',me->get_name()+":\n    Cấp độ chưa đạt tới cấp 105, không thể Chuyển Sinh Lần 10 !!!\n"ESC"Rời khỏi.");
				return;
				}
				else if ( "sys/sys/test_db"->get_yuanbao(who) < 40000 )
				{
				send_user(who,"%c%s",':',me->get_name()+":\n    Không đủ 4000 Kim Nguyên Bảo, không thể Chuyển Sinh Lần 10 !!!\n"ESC"Rời khỏi.");
				return;
				}
			"sys/sys/test_db"->add_yuanbao(who,-40000);
			who->add_save_2("chuyensinh.score",1);	
			...
			who->set_level(who->get_level()-95);
			who->count_gift();
			who->add_gift_point(250);
		...
		case 7:
			...
			who->count_gift();
			who->add_cash(500);
			who->set_fam_contribute(who->get_fam_contribute()+200);
			return 0;
			}
			if(who->get_save_2("chuyensinh.score")==2)
			{	
		...
		else if(who->get_save_2("chuyensinh.score")<10)
		{
		who->add_title("Y015");
		who->show_title("Y015");
		}
		else if(who->get_save_2("chuyensinh.score")<20)
		{
		who->add_title("Y025");
		who->show_title("Y025");
		}
//		USER_D->user_channel(sprintf(HIG "Chúc Mừng "HIC "%s (%d)" HIG " đã thông qua Chuyển Sinh nhận được danh hiệu "HIB " Thiên Hạ Vô Song  "HIG " . ", who->get_name(), who->get_number() ));
	...
	/*	case 55:
		if(who->get_save_2("chuyensinh.score")<10)
		{
			send_user(who,"%c%s",':',sprintf( get_name() + ":\n    Không đủ 10 điểm Chuyển Sinh !!!\n Số điểm Chuyển Sinh của ngươi hiện chỉ là :" HIR " %d điểm",who->get_save_2("chuyensinh.score")));
			return 0;
		}
