...
void do_look(object who)
{
	...
	// 	ESC "Dùng Hắc 3 Cầu đổi thưởng\ntalk %x# gift.28\n"
	// 	ESC "Dùng Lam 3 Cầu đổi thưởng\ntalk %x# gift.29\n"
	// 	ESC "Dùng Hồng Cầu đổi thưởng\ntalk %x# gift.30\n"
	// 	ESC HIY"Dùng Hoa Cầu đổi thưởng\ntalk %x# gift.31\n"
	// 	ESC HIY"Dùng Hắc + Lam + Hồng + Hoa Cầu đổi thưởng\ntalk %x# gift.32\n"
	// 	ESC HIG"Ta muốn đổi Cờ Tướng lấy phần thưởng....\ntalk %x# gift.40\n"
	// 	ESC HIC"Ta muốn lấy phần thưởng Mốc nạp....\nmocnap @\n"
	// 	ESC "Phần thưởng Mốc nạp....\nmocnap\n"
...
void do_gift2(object me, string arg)
{
	...
	//tell_user(who, "Day %d - Hours %d", iDay, iGio );

	if (iDay != 5 && iDay != 6 && iDay != 0 && flag != 40) {

		send_user(who, "%c%c%w%s", ':', 3, get_char_picid(), sprintf(" %s :\n Thời gian hoạt động diễn ra vào thứ 6, thứ 7 và chủ nhật hàng tuần, hãy quay lại sau nhé !!! \n"
	...
	case 40:
		...
		if (rate < 5)
		{
			item = new("/item/common/AXPBlock");
			if (!item)
				return 0;
		}
		else if (rate < 10)
		{
			item = new("/item/common/thienlyhuutinh");
			if (!item)
				return 0;
		}
		/*else if (rate < 15)
		{
			item = new("/item/common/chenbac");
			if (!item)
				return 0;
		}*/
		else if (rate < 20)
		{
			item = new("/item/common/manhngoc");
			if (!item)
				return 0;
		}
		else if (rate < 25)
		{
			item = new("/item/common/nhansam");
			if (!item)
				return 0;
		}
		else if (rate < 45)
		...
		{
			item = new("/item/common/hoathach");
			if (!item)
				return 0;
		}
		else if (rate < 75)
		{
			item = new("/item/common/tuiphapbao");
			if (!item)
				return 0;
		}
		else if (rate < 79)
		...
		if (item)
		{
			...
			log_file("CoTuongDoiThuong.txt", sprintf("%s %s (%d) %s dùng %d cờ tướng đổi nhận %s \n", short_time(), who->get_name(), who->get_number(), who->get_id(), 30, item->get_name()));
