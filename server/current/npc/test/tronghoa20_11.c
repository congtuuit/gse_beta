...
int into_effect( object who , string arg)
{
	...
	send_user(who, "%c%c%c", 0x5a, 1, 1);
		...
		{
				item = new("/item/event/hoahongdacbiet20_11");
				p = item->move2(who,-1);
				item->add_to_user(p);
		}		
		...
		{	
				item = new("/item/event/hoahong20_11");
				p = item->move2(who,-1);
				item->add_to_user(p);
		}		
				send_user(who, "%c%s", ';', sprintf("Bạn đã trồng được %s !", item->get_name()));
				...
				{
				who->add_cash(1000);
				send_user(who, "%c%s", ';', "Bạn trồng Hoa Hồng thất bại !");
				}
	return 1;
}
