...
void do_look( object who ) 
{
	int id = getoid( this_object() );
		send_user(who,"%c%c%w%s",':',3,get_char_picid(),sprintf(" %s :\n Năm mới đã đến, ta-Sứ giả năm mới, xin chúc toàn thể bằng hữu Chiến Quốc 1 năm mới tràn ngập niềm vui, sức khoẻ dồi dào, vạn sự như ý, thành công trong cuộc sống... \n"
...
void do_gift2( object me, string arg )
{
	...
	iGio = mxTime[TIME_HOUR];
		...
		case 11:
			...
			if ( who->get_save("phongbaolixi")>0 )
			{
				if ( who->get_save("phongbaolixi")>=3600 )
			send_user(who,"%c%c%w%s",':',3,get_char_picid(),sprintf("Sứ Giả Năm Mới :\nMuốn nhận Túi Quà May Mắn phải chờ thêm %d giờ %d phút nữa !\n"ESC"Xác nhận \nCLOSE\n"ESC"Rời khỏi",who->get_save("phongbaolixi")/3600,(who->get_save("phongbaolixi")-3600)/60));
				else if ( who->get_save("phongbaolixi")>=60 )
			send_user(who,"%c%c%w%s",':',3,get_char_picid(),sprintf("Sứ Giả Năm Mới :\nMuốn nhận Túi Quà May Mắn phải chờ thêm %d phút nữa !\n"ESC"Xác nhận \nCLOSE\n"ESC"Rời khỏi",who->get_save("phongbaolixi")/60));
				else if ( who->get_save("phongbaolixi")<60 )
			send_user(who,"%c%c%w%s",':',3,get_char_picid(),sprintf("Sứ Giả Năm Mới :\nMuốn nhận Túi Quà May Mắn phải chờ thêm %d giây nữa !\n"ESC"Xác nhận \nCLOSE\n"ESC"Rời khỏi",who->get_save("phongbaolixi")));	
			return;
			}
			...
			who->set_save("phongbaolixi2",1);
			who->set_save("phongbaolixi",5400);
