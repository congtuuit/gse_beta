...
void create()
{
	set_name("Chủ tiệm khóa");
	set_real_name("Ngưu Nhị");
	...
	set_2("good", ([
		"01":"/item/common/box/ruongdong/key",
		"02":"/item/common/box/trangbi/key",
		//"03":"/item/common/box/tukim/key",
	]));
...
void do_welcome2(object npc, string arg)
{
	...
	case 1:
		...
		if (player->get("diem_danh.today") - iTime > 0) {
			send_user(player, "%c%c%w%s", ':', 3, get_char_picid(), sprintf(" %s :\n Bạn đã đăng ký điểm danh, sau %d giây nữa quay lại nhận thưởng nhé!!\n"
				ESC"Thoát", npc->get_name(), player->get("diem_danh.today") - iTime));
			return;
		}

		if (player->get("diem_danh.today") > 0 && time() - player->get("diem_danh.today") >= 0) {
		...
		player->set("diem_danh.today", time() + 3600);
