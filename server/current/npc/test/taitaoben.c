...
int accept_object_callout(object me, object who, object item)
{
	...
	if (item->is_equip())
	{
		...
		if (item->is_thanbinh() == 1 ||( i != WEAPON_TYPE && i != ARMOR_TYPE && i != HEAD_TYPE && i != BOOTS_TYPE && i != WAIST_TYPE && i != NECK_TYPE)) {
	...
	if (mau == 1) {
		...
		if (who->get_save("giamtientaitao") < 1) {
			if (who->get_cash() < (cap * 270000) + (cap1 * 360000) + (cap2 * 8000) + (cap3 * 6000)) {
				send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("Lương Phan Tiên:\nKhông đủ %d lượng để Tái tạo chỉ số của %s !!!\n"ESC"Xác nhận", (cap * 270000) + (cap1 * 360000) + (cap2 * 8000) + (cap3 * 6000), item->get_name()));
				return -99;
			}
			else {
				send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("Lương Phan Tiên:\nTái tạo chỉ số của %s cần tiêu hao %d lượng, bạn muốn tái tạo không ?\n"ESC"Xác nhận\ntalk %x# giaotrangbi.%x#\n"ESC"Huỷ bỏ", item->get_name(), (cap * 270000) + (cap1 * 360000) + (cap2 * 8000) + (cap3 * 6000), getoid(me), getoid(item)));
			}
		}
		else {
			if (who->get_cash() < (cap * 125000) + (cap1 * 180000) + (cap2 * 4000) + (cap3 * 3000)) {
				send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("Lương Phan Tiên:\nKhông đủ %d lượng để Tái tạo chỉ số của %s !!!\n"ESC"Xác nhận", (cap * 125000) + (cap1 * 180000) + (cap2 * 4000) + (cap3 * 3000), item->get_name()));
				return -99;
			}
			else {
				send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("Lương Phan Tiên:\nTái tạo chỉ số của %s cần tiêu hao %d lượng, bạn muốn tái tạo không ?\n"ESC"Xác nhận\ntalk %x# giaotrangbi.%x#\n"ESC"Huỷ bỏ", item->get_name(), (cap * 125000) + (cap1 * 180000) + (cap2 * 4000) + (cap3 * 3000), getoid(me), getoid(item)));
			}
		}
	}
	...
	if (mau == 1) {
		...
		if (who->get_save("giamtientaitao") < 1) {
			...
			if (random(100) < 51) {
				...
				send_user(who, "%c%s", ';', sprintf("Bạn đã dùng %d lượng để tái tạo chỉ số của %s !", (cap * 270000) + (cap1 * 360000) + (cap2 * 8000) + (cap3 * 6000), item->get_name()));


				who->add_cash(-((cap * 270000) + (cap1 * 360000) + (cap2 * 8000) + (cap3 * 6000)));
			}
			else {
				item->set("make", 0);
				item->delete("app");
			...
			if (random(100) < 51) {
				...
				send_user(who, "%c%s", ';', sprintf("Bạn đã dùng %d lượng để tái tạo chỉ số của %s !", (cap * 125000) + (cap1 * 180000) + (cap2 * 4000) + (cap3 * 3000), item->get_name()));


				who->add_cash(-((cap * 125000) + (cap1 * 180000) + (cap2 * 4000) + (cap3 * 3000)));
				who->add_save("giamtientaitao", -1);
			}
			else {
				item->set("make", 0);
