...
void win_bonus2( object me, object who )
{
	...
	object orgnpc, map, item, leader, item2, item3;
	...
	if (rand<4100*rate/100)
	{
		...
		item = new (file);
		if (rand<1000) "/sys/item/equip"->init_equip_prop_3(item);
		else "/sys/item/equip"->init_equip_prop_1(item);
	}
	else
