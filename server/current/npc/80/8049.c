...
void do_look( object who )
{
	...
	if ( (who->get_legend(TASK_31,23) && !who->get_legend(TASK_31,24)) || (who->get_legend(TASK_31,24) && !who->get_legend(TASK_31,26)) )
		tmp += sprintf(ESC HIY "Hồi chuông\ntalk %x# welcome.8\n",getoid(me));
...
void do_welcome2( object me, string arg )
{
        ...
        case 2:
			...
			who->add_cash(2500);
			send_user(who,"%c%s",';',"带话给程风（2） 经验 3500 tiềm năng 90 Ngân lượng 2500 小米粥图纸 1");
	...
	case 3:
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,5,"Những rắc rối" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Những rắc rối");
        	}
        	break;       
        case 4:
			...
			who->add_cash(4200);
			send_user(who,"%c%s",';',"Những rắc rối 经验 4800 tiềm năng 170 Ngân lượng 4200");
	...
	case 6:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_YANGUO,"Yên Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,34,"Người giàu có" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Người giàu có");
        	}
        	break;       
        case 7:
			...
			who->add_cash(5300);
			send_user(who,"%c%s",';',"Người giàu có 经验 6300 tiềm năng 252 Ngân lượng 5300");
	...
	case 8:
			...
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    这个秘密我们已经保守了很久，现在纸终于包不住火了。\n    五年前，我们去长白山，见到有人开坛作法，偷听到他把所有的Quái vật都放了出来了，并且这个世界上只有Hồi chuông才可以制服他们，他将Hồi chuông放在困魔阵，为了不让人拿到后对付它们，那些妖怪死死的守着这个Hồi chuông。我们为了不惹祸上身，就保守了这个秘密。\n    最近老是听到有妖怪出没伤人的消息，我们作为唯一知情人，当然不能坐视不理，所以请你去困魔阵1层消灭骷髅妖王，然后回来我这里吧。\n"ESC"接受任务\ntalk %x# welcome.12\n"ESC"Rời khỏi.", me->get_name(),getoid(me)));
			...
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    能真正解决这个问题，可能只有拿到Hồi chuông了。\n"ESC"Hoàn thành任务\ntalk %x# welcome.13\n"ESC"Rời khỏi.", me->get_name(),getoid(me)));
        	else if ( who->get_legend(TASK_31,23) && !who->get_legend(TASK_31,24) )
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    现在就是重新封印这些妖怪的关键了，困魔阵中的Hồi chuông既是这些妖怪作祟的力量之源，也是克制他们的利器，Hồi chuông在阵中的时候就能增幅他们的力量，抑制他们身上的封印，而到阵外，就成为重新抑制他们力量，加重封印的法器，你现在去魔神殿拿来Hồi chuông，刚才看守铃铛的怪物已经被你杀掉了。\n"ESC"接受任务\ntalk %x# welcome.14\n"ESC"Rời khỏi.", me->get_name(),getoid(me)));
        	else if ( who->get_legend(TASK_31,24) && !who->get_legend(TASK_31,25) )
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    如果没有Hồi chuông，我还真不知这件事该怎么解决，你找来了吗？\n"ESC"Rời khỏi.", me->get_name()));
        	else if ( who->get_legend(TASK_31,25) && !who->get_legend(TASK_31,26) )
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    拿到了这个Hồi chuông，我再和程林开商量一下，这件事情谢谢你了！\n"ESC"Hoàn thành任务\ntalk %x# welcome.15\n"ESC"Rời khỏi.", me->get_name(),getoid(me)));
        ...
        case 9:
			...
			who->add_cash(29000);
			send_user(who,"%c%s",';',"秘密（1） 经验 30000 tiềm năng 600 Ngân lượng 29000");
        ...
        case 10:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"秘密（2）");
        	}
        	break;  
        case 11:
			...
			who->add_cash(32000);
			send_user(who,"%c%s",';',"秘密（2） 经验 35000 tiềm năng 620 Ngân lượng 32000");
        ...
        case 12:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"秘密（3）");
        	}
        	break;  
        case 13:
			...
			who->add_cash(37000);
			send_user(who,"%c%s",';',"秘密（3） 经验 40000 tiềm năng 650 Ngân lượng 37000 玄云靴图纸 1");
        ...
        case 14:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"秘密（4）");
        	}
        	break;  
        case 15:
        	if ( who->get_legend(TASK_31,24) && !who->get_legend(TASK_31,26) )
        	{
        		...
        		if ( !stringp(tmp) )
        			return ;
        		if (TASK_LEGEND_D->check_task_item(who,"Hồi chuông",1) != 1 )
        			return;
			...
			who->add_cash(140000);
			send_user(who,"%c%s",';',sprintf("秘密（4） 经验 150000 tiềm năng 1800 Ngân lượng 140000 %s 1",tmp->get_name()));
