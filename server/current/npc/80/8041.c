...
void do_welcome2( object me, string arg )
{
        ...
        case 2:
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_CHUGUO,1,"取回货物" );	
        		...
        		send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"取回货物");
        	}
        	break;
        case 3:
			...
			send_user(who,"%c%s",';',"取回货物 经验 300 tiềm năng 30 Ngân lượng 5000 行军散 3");
        	}
        	break;
        case 4:
//        	if( who->get_level() >= 10 && who->get_legend(TASK_37,13) && !who->get_legend(TASK_37,14) )
		if( who->get_legend(TASK_49, 11) && !who->get_legend(TASK_49, 12) && who->get_fam_name() != "")
        		send_user(who,"%c%s",':',sprintf("%s:\n    我们之间的问题是该解决了，谢谢你送来这封Nhật ký thử thách，我接下了！\n"ESC"Hoàn thành任务\ntalk %x# welcome.5\n"ESC"Rời khỏi.",me->get_name(),getoid(me)));
        	break;  
        case 5:
//        	if ( who->get_level() >= 10 && who->get_legend(TASK_37,13) && !who->get_legend(TASK_37,14) )
		if( who->get_legend(TASK_49, 11) && !who->get_legend(TASK_49, 12) && who->get_fam_name() != "")
		{
        		if (TASK_LEGEND_D->check_task_item(who,"Nhật ký thử thách",1) != 1 )
        			return;
			...
			who->add_cash(1000);
			send_user(who,"%c%s",';',"门派向导的恩怨 经验 300 tiềm năng 35 Ngân lượng 1000 Trái quýt 3");
