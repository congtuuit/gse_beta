...
void do_look(object who)
{
	...
	if (who->get_quest("escort.biaowang") && arrayp(npc_info) &&
		...
		tmp += sprintf(ESC HIG "Cuộc chiến của hộ tống Vương\ntalk %x# welcome.21\n", getoid(me));
	...
	if (who->get_legend(TASK_14, 13) && !who->get_legend(TASK_14, 14))
		tmp += sprintf(ESC HIY "Tên\ntalk %x# welcome.1\n", getoid(me));
	if (who->get_legend(TASK_43, 7) && !who->get_legend(TASK_43, 8))
		tmp += sprintf(ESC HIY "Phỏng vấn nội gián\ntalk %x# welcome.3\n", getoid(me));
	if (who->get_legend(TASK_43, 8) && !who->get_legend(TASK_43, 11))
		tmp += sprintf(ESC HIY "Xóa bỏ mối đe dọa\ntalk %x# welcome.3\n", getoid(me));
	if (who->get_legend(TASK_43, 11) && !who->get_legend(TASK_43, 12))
		tmp += sprintf(ESC HIY "Danh sách tìm kiếm\ntalk %x# welcome.3\n", getoid(me));

	send_user(who, "%c%s", ':', sprintf("%s：\n    Tần Quốc Người bảo vệ cũ của Hộ tống Kyoto，Bản chất xuất chúng, thực sự là một anh hùng！\n", get_name(), )
		+ tmp +
		ESC"Rời khỏi."
	);
...
void do_welcome2(object me, string arg)
{
	...
	case 1:
		if (who->get_legend(TASK_14, 13) && !who->get_legend(TASK_14, 14))
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    Khí cụ này có thể gọi là vũ cầu, ba vòng gọi là vòng kinh mạch, vòng xích đạo, vòng thăng thiên phải. Cho nên, ta sẽ phái người đến thảo luận với Yên Quốc, ngươi có thể rời đi trước。\n"ESC"Hoàn thành nhiệm vụ\ntalk %x# welcome.2\n"ESC"Rời khỏi.", me->get_name(), getoid(me)));
	...
	case 2:
		if (who->get_legend(TASK_14, 13) && !who->get_legend(TASK_14, 14))
		{
			...
			who->add_cash(7000);
			send_user(who, "%c%s", ';', "浑仪（3） kinh nghiệm 8000 tiềm năng 480 tiền 7000 Hình kim ong ngọc 1");
	...
	case 4:
		if (who->get_legend(TASK_43, 7) && !who->get_legend(TASK_43, 8))
		{
			if (TASK_LEGEND_D->check_task_item(who, "Thư thăm", 1) != 1)
				return;
			TASK_LEGEND_D->task_finish(who);
			...
			who->add_cash(92000);
			send_user(who, "%c%s", ';', "Kinh nghiệm 96000 tiềm năng 1900 金钱 92000");
	...
	case 6:
		if (who->get_legend(TASK_43, 10) && !who->get_legend(TASK_43, 11))
		{
			...
			who->add_cash(91000);
			send_user(who, "%c%s", ';', "解除威胁 经验 99000 tiềm năng 1800 金钱 91000");
