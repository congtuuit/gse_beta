...
void do_look( object who )
{
	...
	if ( ( who->get_level() >= 40 && !who->get_legend(TASK_10,14) ) || ( who->get_legend(TASK_10,14) && !who->get_legend(TASK_10,16) ) )
		tmp += sprintf(ESC HIY "Kế hoạch săn bắn\ntalk %x# welcome.1\n",getoid(me));
...
void do_welcome2( object me, string arg )
{
        ...
        case 2:
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QINGUO,16,"Kế hoạch săn bắn" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Kế hoạch săn bắn");
        	}
        	break;
       case 3:
			...
			who->add_cash(3200);
			send_user(who,"%c%s",';',"Kế hoạch săn bắn 经验 4500 tiềm năng 130 Ngân lượng 3200");
