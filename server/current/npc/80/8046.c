...
void do_welcome2( object me, string arg )
{
        ...
        case 2:
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_CHUGUO,3,"消灭四脚蛇" );	
        		...
        		send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"消灭四脚蛇");
        	}
        	break;
        case 3:
...
//        		nTmp = WEAPON_FILE->get_family_equip("",0,10,WAIST_TYPE);
			...
			who->add_cash(3000);
			send_user(who,"%c%s",';',sprintf("消灭四脚蛇 经验 400 tiềm năng 40 Ngân lượng 3000 %s 1 地瓜解暑汤 2",item->get_name()));
	...
	case 4:
		...
		if ( who->get_legend(TASK_2_00,26) && !who->get_legend(TASK_2_00,27) ) 
		{
			...
			who->add_cash(1000);
			send_user(who,"%c%s",';',sprintf("给楚管家的口信 经验 600 tiềm năng 35 Ngân lượng 1000"));
