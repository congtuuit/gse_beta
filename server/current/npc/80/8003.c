...
void do_look( object who )
{
	...
	if ( ( who->get_level() >= 35 && !who->get_legend(TASK_10,3) ) || ( who->get_legend(TASK_10,3) && !who->get_legend(TASK_10,5) ) )
		tmp += sprintf(ESC HIY "Yêu cầu rau quả\ntalk %x# welcome.1\n",getoid(me));
...
void do_welcome2( object me, string arg )
{
        ...
        case 2:
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,9,"Yêu cầu rau quả（1）" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Yêu cầu rau quả（1）");
        	}
        	break;
       case 3:
			...
			who->add_cash(2000);
			send_user(who,"%c%s",';',"Yêu cầu rau quả（1） 经验 3000 tiềm năng 70 Ngân lượng 2000");
        ...
        case 5:
			...
			who->add_cash(2000);
			send_user(who,"%c%s",';',"种庄稼的锄头 经验 2500 tiềm năng 65 Ngân lượng 2000");
