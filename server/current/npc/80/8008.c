...
void do_look( object who )
{
	...
	if ( (who->get_level() >=100 && !who->get_legend(TASK_36,23)) || (who->get_legend(TASK_36,23) && !who->get_legend(TASK_36,25)) )
		tmp += sprintf(ESC HIY "Kẻ phản bội\ntalk %x# welcome.3\n",getoid(me));
...
void do_welcome2( object me, string arg )
{
	...
	case 1:
        ...
        case 2:
			...
			who->add_cash(48000);
			send_user(who,"%c%s",';',"手巧的工匠 经验 50000 tiềm năng 615 Ngân lượng 48000");
        ...
        case 3:
			...
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    Kẻ phản bội死不足惜，我倒要看看，Tần Quốc还有什么招术！\n"ESC"Hoàn thành任务\ntalk %x# welcome.5\n"ESC"Rời khỏi.", me->get_name(),getoid(me)));
        	break;
        case 4:
        	if ( who->get_level() >= 100 && !who->get_legend(TASK_36,23)  )
        	{
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,57,"Kẻ phản bội" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Kẻ phản bội");
        	}
        	break;
	case 5:
			...
			who->add_cash(70000);
			send_user(who,"%c%s",';',"Kẻ phản bội 经验 80000 tiềm năng 955 Ngân lượng 70000");
	...
	case 6:
        ...
        case 7:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_QIGUO,"Tề Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,64,"Tư vấn nhiệt tình（1）" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Tư vấn nhiệt tình（1）");
        	}
        	break;         	
	case 8:
			...
			who->add_cash(50300);
			send_user(who,"%c%s",';',"Tư vấn nhiệt tình（1） 经验 51000 tiềm năng 778 Ngân lượng 50300");
        ...
        case 9:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_QIGUO,"Tề Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,65,"Tư vấn nhiệt tình（2）" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Tư vấn nhiệt tình（2）");
        	}
        	break; 
	case 10:
        	if ( who->get_legend(TASK_44,22) && !who->get_legend(TASK_44,23) )
			...
			who->add_potential(778);
			who->add_cash(50300);
			send_user(who,"%c%s",';',sprintf("Tư vấn nhiệt tình（4） 经验 230000 tiềm năng 2400 Ngân lượng 220000 %s 1",tmp->get_name()));
