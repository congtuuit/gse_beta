...
void do_look( object who )
{
	...
	if ( (who->get_level() >=55 && !who->get_legend(TASK_17,10)) || (who->get_legend(TASK_17,10) && !who->get_legend(TASK_17,12)) )
		tmp += sprintf(ESC HIY "Khói trên đèn hiệu\ntalk %x# welcome.1\n",getoid(me));
...
void do_welcome2( object me, string arg )
{
	...
	case 2:
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,23,"Khói trên đèn hiệu" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Khói trên đèn hiệu");
        	}
        	break;
        case 3:
			...
			who->add_cash(6600);
			send_user(who,"%c%s",';',"Khói trên đèn hiệu 经验 6500 tiềm năng 240 Ngân lượng 6600 牛肉炒鸡蛋图纸 1");
        ...
        case 5:
			...
			who->add_cash(17000);
			send_user(who,"%c%s",';',"Tình yêu（2） 经验 17000 tiềm năng 260 Ngân lượng 17000");
	...
	case 6:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_ZHAOGUO,"Triệu Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,40,"Tình yêu（3）" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Tình yêu（3）");
        	}
        	break;  
        }
}