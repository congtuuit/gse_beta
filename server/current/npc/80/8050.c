...
void do_welcome2( object me, string arg )
{
        ...
        case 2:
			...
			who->add_cash(22000);
			send_user(who,"%c%s",';',"同心锁（1） 经验 24000 tiềm năng 420 Ngân lượng 22000");
	...
	case 3:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"同心锁（2）");
        	}
        	break;   
        case 4:
			...
			who->add_cash(25000);
			send_user(who,"%c%s",';',"同心锁（2） 经验 26000 tiềm năng 450 Ngân lượng 25000");
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,38,"Yên Vương rất thích vàng（3）" );
        ...
        case 6:
			...
			who->add_cash(50000);
			send_user(who,"%c%s",';',"论语 经验 58000 tiềm năng 850 Ngân lượng 50000");
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,56,"" );
        	}
        	break;
        case 7:
        	if ( who->get_level() >= 40 && who->get_legend(TASK_38,1) && !who->get_legend(TASK_38,2) )
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    我还以为他只是不想卖给我，没想到是这个原因，快把Quả nho拿来给我尝尝。\n"ESC"Hoàn thành任务\ntalk %x# welcome.8\n"ESC"Rời khỏi.", me->get_name(),getoid(me)));
        		
        	break;
        case 8:
        	if ( who->get_level() >= 40 && who->get_legend(TASK_38,1) && !who->get_legend(TASK_38,2) )
		{
        		if (TASK_LEGEND_D->check_task_item(who,"紫Quả nho",1) != 1 )
        			return;
        		TASK_LEGEND_D->task_finish(who);
			...
			who->add_cash(4000);
			send_user(who,"%c%s",';',"紫Quả nho 经验 4000 tiềm năng 130 Ngân lượng 4000");
