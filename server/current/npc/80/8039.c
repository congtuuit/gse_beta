...
void do_welcome2( object me, string arg )
{
        ...
        case 2:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"贺礼");
       		}
        	break;
        case 3:
        	if ( who->get_legend(TASK_20,19) && !who->get_legend(TASK_20,20) )
        ...
        case 4:
			...
			who->add_cash(9000);
			send_user(who,"%c%s",';',"探听虚实 经验 11000 tiềm năng 320 Ngân lượng 9000");
	...
	case 5:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_QINGUO,"Tần Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QINGUO,24,"Bản vẽ cung điện" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Bản vẽ cung điện");
        	}
        	break;        
	case 6:
		if ( who->get_level() >=40 && who->get_legend(TASK_37,29) && !who->get_legend(TASK_37,30) )
	...
	case 7:
		if ( who->get_level() >=40 && who->get_legend(TASK_37,29) && !who->get_legend(TASK_37,30) )
			...
			who->add_cash(2500);
			send_user(who,"%c%s",';',"交换心得 经验 3000 tiềm năng 70 Ngân lượng 2500");
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,73,"" );
        	}
        	break;
	case 8:
		if ( who->get_level() >=115 && who->get_legend(TASK_39,9) && !who->get_legend(TASK_39,10) )
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    没想到还有人如此的崇拜我，我帮他Hoàn thành这个心愿吧！\n"ESC"Hoàn thành任务\ntalk %x# welcome.9\n"ESC"Rời khỏi.", me->get_name(),getoid(me)));
	...
	case 9:
		if ( who->get_level() >=115 && who->get_legend(TASK_39,9) && !who->get_legend(TASK_39,10) )
			...
			who->add_cash(48000);
			send_user(who,"%c%s",';',"心愿 经验 52000 tiềm năng 620 Ngân lượng 48000");
        ...
        case 11:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"刀魂传说1");
       		}
        	break;
        case 12:
        	if( who->get_legend(TASK_46,11)&&!who->get_legend(TASK_46,12) )
			...
			who->add_cash(93000);
			send_user(who,"%c%s",';',"刀魂传说5 经验 92000 tiềm năng 1100 Ngân lượng 93000 祁龙鞋图纸 1");
...
//			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    这可真是件好事。\n"ESC"Hoàn thành任务\ntalk %x# welcome.12\n"ESC"Rời khỏi.", me->get_name(),getoid(me)));
        ...
        case 14:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"炼药之路1");
       		}
        	break;
        }
}