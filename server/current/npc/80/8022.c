...
void do_look( object who ) 
{
	...
	if ( (who->get_level() >=60 && !who->get_legend(TASK_18,1)) || (who->get_legend(TASK_18,1) && !who->get_legend(TASK_18,3)) )
		tmp += sprintf(ESC HIY "Sự đau khổ của quản gia\ntalk %x# welcome.1\n",getoid(me));
	...
	if ( (who->get_level() >=60 && !who->get_legend(TASK_18,13)) || (who->get_legend(TASK_18,13) && !who->get_legend(TASK_18,15)) )
		tmp += sprintf(ESC HIY "Pháo hoa trên Vạn Lý Trường Thành\ntalk %x# welcome.7\n",getoid(me));
...
void do_welcome2( object me, string arg )
{
	...
	case 2:
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,30,"Sự đau khổ của quản gia" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Sự đau khổ của quản gia");
        	}
        	break;
        case 3:
			...
			who->add_cash(6600);
			send_user(who,"%c%s",';',"Sự đau khổ của quản gia 经验 6900 tiềm năng 260 Ngân lượng 6600 银钥匙图纸 1");
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,30,"" );
        	}
        	break;   
        case 4:
        	if ( who->get_level() >= 60 && !who->get_legend(TASK_18,4) )
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    全QuốcTruy nã——叶青，你应该知道吧？最近他潜逃到Yên Quốc附近了，得赶紧把他捉拿归案，不然伤及更多的无辜百姓就不好了。\n    听说朝廷悬赏十万两黄金来捉拿他，如果你能将他干掉，那你可就发达了，趁这个消息还没走漏，赶紧去燕长城除掉他吧！\n"ESC"接受任务\ntalk %x# welcome.5\n"ESC"Rời khỏi.", me->get_name(),getoid(me)));
	...
	case 5:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"叛Quốc之徒");
        	}
        	break;
        case 6:
			...
			who->add_cash(6200);
			send_user(who,"%c%s",';',"叛Quốc之徒 经验 7100 tiềm năng 270 Ngân lượng 6200 高爆炸弹 5");
	...
	case 8:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_ZHAOGUO,"Triệu Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,34,"Pháo hoa trên Vạn Lý Trường Thành" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Pháo hoa trên Vạn Lý Trường Thành");
        	}
        	break;
        case 9:
			...
			who->add_cash(6800);
			send_user(who,"%c%s",';',"Pháo hoa trên Vạn Lý Trường Thành 经验 6800 tiềm năng 250 Ngân lượng 6800 气脉丹图纸 1");
	...
	case 11:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"论语");
        	}
        	break;
        }
}