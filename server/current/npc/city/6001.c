...
void do_look( object who )
{
	...
	if ( (who->get_level() >= 110 && !who->get_legend(TASK_45,15)) || (who->get_legend(TASK_45,15) && !who->get_legend(TASK_45,17)) )
		tmp += sprintf(ESC HIY "Được mọi người ngưỡng mộ\ntalk %x# welcome.9\n",getoid(me));
...
void do_welcome2( object me, string arg )
{
        ...
        case 2:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"交好");
        	}
        	break; 
	case 3:
		if ( me->get_city_name() != who->get_city_name() )
	...
	case 6:
        ...
        case 7:
		if ( who->get_legend(TASK_45,13) && !who->get_legend(TASK_45,14) )
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    啊，这就是常常有人提及的那种仙山trà Mist么？果然异香扑鼻，一闻就知道不同反响，感谢你为本王带来了仙茶，本王一定要重重赏你.\n"ESC"Hoàn thành nhiệm vụ\ntalk %x# welcome.8\n"ESC"Rời khỏi", me->get_name(),getoid(me)));
        	break; 
		
	case 8:
		if ( who->get_legend(TASK_45,13) && !who->get_legend(TASK_45,14) )
		{
        		if (TASK_LEGEND_D->check_task_item(who,"Trà mây",1) != 1 )
        			return;
			...
			who->add_cash(197000);
			send_user(who,"%c%s",';',"Vinh quang tối cao（3） 经验 200000 tiềm năng 1500 Ngân lượng 197000 飞龙鞋图纸 1");
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,71,"" );
        	}
        	break;
	case 9:
        	if (who->get_level() >= 110 && !who->get_legend(TASK_45,15) )
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    你有没有想过成为一个Được mọi người ngưỡng mộ的人，当然德行、操守、仁爱、忠诚都是必不可少的，但是我们这个世界还需要一样，那就是武力.我看你其他方面的修炼都在步入正轨，唯有武力一项，我还需要检验一下，怎么样愿意接受Thử thách么？前段时间城中居民莫名的头疼经过调查其实都是诛仙阵中的枪魂在捣鬼，如果你能够前去消灭万恶的枪魂12只，到时候我晓谕全城，居民们自然会敬仰你的.\n"ESC"Tiếp nhận nhiệm vụ\ntalk %x# welcome.10\n"ESC"Rời khỏi", me->get_name(),getoid(me)));
		else if ( who->get_legend(TASK_45,15) && !who->get_legend(TASK_45,16) )
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    Được mọi người ngưỡng mộ的人可是武力仁爱缺一不可，你光有仁爱不够，必须时时Rèn luyện bản thân啊！\n"ESC"Rời khỏi", me->get_name()));
		else if ( who->get_legend(TASK_45,16) && !who->get_legend(TASK_45,17) )
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    你果然有勇气有武力，我已经将你的事迹晓谕全城，你出去看吧，他们都在欢呼你呢.怎么样，Được mọi người ngưỡng mộ的感觉不错吧，哈哈，还望你以后能够时时为本王分忧，本王自然会给予你应该得到的.\n"ESC"Hoàn thành nhiệm vụ\ntalk %x# welcome.11\n"ESC"Rời khỏi", me->get_name(),getoid(me)));
        	break;
        case 10:
        	if (who->get_level() >= 110 && !who->get_legend(TASK_45,15) )
        	{
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_CHUGUO,"Sở Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_CHUGUO,41,"Được mọi người ngưỡng mộ" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Được mọi người ngưỡng mộ");
        	}
        	break;        
        case 11:
		if ( who->get_legend(TASK_45,16) && !who->get_legend(TASK_45,17) )
			...
			who->add_cash(95000);
			send_user(who,"%c%s",';',"Được mọi người ngưỡng mộ 经验 91500 tiềm năng 988 Ngân lượng 95000");
