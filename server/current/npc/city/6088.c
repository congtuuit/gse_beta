...
void do_look( object who ) 
{
	...
	{
		tmp += sprintf(ESC HIY "Lửa ma trong dung nham\ntalk %x# welcome.3\n",getoid(me));
	}
	if ( sizeof(tmp) )
...
void do_welcome2( object me, string arg )
{
        ...
        case 2:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"买椟还珠(1)");
        	}
        	break;
        case 3:
	...
	case 4:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_CHUGUO,"Sở Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_CHUGUO,21,"Lửa ma trong dung nham" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Lửa ma trong dung nham");
        	}
        	break;
        case 5:
			...
			who->add_cash(2000);
			send_user(who,"%c%s",';',"Lửa ma trong dung nham 经验 2500 tiềm năng 90 Ngân lượng 2000 硝石粉 5");
