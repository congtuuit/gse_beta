...
void do_look( object who )
{
	...
	{
		tmp += sprintf(ESC HIY "Khó xây dựng\ntalk %x# welcome.1\n",getoid(me));
	}
	...
	{
		tmp += sprintf(ESC HIY "Loại bỏ những kẻ bắt nạt\ntalk %x# welcome.1\n",getoid(me));
	}
	...
	if ( ( who->get_level() >=110 && !who->get_legend(TASK_43,23) ) || ( who->get_legend(TASK_43,23) && !who->get_legend(TASK_43,25) ) )
		tmp += sprintf(ESC HIY "Bóng tối trước bình minh\ntalk %x# welcome.11\n",getoid(me));
...
void do_welcome2( object me, string arg )
{
        ...
        case 1:
			...
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    大王既已下令开始修造，小民即日开始动工。只是修造所用石料被Trương Phú垄断，一味抬高物价，这本是大王之事，他违抗王命，理应伏法，你能前去秦长城烽火台将他除去吗？\n"ESC"接受任务\ntalk %x# welcome.6\n"ESC"Rời khỏi.", me->get_name(),getoid(me)));
	...
	case 2:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"打听消息");
        	}
        	break;
	case 3:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_QINGUO,"Tần Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QINGUO,29,"Khó xây dựng" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Khó xây dựng");
        	}
        	break;
        case 4:
			...
			who->add_cash(13000);
			send_user(who,"%c%s",';',"Khó xây dựng 经验 14000 tiềm năng 360 Ngân lượng 13000");
	...
	case 5:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"降旨修墓");
        	}
        	break; 
	case 6:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_QINGUO,"Tần Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QINGUO,31,"Loại bỏ những kẻ bắt nạt" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Loại bỏ những kẻ bắt nạt");
        	}
        	break;
        case 7:
			...
			who->add_cash(15000);
			send_user(who,"%c%s",';',"Loại bỏ những kẻ bắt nạt 经验 15500 tiềm năng 370 Ngân lượng 15000 日月剑图纸 1");
	...
	case 8:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"寻求支持");
        	}
        	break;
        case 9:
		if ( who->get_level() >= 105 && !who->get_legend(TASK_39,3) )
        ...
        case 10:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"嘱托之事");
        	}
        	break; 
	case 11:
        ...
        case 12:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_QINGUO,"Tần Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QINGUO,60,"Bóng tối trước bình minh" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Bóng tối trước bình minh");
        	}
        	break;
        case 13:
			...
			who->add_cash(95000);
			send_user(who,"%c%s",';',sprintf("Bóng tối trước bình minh 经验 98000 tiềm năng 1120 Ngân lượng 95000 %s 1",tmp->get_name()));
