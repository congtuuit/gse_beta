...
void do_welcome2( object me, string arg )
{
		...
		if ( who->get_legend(TASK_20,13)&&!who->get_legend(TASK_20,14) )
			...
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    我母亲都在九月九日这天，采下初开的Cây cúc和一点青翠的枝叶，掺和在准备酿酒的粮食中，然后一齐用来酿酒，放至第二年九月九日饮用。九九重阳，阳气旺盛才酿得出好酒。传说喝了这种酒，可以延年益寿。\n"ESC"Rời khỏi.", me->get_name()));
        	break;
        case 2:
        	if ( who->get_legend(TASK_20,13)&&!who->get_legend(TASK_20,14) )
        	{
			...
			who->add_cash(7500);
			send_user(who,"%c%s",';',"重阳节（2） 经验 7000 tiềm năng 260 Ngân lượng 7500");
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"重阳节（3）");
        	}
        	break;   
        case 99:
        	GUARDER_D->do_look( who, me ); 
