...
{ 
	...
	object me = this_object();
		...
		tmp += sprintf(ESC HIY "Đá ma thuật\ntalk %x# welcome.1\n",getoid(this_object())); 
...
void do_welcome2( object me, string arg )
{
        ...
        case 2:
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QINGUO,7,"Đá ma thuật" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Đá ma thuật");
        	}
        	break;
       case 3:
        	if ( who->get_legend(TASK_11,12) && !who->get_legend(TASK_11,13) )
        	{
        		if (TASK_LEGEND_D->check_task_item(who,"Đá ma thuật",10) != 1 )
        			return;
			...
			who->add_cash(4000);
			send_user(who,"%c%s",';',"Đá ma thuật 经验 5800 tiềm năng 186 Ngân lượng 4000");
        	...
        	if (who->get_level() >= 110 && !who->get_legend(TASK_43,17))
		{
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_QINGUO,"Tần Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QINGUO,59,"Sự giúp đỡ của bạn bè" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Sự giúp đỡ của bạn bè");
        	}
		break;
        case 99:
        	GUARDER_D->do_look_3( who, me ); 
