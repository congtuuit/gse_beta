...
void do_look( object who )
{
	...
	{
		tmp += sprintf(ESC HIY "Gởi quốc tịch\ntalk %x# welcome.1\n",getoid(me));
	}
	...
	{
		tmp += sprintf(ESC HIY "Măng tây\ntalk %x# welcome.23\n",getoid(me));
	}
	if ( who->get_legend(TASK_45,12) && !who->get_legend(TASK_45,13) )
	{
		tmp += sprintf(ESC HIY "Trà mây\ntalk %x# welcome.23\n",getoid(me));
	}
...
void do_welcome2( object me, string arg )
{
        ...
        case 1:
			...
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    虽然我们已经消灭了山戎族奸细，但是并没有解除他们给燕国带来的威胁，因为山戎人去了孤竹族请求Hỗ trợ，而孤竹族也派出了使节来商量此事，现在，Gởi quốc tịch已进入齐国境内，正在太公府内稍做歇息.请帮助我除掉Gởi quốc tịch这个心头大患！\n"ESC"Tiếp nhận nhiệm vụ\ntalk %x# welcome.5\n"ESC"Rời khỏi", me->get_name(),getoid(me)));
        	else if (who->get_legend(TASK_13,19) && !who->get_legend(TASK_13,20))
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    若让孤竹族的使节安全到达山戎族，那么他们就狼狈为奸、串通一气了，燕国人民又得处于水深火热之中了！所以消灭Gởi quốc tịch的事情你得尽快办好才是！\n"ESC"Rời khỏi", me->get_name()));
	...
	case 2:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"带信给管仲（3）");
        	}
        	break;
	case 3:
        	if (who->get_level() >= 35 && !who->get_legend(TASK_13,16) )
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"山戎族奸细（1）");
        	}
        	break;
        case 4:
			...
			who->add_cash(3500);
			send_user(who,"%c%s",';',"山戎族奸细（1） 经验 3800 tiềm năng 120 Ngân lượng 3500");
	...
	case 5:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_QIGUO,"Tề Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,7,"Gởi quốc tịch（2）" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Gởi quốc tịch（2）");
        	}
        	break;
        case 6:
			...
			who->add_cash(3500);
			send_user(who,"%c%s",';',"Gởi quốc tịch（2） 经验 3900 tiềm năng 125 Ngân lượng 3500");
        ...
        case 7:
			...
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    为了称霸七国，最近本王经常食不甘味、夜不能眠.有人告诉本王，在楚国民间有一种Trà Jasmine可以增进食欲，促进睡眠.但本王作为一国之君是不可以随便Rời khỏi自己的国家的，所以本王想请你去楚国的民间替本王寻回这种Trà Jasmine，到时候本王一定好好酬谢你的.可以先去楚国找那些楚国的百姓谈一谈.\n"ESC"Tiếp nhận nhiệm vụ\ntalk %x# welcome.8\n"ESC"Rời khỏi", me->get_name(),getoid(me)));
        	else if ( who->get_legend(TASK_24,15) && !who->get_legend(TASK_24,16) )
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    恩，本王果然没有看错人，多亏了你替本王找回这种香浓扑鼻的Trà Jasmine，本王现在又可以专注更多的精力于称霸七国的霸业了.本王说过一定要好好酬谢你的，你就别客气了，这些是你应得的，你就收下吧.\n"ESC"Hoàn thành nhiệm vụ\ntalk %x# welcome.9\n"ESC"Rời khỏi", me->get_name(),getoid(me)));
        ...
        case 8:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_QIGUO,"Tề Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,15,"Trà Jasmine（1）" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Trà Jasmine（1）");
        	}
        	break; 
        case 9:
        	if ( who->get_legend(TASK_24,15) && !who->get_legend(TASK_24,16) )
        	{
        		if (TASK_LEGEND_D->check_task_item(who,"雪水",1) != 1 )
        			return;
        		if (TASK_LEGEND_D->check_task_item(who,"Trà Jasmine",1) != 1 )
        		{
			...
			who->add_cash(63000);
			send_user(who,"%c%s",';',"Trà Jasmine（4） 经验 63000 tiềm năng 860 Ngân lượng 63000 幽魂项链图纸 1");
...
//        			return ;
			...
			who->add_cash(55000);
			send_user(who,"%c%s",';',"禀告齐王 经验 71000 tiềm năng 850 Ngân lượng 55000 紫心丹 2");
	...
	case 13:
			...
			who->add_cash(30000);
			send_user(who,"%c%s",';',"完美宝石（1） 经验 32000 tiềm năng 608 Ngân lượng 30000");
	...
	case 14:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"完美宝石（2）");
        	}
        	break;
	case 15:
			...
			who->add_cash(37000);
			send_user(who,"%c%s",';',"完美宝石（2） 经验 39000 tiềm năng 650 Ngân lượng 37000");
	...
	case 16:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"完美宝石（3）");
        	}
        	break;
        case 17:
	        if ( who->get_level() >= 100 && !who->get_legend(TASK_36,26) )
	...
	case 18:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_QIGUO,"Tề Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,58,"Nổi loạn của Kỳ" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Nổi loạn của Kỳ");
        	}
        	break;
	case 19:
		if ( me->get_city_name() != who->get_city_name() )
	...
	case 23:
			...
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    你知道什么是齐国至高Vinh quang tối cao吗，你想获得齐国至高Vinh quang tối cao吗？到我身边来吧，我告诉你，这个，也不能太早让你知道了不是？你先去消灭诛仙阵的10只剑魂，回来我将给你答案.\n"ESC"Tiếp nhận nhiệm vụ\ntalk %x# welcome.24\n"ESC"Rời khỏi", me->get_name(),getoid(me)));
		else if ( who->get_legend(TASK_45,7) && !who->get_legend(TASK_45,8) )
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    呃，看来什么是至高Vinh quang tối cao我不能告诉你啊，因为我必须告诉匹配得上这个荣耀的身份的人才行.\n"ESC"Rời khỏi", me->get_name()));
		else if ( who->get_legend(TASK_45,8) && !who->get_legend(TASK_45,9) )
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    嗯，看来你还是有点能力的，这样吧，我悄悄告诉你什么是齐国至高Vinh quang tối cao，那就是——为本王做事.\n"ESC"Hoàn thành nhiệm vụ\ntalk %x# welcome.25\n"ESC"Rời khỏi", me->get_name(),getoid(me)));
        	else if (who->get_legend(TASK_45,9) && !who->get_legend(TASK_45,10) )
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    最近本王爱妃患上一种怪病，非要闻到Măng tây的味道才能醒来，要么就沉睡不醒，为了爱妃能在宫廷之中陪寡人走动，寡人要尽量在宫廷之中多多种植Măng tây，看爱卿如此关心寡人，想必区区20株Măng tây对于爱卿来说不是难事吧.听说天门阵中许多刁民都喜爱收集Măng tây，想必去问他们取应该很容易办到.\n"ESC"Tiếp nhận nhiệm vụ\ntalk %x# welcome.26\n"ESC"Rời khỏi", me->get_name(),getoid(me)));
		else if ( who->get_legend(TASK_45,10) && !who->get_legend(TASK_45,11) )
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    什么，难道你竟然不能从刁民手中拿来Măng tây么？寡人对你甚为失望，寡人的宫廷可不是毫无荣耀的人来的地方.\n"ESC"Rời khỏi", me->get_name()));
			...
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    楚王考烈王前段时间差人送来了100尾越晶鱼，放养在闭夜池中，母后非常喜爱，来而不往非礼也，寡人也要回赠一些礼物给他，本王有一种trà Mist，产于仙山绝壁，都是有道之士与本王论道之时赠送的，据说每年不超过10斤，相信这种礼物，对得起我们齐国大国的身份了吧.\n"ESC"Tiếp nhận nhiệm vụ\ntalk %x# welcome.28\n"ESC"Rời khỏi", me->get_name(),getoid(me)));
		break;
	case 24:
		if (who->get_level() >= 110 && !who->get_legend(TASK_45,7) )
        	{
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_QIGUO,"Tề Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,69,"Vinh quang tối cao（1）" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Vinh quang tối cao（1）");
        	}
        	break;
        case 25:
		if ( who->get_legend(TASK_45,8) && !who->get_legend(TASK_45,9) )
			...
			who->add_cash(51000);
			send_user(who,"%c%s",';',"Vinh quang tối cao（1） 经验 51000 tiềm năng 780 Ngân lượng 51000");
	...
	case 26:
		if (who->get_legend(TASK_45,9) && !who->get_legend(TASK_45,10) )
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_QIGUO,"Tề Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,70,"Vinh quang tối cao（2）" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Vinh quang tối cao（2）");
        	}
        	break;
        case 27:
		if ( who->get_legend(TASK_45,11) && !who->get_legend(TASK_45,12) )
        	{
			if (TASK_LEGEND_D->check_task_item(who,"Măng tây",20) != 1 )
        			return;        		
        		TASK_LEGEND_D->task_finish(who);
			...
			who->add_cash(53000);
			send_user(who,"%c%s",';',"Vinh quang tối cao（2） 经验 55000 tiềm năng 830 Ngân lượng 53000");
	...
	case 28:
		if (who->get_legend(TASK_45,12) && !who->get_legend(TASK_45,13) )
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_QIGUO,"Tề Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,71,"Vinh quang tối cao（3）" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Vinh quang tối cao（3）");
        	}
        	break;		
	case 99:
