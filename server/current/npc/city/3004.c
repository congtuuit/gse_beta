...
void do_look( object who )
{
	...
	if ( who->get_legend(TASK_43,2) && !who->get_legend(TASK_42,28) )
		tmp += sprintf(ESC HIY "Kẻ biến thái\ntalk %x# welcome.10\n",getoid(me));
...
void do_welcome2( object me, string arg )
{
	...
	case 2:
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,35,"Rượu hảo hạng của Triệu" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Rượu hảo hạng của Triệu");
        	}
        	break;
        case 3:
			...
			who->add_cash(7000);
			send_user(who,"%c%s",';',"Rượu hảo hạng của Triệu 经验 7000 tiềm năng 280 Ngân lượng 7000");
	...
	case 5:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_ZHAOGUO,"Triệu Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,54,"Việc vặt（1）" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Việc vặt（1）");
        	}
        	break;
	case 6:
		if ( who->get_legend(TASK_42,1) && !who->get_legend(TASK_42,2) )
	...
	case 7:
		if ( who->get_legend(TASK_42,1) && !who->get_legend(TASK_42,2) )
		{
			...
			who->add_cash(83000);
			send_user(who,"%c%s",';',"询问消息 经验 95000 tiềm năng 1550 Ngân lượng 83000");
	...
	case 8:
		if ( who->get_legend(TASK_42,2) && !who->get_legend(TASK_42,3) )
		{
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"询问战果");
        	}
		break;
	case 9:
		if ( who->get_legend(TASK_42,8) && !who->get_legend(TASK_42,9) )
		{
			...
			who->add_cash(85000);
			send_user(who,"%c%s",';',"Tin tức chiến tranh 经验 92000 tiềm năng 1700 Ngân lượng 85000 碎星剑图纸 1");
	...
	case 11:
		if ( who->get_level() >= 105 && !who->get_legend(TASK_42,23) )
		{
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"探访民情");
        	}
		break;
	case 12:
		if ( who->get_legend(TASK_43,1) && !who->get_legend(TASK_43,2) )
		{
			...
			who->add_cash(91000);
			send_user(who,"%c%s",';',"陈述实情 经验 99000 tiềm năng 1800 Ngân lượng 91000");
	...
	case 13:
		if ( who->get_legend(TASK_43,2) && !who->get_legend(TASK_42,28) )
		{
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_ZHAOGUO,"Triệu Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,86,"Kẻ biến thái" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Kẻ biến thái");
        	}
		break;
        case 99:
        	INTERIOR_D->do_look( who, me ); 
