...
{ 
	...
	{
		tmp += sprintf(ESC HIY "Đòn quyết định\ntalk %x# welcome.3\n",getoid(me));
	}
...
void do_welcome2( object me, string arg )
{
	...
	case 2:
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,5,"Hình phạt cho gian lận" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Hình phạt cho gian lận");
        	}
        	break;
        case 3:
        	if ( who->get_level() >= 75 && !who->get_legend(TASK_25,1) )
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    最近灵谷洞的一些魍魉鬼经常想飘进城里来，但是因为有我们在守卫，所以他们一直未能得逞，但是如果让他们成功飘进来的话，我们城里面的百姓将不堪设想，为了不让我们城里面的百姓受到危害，你可以代替我们去灵谷洞对这些魍魉鬼作一次Đòn quyết định吗？我想只要一次性消灭他们20个魍魉的话，应该可以对他们造成Đòn quyết định了。\n"ESC"接受任务\ntalk %x# welcome.4\n"ESC"Rời khỏi.", me->get_name(),getoid(me)));
        	else if ( who->get_legend(TASK_25,1) && !who->get_legend(TASK_25,2) )
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    我们是无法Rời khỏi.我们守卫的岗位的，所以就要麻烦你了，如果你觉得实在不行，你可以去找个帮手。\n"ESC"Rời khỏi.", me->get_name()));
		else if ( who->get_legend(TASK_25,2) && !who->get_legend(TASK_25,3) )
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    你做得很好呀，这肯定是一次Đòn quyết định了，看来这些魍魉鬼以后都没有实力再像以前那样频繁地想要飘进城里了，我代表齐国百姓唐茂谢谢你。\n"ESC"Hoàn thành任务\ntalk %x# welcome.5\n"ESC"Rời khỏi.", me->get_name(),getoid(me)));
	...
	case 4:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_QIGUO,"Tề Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,20,"Đòn quyết định" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Đòn quyết định");
        	}
        	break;
        case 5:
			...
			who->add_cash(37000);
			send_user(who,"%c%s",';',"Đòn quyết định 经验 40000 tiềm năng 560 Ngân lượng 37000 嗜天鞋图纸 1");
        ...
        case 7:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"答谢");
        	}
        	break;	   
        case 99:
        	GUARDER_D->do_look_3( who, me ); 
