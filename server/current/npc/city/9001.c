...
mapping Map=([
		...
		"Thượng Đảng":({532,256,160}),"Bình Dương":({ 531, 165, 215}),"Vũ Quan":({519, 231, 179}),"Đồng Quan":({ 518, 84, 83}),
			"Tỉ Võ Trường" : ({ 885,99,71 }), "Tu Luyện Trường" : ({887}), "Cấm Vệ Quân" : ({ 080,185,74 }), "Cầu Cửu Khúc 3" : ({ 374,54,84 }),
...
int get_price_value() { return 5000; }
...
void create()
{
	set_name("Trường Cảnh Truyền Tống Viên");
...
{
	int flag,z,x,y,level,*zxy, price;
	...
	switch(flag)
	{
		case 1:
			...
			result=sprintf(me->get_name() + ":\n    Ngươi muốn tới bản đồ cấp 45 - 50 không ? Ấn Alt+M có thể xem xét trước quái vật trong bản đồ này. Phí %d lượng.\n", price);
		...
		case 2:
			...
			result=sprintf(me->get_name()+":\n    Ngươi muốn tới bản đồ cấp 55 - 60 không ? Ấn Alt+M có thể xem xét trước quái vật trong bản đồ này. Phí %d lượng.\n", price);
		...
		case 3:
			...
			result=sprintf(me->get_name()+":\n    Ngươi muốn tới bản đồ cấp 65 - 70 không ? Ấn Alt+M có thể xem xét trước quái vật trong bản đồ này. Phí %d lượng.\n", price);
		...
		case 4:
			...
			result=sprintf(me->get_name()+":\n    Ngươi muốn tới bản đồ cấp 75 - 80 không ? Ấn Alt+M có thể xem xét trước quái vật trong bản đồ này. Phí %d lượng.\n", price);
		...
		case 5:
			...
			result=sprintf(me->get_name()+":\n    Ngươi muốn tới bản đồ cấp 85 - 90 không ? Ấn Alt+M có thể xem xét trước quái vật trong bản đồ này. Phí %d lượng.\n", price);
		...
		case 6:
			...
			result=sprintf(me->get_name()+":\n    Ngươi muốn tới bản đồ cấp 95 - 100 không ? Ấn Alt+M có thể xem xét trước quái vật trong bản đồ này. Phí %d lượng.\n", price);
		...
		case 7:
			...
			result=sprintf(me->get_name()+":\n    Ngươi muốn tới bản đồ cấp 105 - 110 không ? Ấn Alt+M có thể xem xét trước quái vật trong bản đồ này. Phí %d lượng.\n", price);
		...
		case 8:
			...
			result=sprintf(me->get_name()+":\n    Ngươi muốn tới những MAP khó đi hay không ? Phí %d lượng.\n", price);
...
void send_ok(object who,int z,int x,int y)
{
	...
	price = get_price_value();
	if (who->get_cash() < price) {
		tell_user(who, sprintf(ECHO"Ngươi không có đủ %d lượng", price));
		return;
	}
	...
	tell_user(who, sprintf(ECHO"Phí di chuyển %d lượng.", price));
