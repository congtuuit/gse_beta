...
{ 
	...
	{
		tmp += sprintf(ESC HIY "Cứu mọi người khỏi nguy hiểm\ntalk %x# welcome.1\n",getoid(me));
	}
	if ( sizeof(tmp) )
...
void do_welcome2( object me, string arg )
{
	...
	case 2:
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_WEIGUO,31,"Cứu mọi người khỏi nguy hiểm" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Cứu mọi người khỏi nguy hiểm");
        	}
        	break;       
        case 3:
			...
			who->add_cash(50000);
			send_user(who,"%c%s",';',"Cứu mọi người khỏi nguy hiểm 经验 53000 tiềm năng 740 Ngân lượng 50000 寒星头巾图纸 1");
