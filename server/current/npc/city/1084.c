...
{ 
	...
	if ( (who->get_legend(TASK_30,4) && !who->get_legend(TASK_29,28)) || (who->get_legend(TASK_29,28) && !who->get_legend(TASK_29,30)))
		tmp += sprintf(ESC HIY "Tự kiểm tra\ntalk %x# welcome.6\n",id);
	if ( who->get_legend(TASK_29,30) && !who->get_legend(TASK_30,5) )
		tmp += sprintf(ESC HIY "Mất và phục hồi\ntalk %x# welcome.6\n",id);
	...
	if ( (who->get_level() >= 100 && !who->get_legend(TASK_36,20)) || (who->get_legend(TASK_36,20) && !who->get_legend(TASK_36,22)) )
		tmp += sprintf(ESC HIY "Cuộc thám hiểm\ntalk %x# welcome.17\n",id);		
...
void do_welcome2( object me, string arg )
{
        ...
        case 2:
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,18,"Chuẩn bị" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Chuẩn bị");
        	}
        	break;
        case 3:
			...
			who->add_cash(33000);
			send_user(who,"%c%s",';',"Chuẩn bị 经验 35000 tiềm năng 510 Ngân lượng 33000");
        ...
        case 5:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"询问原因");
        	}
        	break;
        case 6:
        	if ( who->get_level() >= 85 && !who->get_legend(TASK_29,26) )
        ...
        case 7:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"探访将军");
        	}
        	break;
        case 8:
        	if ( who->get_legend(TASK_30,3) && !who->get_legend(TASK_30,4) )
			...
			who->add_cash(50000);
			send_user(who,"%c%s",';',"民间探访 经验 57000 tiềm năng 790 Ngân lượng 50000");
        ...
        case 9:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_QIGUO,"Tề Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,33,"Tự kiểm tra" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Tự kiểm tra");
        	}
        	break;
        case 10:
			...
			who->add_cash(51000);
			send_user(who,"%c%s",';',"Tự kiểm tra 经验 60000 tiềm năng 780 Ngân lượng 51000");
        ...
        case 11:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_QIGUO,"Tề Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,34,"Mất và phục hồi" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Mất và phục hồi");
        	}
        	break;
	case 12:
		if ( who->get_legend(TASK_30,20) && !who->get_legend(TASK_30,21) )
        ...
        case 13:
			...
			who->add_cash(50000);
			send_user(who,"%c%s",';',"深入民间 经验 69000 tiềm năng 810 Ngân lượng 50000");
        ...
        case 14:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"商人诉苦");
        	}
        	break;
        case 15:
        	if ( who->get_legend(TASK_31,11) && !who->get_legend(TASK_31,12) ) 
        ...
        case 16:
			...
			who->add_cash(57000);
			send_user(who,"%c%s",';',sprintf("通告百姓 经验 69000 tiềm năng 830 Ngân lượng 57000 %s 1",tmp->get_name()));
        ...
        case 18:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_QIGUO,"Tề Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,56,"Cuộc thám hiểm" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Cuộc thám hiểm");
        	}
        	break;
	case 19:
			...
			who->add_cash(73000);
			send_user(who,"%c%s",';',"Cuộc thám hiểm 经验 84000 tiềm năng 1040 Ngân lượng 73000");
	...
	case 21:
			...
			who->add_cash(38000);
			send_user(who,"%c%s",';',"四面楚歌（2） 经验 37000 tiềm năng 640 Ngân lượng 38000");
        ...
        case 22:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"四面楚歌（3）");
        	}
        	break;
        }
}