...
{ 
	...
	{
		tmp += sprintf(ESC HIY "Ngọc ở mộ cổ\ntalk %x# welcome.1\n",getoid(me));
	}
	if ( sizeof(tmp) )
...
void do_welcome2( object me, string arg )
{
	...
	case 2:
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_WEIGUO,30,"Ngọc ở mộ cổ" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Ngọc ở mộ cổ");
        	}
        	break;       
        case 3:
			...
			who->add_cash(47000);
			send_user(who,"%c%s",';',"Ngọc ở mộ cổ 经验 47000 tiềm năng 690 Ngân lượng 47000");
