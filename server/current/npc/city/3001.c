...
void do_look( object who ) 
{
	...
	if ( who->get_legend(TASK_43,15) && !who->get_legend(TASK_43,16) )
		tmp += sprintf(ESC HIY "Hoàn thành使命\ntalk %x# welcome.23\n",getoid(me));
...
void do_welcome2( object me, string arg )
{
	...
	case 2:
			...
			who->add_cash(19000);
			send_user(who,"%c%s",';',"面见赵王 经验 17500 tiềm năng 380 Ngân lượng 19000 救急丹图纸 1");
	...
	case 3:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"回奏秦王");
        	}
        	break;
        case 4:
        	if ( who->get_level() >= 50 && who->get_legend(TASK_38,3) && !who->get_legend(TASK_38,4) )
		...
		if ( who->get_level() >= 50 && who->get_legend(TASK_38,3) && !who->get_legend(TASK_38,4) )
		{
			...
			who->add_cash(5000);
			send_user(who,"%c%s",';',"丢失的夜明珠 经验 5500 tiềm năng 230 Ngân lượng 5000");
	...
	case 6:
		...
		if ( who->get_level() >= 100 && who->get_legend(TASK_38,23) && !who->get_legend(TASK_38,24) )
		{
			...
			who->add_cash(38000);
			send_user(who,"%c%s",';',"讨好 经验 40000 tiềm năng 605 Ngân lượng 38000");
	...
	case 8:
		...
		if ( who->get_level() >= 110 && who->get_legend(TASK_39,7) && !who->get_legend(TASK_39,8) )
		{
			...
			who->add_cash(48000);
			send_user(who,"%c%s",';',"交好 经验 50000 tiềm năng 620 Ngân lượng 48000");
        ...
        case 11:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"完美宝石（1）");
        	}
        	break;
	case 12:
        	if ( who->get_legend(TASK_36,9) && !who->get_legend(TASK_36,10) )
			...
			who->add_cash(200000);
			send_user(who,"%c%s",';',sprintf("完美宝石（4） 经验 200000 tiềm năng 2250 Ngân lượng 200000 %s 1",tmp->get_name()));
	...
	case 18:
		if ( who->get_legend(TASK_41,26) && !who->get_legend(TASK_41,27) )
		{
			...
			who->add_cash(83000);
			send_user(who,"%c%s",';',"上奏赵王 经验 93000 tiềm năng 1600 Ngân lượng 83000 斥云冠图纸 1");
	...
	case 20:
		if ( who->get_legend(TASK_42,21) && !who->get_legend(TASK_42,22) )
		{
			...
			who->add_cash(88000);
			send_user(who,"%c%s",';',"禀报赵王 经验 98000 tiềm năng 1750 Ngân lượng 88000 破空刀图纸 1");
	...
	case 22:
		if ( who->get_legend(TASK_43,3) && !who->get_legend(TASK_43,4) )
		{
			...
			who->add_cash(92000);
			send_user(who,"%c%s",';',"施行仁政 经验 96000 tiềm năng 1900 Ngân lượng 92000 霸王枪图纸 1");
	...
	case 24:
		if ( who->get_legend(TASK_43,15) && !who->get_legend(TASK_43,16) )
		{
			...
			who->add_cash(92000);
			send_user(who,"%c%s",';',sprintf("Hoàn thành使命 经验 120000 tiềm năng 2000 Ngân lượng 92000 %s 1",tmp->get_name()));
	...
	case 25:
			...
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    徐福道长前来用了如意玉牌世子果然病情大为好转，他向寡人提起云梦泽中有一位隐世神医，唤作鬼谷子，据说能活死人，肉白骨，世间尚无他不可治之病，你可代本王前去Theo y tế ，看他能否为世子除去病根.如若事谐，本王必当重赏.\n"ESC"Tiếp nhận nhiệm vụ\ntalk %x# welcome.28\n"ESC"Rời khỏi", me->get_name(),getoid(me)));
	...
	case 26:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_ZHAOGUO,"Triệu Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,60,"Theo y tế （1）" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Theo y tế （1）");
        	}
        	break;
	case 27:
		if ( who->get_legend(TASK_45,19) && !who->get_legend(TASK_45,20) )
		{
			...
			who->add_cash(51000);
			send_user(who,"%c%s",';',"Theo y tế （1）经验 51500 tiềm năng 780 Ngân lượng 51000");
	...
	case 28:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_ZHAOGUO,"Triệu Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,61,"Theo y tế （2）" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Theo y tế （2）");
        	}
        	break;
	case 29:
		if ( who->get_legend(TASK_45,29) && !who->get_legend(TASK_45,30) )
		{
			...
			who->add_cash(225000);
			send_user(who,"%c%s",';',"Theo y tế （5）经验 230000 tiềm năng 2450 Ngân lượng 225000 冥光鞋图纸 1");
