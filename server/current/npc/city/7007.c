...
void do_welcome2( object me, string arg )
{
        ...
        case 1:
			...
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    找来了Vải thiều，我还得派人送过去，路途遥远，又害怕Vải thiều坏掉，还真有点麻烦呀。\n"ESC"Hoàn thành任务\ntalk %x# welcome.4\n"ESC"Rời khỏi.", me->get_name(),getoid(me)));
	...
	case 2:
			...
			who->add_cash(50000);
			send_user(who,"%c%s",';',"Việc vặt（1） 经验 58000 tiềm năng 850 Ngân lượng 50000");
        ...
        case 3:
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,53,"Việc vặt（2）" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Việc vặt（2）");
        	}
        	break;
        case 4:
        	if ( who->get_legend(TASK_32,7) && !who->get_legend(TASK_32,8) )
        	{
        		if (TASK_LEGEND_D->check_task_item(who,"Vải thiều",10) != 1 )
        			return;
			TASK_LEGEND_D->task_finish(who);
			...
			who->add_cash(55000);
			send_user(who,"%c%s",';',"Việc vặt（2） 经验 65000 tiềm năng 920 Ngân lượng 55000");
