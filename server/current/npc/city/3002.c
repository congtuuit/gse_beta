...
void do_look( object who )
{
	...
	if ( who->get_legend(TASK_41,24) && !who->get_legend(TASK_41,25) )
		tmp += sprintf(ESC HIY "Cuộc phiêu lưu Thiên Môn\ntalk %x# welcome.13\n",getoid(me));
...
void do_welcome2( object me, string arg )
{
        ...
        case 2:
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,41,"Bộ xương trong thảm sát" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Bộ xương trong thảm sát");
        	}
        	break; 
        case 3:
			...
			who->add_cash(45000);
			send_user(who,"%c%s",';',"Bộ xương trong thảm sát 经验 46000 tiềm năng 645 Ngân lượng 45000");
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,41,"" );
        	}
        	break;   
        case 4:
        	if (who->get_level() >= 80 && !who->get_legend(TASK_28,26))
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    我可是Triệu Quốc Đại Tướng QuânLý Mục，想跟着我混吗，很好，可是我手下从来不收无名之辈，新来的可得投个投名状，如果仅仅让你收拾个小瘪三那就太没有价值了，那样你最多能够作个小校，要是你想做更高的，那么就得接受我更艰巨的Thử thách，怎么准备好了吗，那好，你去万人坑杀掉刀盾骷髅10只、剑盾骷髅10只、双刀骷髅8只，如果完好无缺的回来本大将军自然会重用你。\n"ESC"接受任务\ntalk %x# welcome.5\n"ESC"Rời khỏi.", me->get_name(),getoid(me)));
			...
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    你能单独Hoàn thành这次行动，并且没有走漏半点风声，真不错！\n"ESC"Hoàn thành任务\ntalk %x# welcome.8\n"ESC"Rời khỏi.", me->get_name(),getoid(me)));
        ...
        case 5:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_ZHAOGUO,"Triệu Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,52,"Bằng chứng sức mạnh" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Bằng chứng sức mạnh");
        	}
        	break; 
        case 6:
			...
			who->add_cash(49000);
			send_user(who,"%c%s",';',"Bằng chứng sức mạnh 经验 52000 tiềm năng 710 Ngân lượng 49000");
        ...
        case 7:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"秘密行动");
        	}
        	break; 
        case 8:
			...
			who->add_cash(50000);
			send_user(who,"%c%s",';',"秘密行动 经验 52500 tiềm năng 730 Ngân lượng 50000 翻云头巾图纸 1");
        ...
        case 10:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"挖掘人才");
        	}
        	break;
	case 11:
		if ( who->get_level() >= 110 && who->get_legend(TASK_39,3) && !who->get_legend(TASK_39,4) )
		...
		if ( who->get_level() >= 110 && who->get_legend(TASK_39,3) && !who->get_legend(TASK_39,4) )
		{
			...
			who->add_cash(48000);
			send_user(who,"%c%s",';',"嘱托之事 经验 50000 tiềm năng 610 Ngân lượng 48000");
	...
	case 13:
		...
		if ( who->get_legend(TASK_41,16) && !who->get_legend(TASK_41,17) )
		{
			...
			who->add_cash(81000);
			send_user(who,"%c%s",';',"询问原由 经验 89000 tiềm năng 1500 Ngân lượng 81000");
	...
	case 15:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"打探下落");
        	}
        	break;
        case 16:
		if ( who->get_legend(TASK_41,24) && !who->get_legend(TASK_41,25) )
		{
			...
			who->add_cash(84000);
			send_user(who,"%c%s",';',"Cuộc phiêu lưu Thiên Môn 经验 94000 tiềm năng 1700 Ngân lượng 84000");
	...
	case 17:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"上奏赵王");
        	}
        	break;
	case 18:
		if ( who->get_legend(TASK_42,3) && !who->get_legend(TASK_42,4) )
		...
		if ( who->get_legend(TASK_42,3) && !who->get_legend(TASK_42,4) )
		{
			...
			who->add_cash(84000);
			send_user(who,"%c%s",';',"询问战果 经验 94000 tiềm năng 1550 Ngân lượng 84000");
	...
	case 20:
		if ( who->get_legend(TASK_42,4) && !who->get_legend(TASK_42,5) )
		{
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"询查战报");
        	}
		break;
	case 21:
	...
	case 22:
		if (who->get_level() >= 110 && !who->get_legend(TASK_43,26))
		{
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_ZHAOGUO,"Triệu Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,59,"Biểu tượng quyền lực" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Biểu tượng quyền lực");
        	}
		break;
        case 23:
		if ( who->get_legend(TASK_43,27) && !who->get_legend(TASK_43,28) )
		{
			...
			who->add_cash(93000);
			send_user(who,"%c%s",';',"Biểu tượng quyền lực 经验 90000 tiềm năng 970 Ngân lượng 93000");
        ...
        case 25:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_ZHAOGUO,"Triệu Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,101,"Thu thập các thành phần" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Thu thập các thành phần");
		}
        	break;
        case 26:
			...
			who->add_cash(9600);
			send_user(who,"%c%s",';',"Thu thập các thành phần 经验 12800 tiềm năng 500 Ngân lượng 9600");
