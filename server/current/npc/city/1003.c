...
void do_look( object who )
{
	...
	{
		tmp = sprintf(ESC HIY "Tìm kiếm kho báu\ntalk %x# welcome.6\n",id);
	}
...
void do_welcome2( object me, string arg )
{
        ...
        case 2:
			...
			who->add_cash(1800);
			send_user(who,"%c%s",';',"唇亡齿寒（1） 经验 2500 tiềm năng 60 Ngân lượng 1800");
        ...
        case 3:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"唇亡齿寒（2）");
        	}
        	break;
 	case 4:
        	if ( who->get_legend(TASK_09,24) && !who->get_legend(TASK_09,25) )
			...
			who->add_cash(2200);
			send_user(who,"%c%s",';',"唇亡齿寒（3） 经验 3000 tiềm năng 95 Ngân lượng 2200");
        ...
        case 5:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"唇亡齿寒（4）");
        	}
        	break;
        case 6:
        	if ( who->get_legend(TASK_29,21) && !who->get_legend(TASK_29,22) ) 
        ...
        case 7:
			...
			who->add_cash(54000);
			send_user(who,"%c%s",';',"水落石出 经验 52000 tiềm năng 720 Ngân lượng 54000");
	...
	case 8:
        	if ( who->get_legend(TASK_29,22) && !who->get_legend(TASK_29,23) )
        	{
 			...
 			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_QIGUO,"Tề Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,29,"Tìm kiếm kho báu" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Tìm kiếm kho báu");
        	}
        	break;
        case 9:
	        if ( who->get_level() >= 100 && who->get_legend(TASK_38,21) && !who->get_legend(TASK_38,22) ) 
        ...
        case 10:
			...
			who->add_cash(35000);
			send_user(who,"%c%s",';',"换份工作 经验 38000 tiềm năng 600 Ngân lượng 35000");
