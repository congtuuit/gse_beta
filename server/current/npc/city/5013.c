...
void do_welcome2( object me, string arg )
{
        ...
        case 1:
			...
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    你肯替天行道，是个真正的%s！\n"ESC"Hoàn thành任务\ntalk %x# welcome.3\n"ESC"Rời khỏi.", me->get_name(),who->get_gender()==1?"大侠":"女侠",getoid(me)));
        	break;
        case 2:
        	if(who->get_level() >= 60 && !who->get_legend(TASK_19,13))
        	{
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QINGUO,20,"Kẻ phản bội" );
       ...
       case 3:
			...
			who->add_cash(7000);
			send_user(who,"%c%s",';',"Kẻ phản bội 经验 7000 tiềm năng 280 Ngân lượng 7000");
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QINGUO,20,"" );
        	}
        	break;     
        case 4:
        	if (who->get_level() >= 65 && !who->get_legend(TASK_20,7))
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    最近从我那楚国亲戚那里学来做Rượu vang的方子，据说可以驱邪祛毒，端午晨时群龙行雨时摆上Rượu vang宴，还可祈求一个风调雨顺，五谷丰登的好年景，可是最近端午将至，雄黄脱销了，听说秦长城烽火台可以找到，你能帮我看看吗？\n"ESC"接受任务\ntalk %x# welcome.5\n"ESC"Rời khỏi.", me->get_name(),getoid(me)));
        ...
        case 5:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_QINGUO,"Tần Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QINGUO,49,"Rượu vang" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Rượu vang");
        	}
        	break;
       case 6:
		...
		if ( who->get_level() >= 65 && !who->get_legend(TASK_38,7) )
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"韩总管的托付");
        	}
        	break;
        case 9:
		if ( who->get_level() >= 95 && !who->get_legend(TASK_38,27) )
		...
		if ( who->get_level() >= 95 && !who->get_legend(TASK_38,27) )
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"恩怨");
        	}
        	break;
        case 99:
        	GUARDER_D->do_look_2( who, me ); 
