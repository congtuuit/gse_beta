...
void do_welcome2( object me, string arg )
{
	...
	case 2:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"探听虚实");
        	}
        	break;
        case 3:
        	if ( who->get_legend(TASK_20,22) && !who->get_legend(TASK_20,23) )
        	{
        		if (TASK_LEGEND_D->check_task_item(who,"Bản vẽ cung điện",1) != 1 )
        			return;
			...
			who->add_cash(9500);
			send_user(who,"%c%s",';',"Bản vẽ cung điện 经验 12000 tiềm năng 330 Ngân lượng 9500");
	...
	case 4:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_QINGUO,"Tần Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QINGUO,25,"Gỗ gụ thiên niên kỷ" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Gỗ gụ thiên niên kỷ");
        	}
        	break;
        case 5:
        	if ( who->get_legend(TASK_20,25) && !who->get_legend(TASK_20,26) )
        	{
        		if (TASK_LEGEND_D->check_task_item(who,"Gỗ gụ thiên niên kỷ",1) != 1 )
        			return;
			TASK_LEGEND_D->task_finish(who);
			...
			who->add_cash(10500);
			send_user(who,"%c%s",';',"Gỗ gụ thiên niên kỷ 经验 12500 tiềm năng 340 Ngân lượng 10500");
	...
	case 6:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_QINGUO,"Tần Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QINGUO,26,"Tránh đường!" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Tránh đường!");
        	}
        	break;
        case 7:
			...
			who->add_potential(340);
			who->add_cash(10000);
			send_user(who,"%c%s",';',"Tránh đường! 经验 13000 tiềm năng 340 Ngân lượng 10000 垂云披袍图纸 1");
	...
	case 8:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"破土动工");
        	}
        	break;
        case 9:
        	if ( who->get_level() >= 60 && who->get_legend(TASK_38,5) && !who->get_legend(TASK_38,6) )
        ...
        case 10:
			...
			who->add_cash(6500);
			send_user(who,"%c%s",';',"打造武器 经验 6500 tiềm năng 230 Ngân lượng 6500");
