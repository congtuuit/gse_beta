...
{ 
	...
	if ( who->get_legend(TASK_42,6) && !who->get_legend(TASK_41,30) )
		tmp += sprintf(ESC HIY "Đột nhập vào Thiên Quân\ntalk %x# welcome.32\n",getoid(me));
	if ( who->get_legend(TASK_41,30) && !who->get_legend(TASK_42,7) )
		tmp += sprintf(ESC HIY "Tin tức chiến tranh\ntalk %x# welcome.32\n",getoid(me));
	...
	if ( who->get_legend(TASK_42,14) && !who->get_legend(TASK_42,15) )
		tmp += sprintf(ESC HIY "Giết sát thủ\ntalk %x# welcome.37\n",getoid(me));
	...
	if ( who->get_legend(TASK_42,24) && !who->get_legend(TASK_42,27) )
		tmp += sprintf(ESC HIY "Loại bỏ cái ác cho nhân dân\ntalk %x# welcome.42\n",getoid(me));
...
void do_welcome2( object me, string arg )
{
	...
	case 5:
        	if( who->get_level() >=55 && !who->get_legend(TASK_16,14) )
        	{
   			...
   			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_ZHAOGUO,"Triệu Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,15,"Khóc trên Vạn Lý Trường Thành" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Khóc trên Vạn Lý Trường Thành");
        	}
        	break;       
        case 6:
			...
			who->add_cash(6500);
			send_user(who,"%c%s",';',"Khóc trên Vạn Lý Trường Thành 经验 6400 tiềm năng 230 Ngân lượng 6500 铁片 2");
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,15,"" );
        	}
        	break;  
 	case 7:
        	if ( who->get_level() >=55 && !who->get_legend(TASK_16,20) )
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    我的叔父临终前嘱咐我，不管用什么方法，一定要将燕长城烽火台上的三刀鬼除掉，我也不知道他们之间有什么恩怨，但是这是他的遗言，我也希望能达成他这个愿望.\n    可是我去燕长城Thử thách了三刀鬼三次，都没能成功，还差点丢了性命，听说三刀鬼号称三刀要人命，我能从他的刀下死里逃生已属万幸，所以想请你前去燕长城的烽火台除掉三刀鬼，Hoàn thành我叔父的遗愿.\n"ESC"Nhận nhiệm vụ\ntalk %x# welcome.8\n"ESC"Rời khỏi", me->get_name(),getoid(me)));
	...
	case 8:
        	if( who->get_level() >=55 && !who->get_legend(TASK_16,20) )
        	{
   			...
   			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_ZHAOGUO,"Triệu Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,17,"Thức ăn" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Thức ăn");
        	}
        	break;       
        case 9:
			...
			who->add_cash(6400);
			send_user(who,"%c%s",';',"Thức ăn 经验 6600 tiềm năng 240 Ngân lượng 6400 玉菩提图纸 1");
	...
	case 11:
        	if( who->get_level() >=55 && !who->get_legend(TASK_16,23) )
        	{
   			...
   			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_ZHAOGUO,"Triệu Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,18,"Truyền thuyết về quần áo thần" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Truyền thuyết về quần áo thần");
        	}
        	break;       
        case 12:
	...
	case 14:
        	if( who->get_level() >=55 && !who->get_legend(TASK_16,26) )
        	{
   			...
   			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_ZHAOGUO,"Triệu Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,19,"Một ngàn dặm đường đi bộ" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Một ngàn dặm đường đi bộ");
        	}
        	break;       
        case 15:
			...
			who->add_cash(6500);
			send_user(who,"%c%s",';',"Một ngàn dặm đường đi bộ 经验 6500 tiềm năng 250 Ngân lượng 6500");
	...
	case 17:
        	if( who->get_level() >=55 && !who->get_legend(TASK_17,7) )
        	{
   			...
   			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_ZHAOGUO,"Triệu Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,22,"Vào nửa đêm" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Vào nửa đêm");
        	}
        	break;       
        case 18:
			...
			who->add_potential(250);
			who->add_cash(6700);
			send_user(who,"%c%s",';',"Vào nửa đêm 经验 6600 tiềm năng 230 Ngân lượng 6700");
	...
	case 20:
        	if( who->get_level() >=55 && !who->get_legend(TASK_17,25) )
        	{
   			...
   			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_ZHAOGUO,"Triệu Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,28,"Cướp nhà" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Cướp nhà");
        	}
        	break;       
        case 21:
			...
			who->add_cash(6800);
			send_user(who,"%c%s",';',"Cướp nhà 经验 6900 tiềm năng 260 Ngân lượng 6800");
 	...
 	case 22:
			...
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    根据燕总管给我的信里面的提示，这对同心锁应该就在万人坑里面的某一个骷髅身上.但是Bộ xương trong thảm sát我可打不过，所以我还需要你的帮忙，帮我去万人坑杀掉那里的骷髅，并找到在他们身上的同心锁.拜托了.\n"ESC"Nhận nhiệm vụ\ntalk %x# welcome.25\n"ESC"Rời khỏi", me->get_name(),getoid(me)));
	...
	case 23:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"同心锁（1）");
        	}
        	break;  
        case 24:
        	if ( who->get_legend(TASK_27,14) && !who->get_legend(TASK_27,15) )
			...
			who->add_cash(22000);
			send_user(who,"%c%s",';',"Yên Vương rất thích vàng（3） 经验 24000 tiềm năng 420 Ngân lượng 22000");
	...
	case 25:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"同心锁（4）");
        	}
        	break;  
        case 26:
			...
			who->add_cash(100000);
			send_user(who,"%c%s",';',sprintf("Yên Vương rất thích vàng（4） 经验 100000 tiềm năng 1400 Ngân lượng 100000 %s 1",tmp->get_name()));
	...
	case 28:
		if ( who->get_level() >=100 && who->get_legend(TASK_38,27) && !who->get_legend(TASK_38,28) )
		{
			...
			who->add_cash(40000);
			send_user(who,"%c%s",';',"恩怨 经验 45000 tiềm năng 610 Ngân lượng 40000");
	...
	case 30:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"四面楚歌（1）");
        	}
        	break; 		
	case 31:
		if ( who->get_legend(TASK_40,3) && !who->get_legend(TASK_40,4) ) 
		{
			...
			who->add_cash(200500);
			send_user(who,"%c%s",';',"四面楚歌（5） 经验 200500 tiềm năng 2255 Ngân lượng 200500 麒麟盔图纸 1");
	...
	case 33:
		if ( who->get_legend(TASK_42,5) && !who->get_legend(TASK_42,6) )
		{
			...
			who->add_cash(85000);
			send_user(who,"%c%s",';',"询查战报 经验 95000 tiềm năng 1650 Ngân lượng 85000");
	...
	case 34:
		if ( who->get_legend(TASK_42,6) && !who->get_legend(TASK_41,28) )
		{
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_ZHAOGUO,"Triệu Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,76,"Đột nhập vào Thiên Quân" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Đột nhập vào Thiên Quân");
        	}
		break;
	case 35:
		if ( who->get_legend(TASK_41,29) && !who->get_legend(TASK_41,30) )
		{
			...
			who->add_potential(1650);
			who->add_cash(86000);
			send_user(who,"%c%s",';',"Đột nhập vào Thiên Quân 经验 95000 tiềm năng 1650 Ngân lượng 86000");
	...
	case 36:
		if ( who->get_legend(TASK_41,30) && !who->get_legend(TASK_42,7) )
		{
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_ZHAOGUO,"Triệu Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,77,"Tin tức chiến tranh" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Tin tức chiến tranh");
        	}
		break;
	case 37:
		if ( who->get_legend(TASK_42,10) && !who->get_legend(TASK_42,11) )
	...
	case 38:
		if ( who->get_legend(TASK_42,10) && !who->get_legend(TASK_42,11) )
		{
			...
			who->add_cash(87000);
			send_user(who,"%c%s",';',"询查原因 经验 96000 tiềm năng 1750 Ngân lượng 87000");
	...
	case 39:
		if ( who->get_legend(TASK_42,11) && !who->get_legend(TASK_42,12) )
		{
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"小试身手");
        	}
		break;
	case 40:
		if ( who->get_legend(TASK_42,13) && !who->get_legend(TASK_42,14) )
		{
			...
			who->add_cash(88000);
			send_user(who,"%c%s",';',"小试身手 经验 97000 tiềm năng 1700 Ngân lượng 88000");
	...
	case 41:
		if ( who->get_legend(TASK_42,14) && !who->get_legend(TASK_42,15) )
		{
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_ZHAOGUO,"Triệu Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,80,"Giết sát thủ" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Giết sát thủ");
        	}
		break;
	case 42:
		if ( who->get_legend(TASK_42,23) && !who->get_legend(TASK_42,24) )
	...
	case 43:
		if ( who->get_legend(TASK_42,23) && !who->get_legend(TASK_42,24) )
		{
			...
			who->add_cash(89000);
			send_user(who,"%c%s",';',"探访民情 经验 98000 tiềm năng 1800 Ngân lượng 89000");
	...
	case 44:
		if ( who->get_legend(TASK_42,24) && !who->get_legend(TASK_42,25) )
		{
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_ZHAOGUO,"Triệu Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,84,"Loại bỏ cái ác cho nhân dân" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Loại bỏ cái ác cho nhân dân");
        	}
		break;
	case 45:
		if ( who->get_legend(TASK_42,26) && !who->get_legend(TASK_42,27) )
		{
			...
			who->add_cash(90000);
			send_user(who,"%c%s",';',"Loại bỏ cái ác cho nhân dân 经验 97000 tiềm năng 1850 Ngân lượng 90000");
	...
	case 46:
		if ( who->get_legend(TASK_42,27) && !who->get_legend(TASK_43,1) )
		{
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"陈述实情");
        	}
		break;
	case 47:
		if ( who->get_legend(TASK_43,5) && !who->get_legend(TASK_43,6) )
			...
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    我想起可能有个人能够帮助你彻查此事，你早日查明真相也好让我们安心啊，我在人口主要失踪地赵长城入口处一带，有个朋友叫做李防，你将我写的Thư thăm给他，他兴许能够透露更多的情况.\n"ESC"Nhận nhiệm vụ\ntalk %x# welcome.49\n"ESC"Rời khỏi", me->get_name(),getoid(me)));
	...
	case 48:
		if ( who->get_legend(TASK_43,5) && !who->get_legend(TASK_43,6) )
		{
			...
			who->add_potential(1750);
			who->add_cash(91000);
			send_user(who,"%c%s",';',"询察起因 经验 93000 tiềm năng 1750 Ngân lượng 91000");
	...
	case 49:
		if ( who->get_legend(TASK_43,6) && !who->get_legend(TASK_43,7) )
		{
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"查访知情人");
        	}
		break;
        }
}