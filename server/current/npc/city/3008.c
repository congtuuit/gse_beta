...
void do_look( object who )
{
	...
	if ( who->get_legend(TASK_42,17) && !who->get_legend(TASK_42,20) )
		tmp += sprintf(ESC HIY "Tìm bản đồ\ntalk %x# welcome.1\n",getoid(me));
	...
	if ( who->get_legend(TASK_43,13) && !who->get_legend(TASK_43,14) )
		tmp += sprintf(ESC HIY "Tìm danh sách\ntalk %x# welcome.6\n",getoid(me));
...
void do_welcome2( object me, string arg )
{
	...
	case 2:
			...
			who->add_cash(88000);
			send_user(who,"%c%s",';',"Giết sát thủ 经验 97000 tiềm năng 1650 Ngân lượng 88000");
	...
	case 3:
		if ( who->get_legend(TASK_42,17) && !who->get_legend(TASK_42,18) )
		{
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,81,"Tìm bản đồ" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Tìm bản đồ");
        	}
		break;
	case 4:
			...
			who->add_cash(86000);
			send_user(who,"%c%s",';',"Tìm bản đồ 经验 99000 tiềm năng 1600 Ngân lượng 86000");
	...
	case 5:
		if ( who->get_legend(TASK_42,20) && !who->get_legend(TASK_42,21) )
		{
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"禀报赵王");
        	}
		break;
        case 6:
        	if ( who->get_legend(TASK_43,13) && !who->get_legend(TASK_43,14) )
	...
	case 7:
			...
			who->add_cash(93000);
			send_user(who,"%c%s",';',"Tìm danh sách 经验 110000 tiềm năng 1850 Ngân lượng 93000");
	...
	case 8:
		if ( who->get_legend(TASK_43,14) && !who->get_legend(TASK_43,15) )
		{
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_ZHAOGUO,"Triệu Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,92,"Hoàn thành使命" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Hoàn thành使命");
        	}
		break;
        case 99:
