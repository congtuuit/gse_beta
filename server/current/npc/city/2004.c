...
void do_look( object who )
{
	...
	{
		tmp = sprintf(ESC HIY "Những rắc rối của Cán bộ Nội vụ\ntalk %x# welcome.1\n",getoid(me));
	}
	...
	{
		tmp += sprintf(ESC HIY "Gia vị nấu cá\ntalk %x# welcome.8\n",getoid(me));
	}
	...
	if ( (who->get_legend(TASK_25,18) && !who->get_legend(TASK_25,19)) || (who->get_legend(TASK_25,19) && !who->get_legend(TASK_25,21)) )
		tmp += sprintf(ESC HIY "Thử thách bản thân\ntalk %x# welcome.11\n",getoid(me));
	if ( who->get_legend(TASK_25,21) && !who->get_legend(TASK_25,22) )
		tmp += sprintf(ESC HIY "Loại bỏ những kẻ phản bội\ntalk %x# welcome.11\n",getoid(me));
	...
	if ( (who->get_legend(TASK_26,13) && !who->get_legend(TASK_26,14)) || (who->get_legend(TASK_26,14) && !who->get_legend(TASK_26,16)) )
		tmp += sprintf(ESC HIY "Kiểm tra sức mạnh\ntalk %x# welcome.19\n",getoid(me));
...
void do_welcome2( object me, string arg )
{
        ...
        case 1:
			...
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    大侠你终于出现了！人心难测，小人难防。本官的得力助手吴孜，为了谋取私利，竟然偷取本国的机密要件。如果大王知道此事定会怪罪于我。还望%s能速将此Kẻ phản bội除掉。\n"ESC"接受任务\ntalk %x# welcome.2\n"ESC"Rời khỏi.", me->get_name(),NPC_RANK_D->get_respect_2(who),getoid(me)));
        ...
        case 2:
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_HANGUO,2,"Những rắc rối của Cán bộ Nội vụ" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Những rắc rối của Cán bộ Nội vụ");
        	}
        	break;
        case 3:
			...
			who->add_cash(1300);
			send_user(who,"%c%s",';',"Những rắc rối của Cán bộ Nội vụ 经验 1100 tiềm năng 60 Ngân lượng 1300 地瓜解暑汤 2");
        ...
        case 5:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"宫廷的喧闹");
        	}
        	break;
        case 6: 
		...
		who->add_cash(1200);
		send_user(who,"%c%s",';',"宫廷的喧闹 经验 1300 tiềm năng 70 Ngân lượng 1200 Trái quýt 3");
        ...
        case 9:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_HANGUO,"Hàn Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_HANGUO,41,"Gia vị nấu cá" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Gia vị nấu cá");
        	}
        	break;        	
        case 10:
			...
			who->add_cash(25000);
			send_user(who,"%c%s",';',"Gia vị nấu cá 经验 26000 tiềm năng 440 Ngân lượng 25000");
        ...
        case 11:
			...
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    大王已经阅读了刘方士的请命函后，察觉事态严重。现在，大王派在下募集有能之是开始围剿灵谷洞的妖物，我看阁下也是有能之士，如果阁下有兴趣的话，可以替我国出一分力，到灵谷洞一层除掉20只铜甲尸和20只Bảo vệ người chết。你除掉20只铜甲尸和20只Bảo vệ người chết后，回来我这里，我会给你丰厚的报酬的。\n"ESC"接受任务\ntalk %x# welcome.13\n"ESC"Rời khỏi.", me->get_name(),getoid(me)));
			...
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    你做得实在太出色了，居然在这么短的时间内就把20只铜甲尸和20只Bảo vệ người chết除掉。看来，你有实力帮助我们去实行这次围剿的一个最艰巨的任务。\n"ESC"Hoàn thành任务\ntalk %x# welcome.14\n"ESC"Rời khỏi.", me->get_name(),getoid(me)));
         ...
         case 12:
			...
			who->add_cash(34000);
			send_user(who,"%c%s",';',"出谋献策 经验 35000 tiềm năng 520 Ngân lượng 34000");
	...
	case 13:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_HANGUO,"Hàn Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_HANGUO,22,"Thử thách bản thân" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Thử thách bản thân");
        	}
        	break;       
         case 14:
			...
			who->add_cash(34000);
			send_user(who,"%c%s",';',"Thử thách bản thân 经验 34000 tiềm năng 510 Ngân lượng 34000");
	...
	case 15:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_HANGUO,"Hàn Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_HANGUO,23,"Loại bỏ những kẻ phản bội" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Loại bỏ những kẻ phản bội");
        	}
        	break;       
	case 16:
        	if ( who->get_legend(TASK_25,29) && !who->get_legend(TASK_25,30) )
	...
	case 17:
			...
			who->add_cash(36000);
			send_user(who,"%c%s",';',"内政官的心事 经验 35000 tiềm năng 530 Ngân lượng 36000");
	...
	case 18:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"寻求帮助");
        	}
        	break;
	case 19:
        	if ( who->get_legend(TASK_26,12) && !who->get_legend(TASK_26,13) )
			...
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    我看你武功高强，如果你肯帮忙的话，那么事情就简单多了。但是要杀掉那土匪，先要除去他的爪牙。如果你愿意帮忙你话，那么你先去灵谷洞把那个土匪才操纵的30个恶灵和30个Bảo vệ người chết除掉。然后回来告诉我，我会告诉你那么土匪藏身在哪里的。\n"ESC"接受任务\ntalk %x# welcome.21\n"ESC"Rời khỏi.", me->get_name(),getoid(me)));
	...
	case 20:
			...
			who->add_cash(38000);
			send_user(who,"%c%s",';',"御史的担心 经验 40000 tiềm năng 560 Ngân lượng 38000");
         ...
         case 21:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_HANGUO,"Hàn Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_HANGUO,33,"Kiểm tra sức mạnh" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Kiểm tra sức mạnh");
        	}
        	break;
	case 22:
			...
			who->add_cash(39000);
			send_user(who,"%c%s",';',"Kiểm tra sức mạnh 经验 39000 tiềm năng 580 Ngân lượng 39000");
         ...
         case 23:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_HANGUO,"Hàn Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_HANGUO,34,"Loại bỏ cái ác cho nhân dân" );
	...
	case 24:
			...
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    现在这件案件已经Sự thật được tiết lộ了，但是乐师的父亲现在还被关押在大王的监狱里面。我这就写一封准放奏书，你将它交给大王。书里面已经把这次的事情写得清清楚楚，大王看过之后应该就会把乐师父亲释放。\n"ESC"接受任务\ntalk %x# welcome.28\n"ESC"Rời khỏi.", me->get_name(),getoid(me)));
	...
	case 25:
			...
			who->add_cash(42000);
			send_user(who,"%c%s",';',"Tìm một cách 经验 43000 tiềm năng 620 Ngân lượng 42000");
         ...
         case 26:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_HANGUO,"Hàn Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_HANGUO,39,"Ngăn rò rỉ" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Ngăn rò rỉ");
        	}
        	break;
	case 27:
			...
			who->add_cash(43000);
			send_user(who,"%c%s",';',"Ngăn rò rỉ 经验 44000 tiềm năng 630 Ngân lượng 43000 承影剑图纸 1");
	...
	case 28:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"上奏韩王");
        	}
        	break;
        case 99:	
