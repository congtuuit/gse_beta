...
void do_look( object who ) 
{
	...
	{
		tmp = sprintf(ESC HIY "vá lại chuồng sau khi con cừu bị ăn trộm\ntalk %x# welcome.1\n",getoid(me));
	}
...
void do_welcome2( object me, string arg )
{
        ...
        case 1:
			...
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    Người ta bảo anh Trương In phải làm gì đó.Anh chưa xong sao?\n"ESC"Rời khỏi.", me->get_name()));
        	else if ( who->get_legend(TASK_NEWBIE_03,27) && !who->get_legend(TASK_NEWBIE_03,28) )
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    Vẫn chưa quá muộn để hàn gắn khi lũ cừu chết. thì ta đã mất đi Trang Xương, một linh mục tháo vát.！\n"ESC"Hoàn thành nhiệm vụ\ntalk %x# welcome.2\n"ESC"Rời khỏi.", me->get_name(),getoid(me)));
        	break;
        case 2:
