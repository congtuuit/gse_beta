...
void do_welcome2( object me, string arg )
{
        ...
        case 2:
			...
			who->add_cash(3800);
			send_user(who,"%c%s",';',"兄弟情深 经验 4400 tiềm năng 140 Ngân lượng 3800");
	...
	case 4:
		if ( who->get_level() >= 50 && !who->get_legend(TASK_13,25) ) 
		{
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,21,"Sở thích" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Sở thích");
        	}
		break;
	case 5:
		if ( who->get_legend(TASK_13,26) && !who->get_legend(TASK_13,27) )
			...
			who->add_cash(4600);
			send_user(who,"%c%s",';',"Sở thích 经验 5700 tiềm năng 220 Ngân lượng 4600");
	...
	case 7:
		if ( who->get_level() >= 45 && !who->get_legend(TASK_38,3) )
		{
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"丢失的夜明珠");
        	}
		break;
        case 99:
        	GUARDER_D->do_look_3( who, me ); 
