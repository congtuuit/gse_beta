...
void do_welcome2( object me, string arg )
{
	...
	case 2:
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,18,"Thử thách" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Thử thách");
        	}
        	break;       
        case 3:
			...
			who->add_cash(4000);
			send_user(who,"%c%s",';',"Thử thách 经验 5000 tiềm năng 160 Ngân lượng 4000");
	...
	case 5:
		if ( who->get_level() >= 50 && !who->get_legend(TASK_13,28) ) 
		{
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_YANGUO,"Yên Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,22,"Nguy hiểm tìm ẩn" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Nguy hiểm tìm ẩn");
        	}
		break;
	case 6:
		if ( who->get_legend(TASK_13,29) && !who->get_legend(TASK_13,30) )
			...
			who->add_cash(4700);
			send_user(who,"%c%s",';',"Nguy hiểm tìm ẩn 经验 5800 tiềm năng 230 Ngân lượng 4700");
	...
	case 7:
        ...
        case 8:
        	if ( who->get_legend(TASK_32,2) && !who->get_legend(TASK_32,3) )
        	{
        		if (TASK_LEGEND_D->check_task_item(who,"Dao đơn",8) != 1 )
        			return;
			...
			who->add_cash(53000);
			send_user(who,"%c%s",';',"Dao đơn 经验 62000 tiềm năng 900 Ngân lượng 53000");
	...
	case 9:
			...
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    Lửa的尸体在何处！来人，将Lửa鞭尸后，割下头颅挂于军营上方，尸体剁碎了喂狗！让每个人牢记Kẻ phản bội的下场！\n"ESC"Hoàn thành任务\ntalk %x# welcome.11\n"ESC"Rời khỏi.", me->get_name(),getoid(me)));
        	break;
	case 10:
		if ( who->get_level() >= 90 && !who->get_legend(TASK_32,20) ) 
		{
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_YANGUO,"Yên Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,47,"Nguồn gốc của tội lỗi" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Nguồn gốc của tội lỗi");
        	}
		break;
	case 11:
		if ( who->get_legend(TASK_32,21) && !who->get_legend(TASK_32,22) )
			...
			who->add_cash(57000);
			send_user(who,"%c%s",';',"Nguồn gốc của tội lỗi 经验 68000 tiềm năng 915 Ngân lượng 57000");
	...
	case 12:
		...
		if ( who->get_level() >=110 && who->get_legend(TASK_39,1) && !who->get_legend(TASK_39,2) )
		{
			...
			who->add_cash(45000);
			send_user(who,"%c%s",';',"求份差事 经验 48000 tiềm năng 610 Ngân lượng 45000");
	...
	case 15:
		if ( who->get_level() >=110 && !who->get_legend(TASK_43,20) )
		{
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_YANGUO,"Yên Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,50,"Nguồn cung cấp doanh trại" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Nguồn cung cấp doanh trại");
        	}
		break;
	case 16:
		if ( who->get_legend(TASK_43,21) && !who->get_legend(TASK_43,22) )
			...
			who->add_cash(95000);
			send_user(who,"%c%s",';',"Nguồn cung cấp doanh trại 经验 98000 tiềm năng 1100 Ngân lượng 95000 天魔棍图纸 1");
