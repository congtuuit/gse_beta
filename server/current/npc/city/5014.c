...
void do_welcome2( object me, string arg )
{
	...
	case 2:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"身世之谜");
        	}
        	break;
	case 3:
		if ( who->get_level() >= 100 && who->get_legend(TASK_38,25) && !who->get_legend(TASK_38,26) )
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    本来我就早想下Nhật ký thử thách给他们了，没想到被他们抢先一步，好，这封Nhật ký thử thách我接下了！\n"ESC"Hoàn thành任务\ntalk %x# welcome.4\n"ESC"Rời khỏi.", me->get_name(),getoid(me)));
		break;
        case 4:
		if ( who->get_level() >= 100 && who->get_legend(TASK_38,25) && !who->get_legend(TASK_38,26) )
		{
        		if (TASK_LEGEND_D->check_task_item(who,"齐国守卫的Nhật ký thử thách",1) != 1 )
        			return;
			...
			who->add_cash(40000);
			send_user(who,"%c%s",';',"Thử thách 经验 42000 tiềm năng 600 Ngân lượng 40000");
	...
	case 5:
		...
		if ( who->get_level() >= 110 && who->get_legend(TASK_38,29) && !who->get_legend(TASK_38,30) )
		{
			...
			who->add_cash(45000);
			send_user(who,"%c%s",';',"挖掘人才 经验 48000 tiềm năng 610 Ngân lượng 45000");
