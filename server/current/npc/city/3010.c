...
void do_welcome2( object me, string arg )
{
	...
	case 2:
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,6,"Trừng phạt cái ác" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Trừng phạt cái ác");
        	}
        	break;       
        case 3:
			...
			who->add_cash(5500);
			send_user(who,"%c%s",';',"Trừng phạt cái ác 经验 6350 tiềm năng 260 Ngân lượng 5500 炎阳项链图纸 1");
	...
	case 5:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_ZHAOGUO,"Triệu Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,33,"Thật đáng tiếc" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Thật đáng tiếc");
        	}
        	break;
        case 6:
			...
			who->add_cash(6300);
			send_user(who,"%c%s",';',sprintf("Thật đáng tiếc 经验 7000 tiềm năng 270 Ngân lượng 6300 %s 1",tmp->get_name()));
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,33,"" );
        	}
        	break;  
        case 7:
        	if ( who->get_level() >= 80 && !who->get_legend(TASK_27,28) )
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    我表哥说万人坑里那些人不少曾经是贩卖Kiến的商贾，他们至死都不肯放弃自己的货物，结果白白便宜了我们，去那里随便拣都能拣到不少上好的Kiến，可以卖不少钱呢，当然了，Kiến只中意有勇气的勇士，所以请你帮我找来吧，至于佣金是绝对会让你满意的，要知道，敢收这种地方Kiến的人可就我，因为只有我才知道怎么样除去Kiến上的秽气。\n"ESC"接受任务\ntalk %x# welcome.8\n"ESC"Rời khỏi.", me->get_name(),getoid(me)));
        	else if ( who->get_legend(TASK_27,28) && !who->get_legend(TASK_27,29) )
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    叫你找的Kiến呢？我可是准备了丰厚的报酬给你哦。\n"ESC"Rời khỏi.", me->get_name()));
        	else if ( who->get_legend(TASK_27,29) && !who->get_legend(TASK_27,30) )
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    哈哈，拿到这些Kiến，我这辈子都衣食无忧了！\n"ESC"Hoàn thành任务\ntalk %x# welcome.9\n"ESC"Rời khỏi.", me->get_name(),getoid(me)));
	...
	case 8:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_ZHAOGUO,"Triệu Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,44,"Kiến" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Kiến");
        	}
        	break;
        case 9:
        	if ( who->get_legend(TASK_27,29) && !who->get_legend(TASK_27,30) )
        	{
        		if (TASK_LEGEND_D->check_task_item(who,"Kiến",20) != 1 )
        			return;
			...
			who->add_cash(46000);
			send_user(who,"%c%s",';',"Kiến 经验 48000 tiềm năng 680 Ngân lượng 46000 强效法力 1");
        	...
        	if ( who->get_level() >= 55 && !who->get_legend(TASK_38,5) )
		{
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"打造武器");
        	}
		break;
        case 99:
        	GUARDER_D->do_look_3( who, me ); 
