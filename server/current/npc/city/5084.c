...
{ 
	...
	if ( who->get_level() >= 105 && !who->get_legend(TASK_39,1) )
		...
		tmp += sprintf(ESC HIY "Sự giúp đỡ của bạn bè\ntalk %x# welcome.12\n",getoid(me)); 
...
void do_welcome2( object me, string arg )
{
        ...
        case 1:
			...
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    古语有云：九月九日，佩茱萸，食莲耳，饮Cây cúc酒，令长寿，今年的Cây cúc酒我已经可以起坛了，壮士要不先来尝一口？\n"ESC"Hoàn thành任务\ntalk %x# welcome.7\n"ESC"Rời khỏi.", me->get_name(),getoid(me)));
        ...
        case 2:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"重阳节（1）");
        	}
        	break;
       case 3:
			...
			who->add_cash(8000);
			send_user(who,"%c%s",';',"重阳节（1） 经验 7800 tiềm năng 280 Ngân lượng 8000");
	...
	case 4:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"重阳节（2）");
        	}
        	break; 
	case 5:
        	if ( who->get_legend(TASK_20,14) && !who->get_legend(TASK_20,15) )
        	{
        		if (TASK_LEGEND_D->check_task_item(who,"Cây cúc",1) != 1 )
        			return;
        		TASK_LEGEND_D->task_finish(who);
			...
			who->add_cash(7500);
			send_user(who,"%c%s",';',"重阳节（3） 经验 7000 tiềm năng 260 Ngân lượng 7500");
	...
	case 6:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"重阳节（4）");
        	}
        	break; 
	case 7:
        	if ( who->get_legend(TASK_20,17) && !who->get_legend(TASK_20,18) )
        	{
        		if (TASK_LEGEND_D->check_task_item(who,"Lão Tửu",1) != 1 )
        			return;
			...
			who->add_cash(17000);
			send_user(who,"%c%s",';',"重阳节（4） 经验 13000 tiềm năng 750 Ngân lượng 17000 金丝行装图纸 1");
        ...
        case 9:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"道听途说");
        	}
        	break; 
        case 10:
		if ( who->get_level() >= 105 && !who->get_legend(TASK_39,1) )
        ...
        case 11:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"求份差事");
        	}
        	break; 
        case 12:
		if ( who->get_legend(TASK_43,18) && !who->get_legend(TASK_43,19) ) 
	...
	case 13:
			...
			who->add_cash(92000);
			send_user(who,"%c%s",';',"Sự giúp đỡ của bạn bè 经验 91000 tiềm năng 1000 Ngân lượng 92000");
