...
void do_look( object who ) 
{
	...
	if ( who->get_legend(TASK_2_00,19) && !who->get_legend(TASK_2_00,20) ) 
		cmd = sprintf(ESC HIY "Thăm viếng công tượng kỹ năng sống\ntalk %x# welcome.13\n",id);
	if ( who->get_legend(TASK_2_00,21) && !who->get_legend(TASK_2_00,22) ) 
		cmd = sprintf(ESC HIY "Hoàn thành拜访\ntalk %x# welcome.15\n",id);
	if ( who->get_legend(TASK_2_00,22) && !who->get_legend(TASK_2_00,23) ) 
		cmd = sprintf(ESC HIY "Thăm viếng Chu\ntalk %x# welcome.17\n",id);
...
void do_welcome2( object me, string arg )
{
		...
		send_user( who, "%c%c%w%s", ':',3,me->get_char_picid(), sprintf("%s：\n    您于%d年%d月%d日%d时成功投保，该保险的过期日期为%d年%d月%d日%d时。如有需要，保险过期后您还可以继续投保。\n"ESC"Rời khỏi.", me->get_name(),mxTmp[TIME_YEAR],mxTmp[TIME_MON]+1,mxTmp[TIME_MDAY],mxTmp[TIME_HOUR],mxTmp1[TIME_YEAR],mxTmp1[TIME_MON]+1,mxTmp1[TIME_MDAY],mxTmp1[TIME_HOUR] ) );
     		break;
     	case 4:		//Mua
		...
		who->add_cash(- iMoney);
		send_user( who, "%c%c%w%s", ':',3,me->get_char_picid(), sprintf("%s：\n    您花费%d金购买了保险。虽然购买了保险，但是还是要注意安全哦～\n"ESC"Rời khỏi.", me->get_name(),iMoney ) );
...
//			if ( who->get_upgrade_exp() + mpTmp["exp"] <= maxExp )
		...
		if ( iMoney && iExp )
			send_user( who, "%c%c%w%s", ':',3,me->get_char_picid(), sprintf("%s：\n    您领取了%d金和%d点经验的赔偿！%s\n"ESC"Rời khỏi.", me->get_name(),iMoney,iExp,cTmp ) );
		else if ( iMoney )
			send_user( who, "%c%c%w%s", ':',3,me->get_char_picid(), sprintf("%s：\n    您领取了%d金的赔偿！%s\n"ESC"Rời khỏi.", me->get_name(),iMoney,cTmp ) );
		else if ( iExp )
			send_user( who, "%c%c%w%s", ':',3,me->get_char_picid(), sprintf("%s：\n    您领取了%d点经验的赔偿！%s\n"ESC"Rời khỏi.", me->get_name(),iExp,cTmp ) );
		else
			send_user( who, "%c%c%w%s", ':',3,me->get_char_picid(), sprintf("%s：\n    对不起，您身上似乎放不下我给您的赔偿，最好清空了再来找我。%s\n"ESC"Rời khỏi.", me->get_name(), cTmp ) );
	...
	case 12:
		if ( who->get_legend(TASK_2_00,18) && !who->get_legend(TASK_2_00,19) ) 
		{
			...
			who->add_cash(500);
			send_user(who,"%c%s",';',sprintf("拜访周王 经验 300 tiềm năng 35 Ngân lượng 500"));
	...
	case 14:
		if ( who->get_legend(TASK_2_00,19) && !who->get_legend(TASK_2_00,20) ) 
		{
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHOUGUO,20,"Thăm viếng công tượng kỹ năng sống" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Thăm viếng công tượng kỹ năng sống");
		}
		break;	
	case 15:
		if ( who->get_legend(TASK_2_00,21) && !who->get_legend(TASK_2_00,22) ) 
	...
	case 16:
		if ( who->get_legend(TASK_2_00,21) && !who->get_legend(TASK_2_00,22) ) 
		{
			...
			who->add_cash(1500);
			send_user(who,"%c%s",';',sprintf("Thăm viếng công tượng kỹ năng sống 经验 800 tiềm năng 50 Ngân lượng 1500"));
	...
	case 18:
		if ( who->get_legend(TASK_2_00,22) && !who->get_legend(TASK_2_00,23) ) 
		{
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_ZHOUGUO,"Chu Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHOUGUO,21,"Thăm viếng Chu" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Thăm viếng Chu");
		}
		break;	
	case 19:
		if ( who->get_legend(TASK_2_00,24) && !who->get_legend(TASK_2_00,25) ) 
	...
	case 20:
		if ( who->get_legend(TASK_2_00,24) && !who->get_legend(TASK_2_00,25) ) 
		{
			...
			who->add_potential(50);
			who->add_cash(1500);
			send_user(who,"%c%s",';',sprintf("Thăm viếng Chu 经验 800 tiềm năng 50 Ngân lượng 1500"));
	...
	case 22:
		if ( who->get_legend(TASK_2_00,25) && !who->get_legend(TASK_2_00,26) ) 
		{
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"给楚管家的口信");
		}
		break;
	case 73:
