	...
	for(i=0;i<2;i++)
	{
        ...
        if( size > 1 )    // 多人
        {
                for( i = 0; i < size; i ++ ) if( team[i] )
                {
                        ...
                        write_user( team[i], ECHO "您参与击毙了%s，经验 %+d, tiềm năng %+d。", me->get_name(), exp2, pot2 );
                }
        }
        else    // 单人
        {
                ...
                write_user( who, ECHO "您击毙了%s，经验 %+d, tiềm năng %+d。", me->get_name(), exp2, pot2 );
        }
        ...
        &&      ( owner = find_player( sprintf("%d", id) ) ) )
        {
                ...
                write_user( owner, ECHO "洗劫您的%s被击毙了，经验 %+d, tiềm năng %+d。", me->get_name(), exp2, pot2 );
        }
}
...
void drop_items( object me, object who )
{
        ...
        else if( p < 7120 * rate / 100 )    // Ngân lượng
        {
