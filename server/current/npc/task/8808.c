...
void create()
{
        set_name("Doanh Nhân Lang Thang");
...
//        if( p = XY_D->get_seller_point( IS_CHAR_BLOCK ) )
	...
	buf = sprintf("Nghe nói Doanh Nhân Lang Thang sẽ xuất hiện trong khoảng 5 phút với một kho báu bí ẩn"HIR"%s(%d,%d)"NOR"Khu vực, Hey Hey ~ ~ hầu hết mọi người, tôi sẽ không nói với anh Oh！",map->get_name(),x-7,y-7);
...
void reset_2(int p,int x,int y,int z)
{
	...
	log_file("yunyou.txt",sprintf("Lang thang trong %s ，Bàn chải %d(%d,%d) Gần đó。 \n",short_time(),z,x,y));
	TASK_MASTER_D->set_seller_locate(p);
	reset_good2();
}
/*void reset_good()
