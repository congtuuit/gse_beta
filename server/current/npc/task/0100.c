...
void check_legend_task( object who )
{
 	if ( who->get_legend(TASK_NEWBIE_02, 14)&&!who->get_legend(TASK_NEWBIE_02, 15) )
		...
		send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_NEWBIE_01,20,"惩治恶将(已经Hoàn thành)" );
		USER_TASK_D->send_task_intro(who,2,TID_NEWBIE_01,20);
		send_user( who, "%c%s", '!', HIY"惩治恶将"NOR" 任务Hoàn thành");			
		who->add_exp(50);
		who->add_potential(1);
		send_user(who,"%c%s",';',"你得到50点经验和1点tiềm năng");
	}	
}