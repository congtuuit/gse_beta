...
void do_look( object who )
{
	...
	if ( (who->get_level() >= 45 &&!who->get_legend(TASK_12, 19)) || (who->get_legend(TASK_12, 19)&&!who->get_legend(TASK_12, 21)) )
	{
		tmp = get_name() + "：\n    "+"/quest/word"->get_normal_word(this_object()) + "\n"+
			sprintf("\n"ESC"修理和鉴定\ntalk %x# welcome.99\n",getoid(this_object()))+
               		sprintf(ESC HIY "Khóc ở nước Trịnh\ntalk %x# welcome.1\n",getoid(this_object())) ;	
	}
	if ( stringp(tmp) )
	{
		tmp += ESC"Rời khỏi.";
...
void do_welcome2( object me, string arg )
{
	...
	case 2:
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,7,"Khóc ở nước Trịnh" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Khóc ở nước Trịnh");
        	}
        	break;       
        case 3:
			...
			who->add_cash(4600);
			send_user(who,"%c%s",';',"Khóc ở nước Trịnh 经验 5200 tiềm năng 190 Ngân lượng 4600");
