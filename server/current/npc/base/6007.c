...
void do_look( object who )
{
	...
	if ( ( who->get_level() >= 35 && !who->get_legend(TASK_09,16) ) || ( who->get_legend(TASK_09,16) && !who->get_legend(TASK_09,18) ) )
	{
		tmp = get_name() + "：\n    "+"/quest/word"->get_normal_word(me) + "\n"+
			sprintf("\n"ESC"修理和鉴定\ntalk %x# welcome.99\n",getoid(me))+
               		sprintf(ESC HIY "Cháy rừng trên đường núi\ntalk %x# welcome.1\n",getoid(me)) ;	
	}
...
void do_welcome2( object me, string arg )
{
	...
	case 2:
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_CHUGUO,28,"Cháy rừng trên đường núi" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Cháy rừng trên đường núi");
        	}
        	break;
        case 3:
			...
			who->add_cash(2000);
			send_user(who,"%c%s",';',sprintf("Cháy rừng trên đường núi 经验 2800 tiềm năng 110 Ngân lượng 2000 %s 1",tmp->get_name()));
