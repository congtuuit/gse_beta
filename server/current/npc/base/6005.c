...
void do_welcome2( object me, string arg )
{
	...
	case 2:
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_CHUGUO,16,"Đầm lầy" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Đầm lầy");
        	}
        	break;
        case 3:
			...
			who->add_cash(1800);
			send_user(who,"%c%s",';',sprintf("Đầm lầy 经验 2400 tiềm năng 80 Ngân lượng 1800 %s 1",tmp->get_name()));
	...
	case 5:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_CHUGUO,"Sở Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_CHUGUO,26,"Máu ở làng" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Máu ở làng");
        	}
        	break;
        case 6:
			...
			who->add_cash(2200);
			send_user(who,"%c%s",';',sprintf("Máu ở làng 经验 2600 tiềm năng 90 Ngân lượng 2200 土制炸弹图纸 1 %s 1",tmp->get_name()));
