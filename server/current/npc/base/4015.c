...
void do_look( object who )
{
	...
	tmp = "";
        ...
        {
        	tmp = sprintf(ESC HIY "Tên cướp kiêu ngạo\ntalk %x# welcome.1\n",id);
        }
        if (who->get_level()>=25&&(!who->get_legend(TASK_06, 13)||!who->get_legend(TASK_06, 15)))
        {
        	tmp += sprintf(ESC HIY "Huyền thoại về hươu khổng lồ\ntalk %x# welcome.4\n",id);
        }
...
void do_welcome2( object me, string arg )
{
        ...
        case 2:
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_WEIGUO,20,"Tên cướp kiêu ngạo" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Tên cướp kiêu ngạo");
        	}
        	break;
        case 3:
			...
			who->add_cash(1500);
			send_user(who,"%c%s",';',"Tên cướp kiêu ngạo 经验 1600 tiềm năng 70 Ngân lượng 1500 细麻袍图纸 1");
        ...
        case 5:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_WEIGUO,"Ngụy Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_WEIGUO,23,"Huyền thoại về hươu khổng lồ" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Huyền thoại về hươu khổng lồ");
        	}
        	break;
        case 6:
        	if ( who->get_legend(TASK_06,14) && !who->get_legend(TASK_06,15) )
        	{
        		if (TASK_LEGEND_D->check_task_item(who,"玉Kiến",1) != 1 )
        			return;
			...
			who->add_cash(1500);
			send_user(who,"%c%s",';',"Huyền thoại về hươu khổng lồ 经验 1800 tiềm năng 80 Ngân lượng 1500 金项链图纸 1");
