...
//        tmp += sprintf(ESC"换取食物\ntalk %x# exchange\n",id);
	...
	{
		tmp += sprintf(ESC HIY "Huyền thoại dòng sông\ntalk %x# welcome.1\n",getoid(me));
	}
        tmp += ESC"Rời khỏi.\nCLOSE\n";
...
void do_welcome2( object me, string arg )
{
	...
	case 2:
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,3,"Huyền thoại dòng sông" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Huyền thoại dòng sông");
        	}
        	break;       
        case 3:
			...
			who->add_cash(4000);
			send_user(who,"%c%s",';',"Huyền thoại dòng sông 经验 4700 tiềm năng 180 Ngân lượng 4000");
