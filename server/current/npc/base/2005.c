...
void do_welcome2( object me, string arg )
{
        ...
        case 1:
			...
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    我听说在无定河的将军冢曾经出现过一件神奇的Quần áo màu nhiệm。要是我的店能有这样一件衣服那该多好啊！你能帮我寻回来吗？\n"ESC"Nhận nhiệm vụ\ntalk %x# welcome.2\n"ESC"Rời khỏi.", me->get_name(),getoid(me)));
        ...
        case 2:
			...
			TASK_LEGEND_D->remove_task_item(who, "Quần áo màu nhiệm");                   	
        		who->set_legend(TASK_04,15);
			who->delete_save("sqcaiyi");
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_HANGUO,"Hàn Quốc" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_HANGUO,7,"Nhiệm vụ - Quần áo màu nhiệm" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Quần áo màu nhiệm");
        	}
        	break;
        case 3:
        	if ( !who->get_legend(TASK_04,15) || who->get_legend(TASK_04,16) )
        		return ;
        	if ( objectp(item=present("Quần áo màu nhiệm",who,1,MAX_CARRY)) && item->is_legend_item() == 1 )
        ...
        case 4:
        	if ( who->get_legend(TASK_04,15) && !who->get_legend(TASK_04,16) )
        	{
        		if (TASK_LEGEND_D->check_task_item(who,"Quần áo màu nhiệm",1) != 1 )
        			return;
			...
			who->add_cash(1300);
			send_user(who,"%c%s",';',"Nhiệm vụ Quần áo màu nhiệm Kinh nghiệm 1500 tiềm năng 80 Ngân lượng 1300");
