...
//			send_user( who, "%c%c%c%w%c", 0x51, 2, 1,TID_NEWBIE_01,"" );
	...
	case 6:
		...
		send_user( who, "%c%c%w%s", ':',3,me->get_char_picid(), sprintf("%s:\n    拿着你手上的藏宝图一直向南走，到清溪最突出那个岛上好好找找，就在那个草丛里（88，62），找到那里，你就可以挖宝了！当然，这个任务比找到藏宝图的奖励会更多，你是否愿意Thử thách呢？你找到宝物后，去找药店老板，他会给你想要的东西。\n    做完任务后，用alt+w打开人物属性界面，再使用新手信物就可以快速回到新手村哦！\n"ESC"Nhận nhiệm vụ\ntalk %x# welcome.7\n"ESC"Rời khỏi.",me->get_name(),getoid(me)));
		break;
	case 7:
		if (!who->get_legend(TASK_48, 7))
			return ;		
	...
	case 99:
		result = sprintf("%s：\n    不知道去哪里买称心的装备？我告诉你好了。\n"
			...
			"    在"HIR" Triệu Quốc "NOR"可以买到"HIR" Hoa đào源 "NOR"的装备；\n"
