...
//		if ( !who->get_legend(TASK_NEWBIE_01, 10) )
	...
	case 6:
		if( who->get_legend(TASK_40, 24) && !who->get_legend(TASK_40, 25) )
		{
			...
			send_user(who,"%c%s",';',"Đau đầu（1） 经验 150 tiềm năng 20");
	...
	case 7:
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_NEWBIE_01,25,"Đau đầu（2）" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Đau đầu（2）");
        	}
        	break;
        case 8:
			...
			who->add_potential(30);
			who->add_cash(1800);
			send_user(who,"%c%s",';',"Đau đầu（2） 经验 300 tiềm năng 30 Ngân lượng 1800");
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_NEWBIE_01,26,"Đau đầu（3）" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Đau đầu（3）");
	...
	case 10:
		if( !who->get_legend(TASK_49, 21) )
		{
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_NEWBIE_01,"新手村" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_NEWBIE_01,47,"Hãy loại bỏ Bù nhìn" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Hãy loại bỏ Bù nhìn");	
		}
		break;
	case 11:
		if ( who->get_legend(TASK_49, 22) && !who->get_legend(TASK_49, 23) )
		{
			...
			send_user(who,"%c%s",';',"Hãy loại bỏ Bù nhìn 经验150 tiềm năng25 Ngân lượng500 行军散5");
