...
//	if ( !who->get_legend(TASK_NEWBIE_01, 14) )
	...
	{
		result = result + sprintf( ESC +HIY+ "Rắc rối do Kiến gây ra\ntalk %x# welcome.8\n", getoid(me) );
	}
...
//			return;
		if ( who->get_legend(TASK_48, 13) && !who->get_legend(TASK_48, 14) )
		{
			...
			who->add_cash(200);
			send_user(who,"%c%s",';',"飞鸽传书 经验 290 tiềm năng 20 Ngân lượng 200 普通鉴定镜 1");
...
//			return;
	...
	case 9:
		if ( who->get_level() >=5 && !who->get_legend(TASK_49, 15) )
		{
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_NEWBIE_01,"新手村" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_NEWBIE_01,45,"Rắc rối do Kiến gây ra" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Rắc rối do Kiến gây ra");	
		}
		break;
	case 10:
		if ( who->get_legend(TASK_49, 16) && !who->get_legend(TASK_49, 17) )
		{
			...
			send_user(who,"%c%s",';',"Rắc rối do Kiến gây ra 经验 150 tiềm năng 25 Ngân lượng 800");
