...
void do_look( object who )
{
	...
	{
		result = result + sprintf( ESC HIY "Tính khí anh hùng\ntalk %x# welcome.1\n", getoid(me) );
	}	
...
void do_welcome2( object me, string arg )
{
        ...
        case 2:
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_WEIGUO,14,"Tính khí anh hùng" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Tính khí anh hùng");
        	}
        	break;
        case 3:
			...
			who->add_cash(1200);
			send_user(who,"%c%s",';',"Tính khí anh hùng 经验 1100 tiềm năng 70 Ngân lượng 1200 随风行装图纸 1");
