...
void do_look( object who )
{
	...
	if ( ( who->get_level() >= 45 && !who->get_legend(TASK_13,4) ) || ( who->get_legend(TASK_13,4) && !who->get_legend(TASK_13,6) ) )
		result += sprintf(ESC HIY "Truyền thuyết về lụa lạnh\ntalk %x# welcome.1\n",getoid(me));
...
void do_welcome2( object me, string arg )
{
	...
	case 2:
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,12,"Truyền thuyết về lụa lạnh" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Truyền thuyết về lụa lạnh");
        	}
        	break;       
        case 3:
			...
			who->add_cash(5000);
			send_user(who,"%c%s",';',"Truyền thuyết về lụa lạnh 经验 5400 tiềm năng 200 Ngân lượng 5000");
