...
void do_look( object who )
{
	...
	{
		tmp = sprintf(ESC HIY "Huyền thoại bí ẩn\ntalk %x# welcome.1\n",getoid(me));
	}
...
void do_welcome2( object me, string arg )
{
	...
	string *nTmp,tmp;     
        ...
        case 2:
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_WEIGUO,12,"Huyền thoại bí ẩn" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Huyền thoại bí ẩn");
        	}
        	break;
        case 3:
			...
			who->add_cash(1000);
			send_user(who,"%c%s",';',sprintf("Huyền thoại bí ẩn 经验 1300 tiềm năng 70 Ngân lượng 1000 %s 1",tmp->get_name()));
