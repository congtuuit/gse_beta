...
void do_look( object who )
{
	...
	if ( (who->get_level() >=60 && !who->get_legend(TASK_18,7)) || (who->get_legend(TASK_18,7) && !who->get_legend(TASK_18,9)) )
		result += sprintf(ESC HIY "Báo động\ntalk %x# welcome.1\n",getoid(me));
...
void do_welcome2( object me, string arg )
{
	...
	case 2:
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,32,"Báo động" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Báo động");
        	}
        	break;
        case 3:
			...
			who->add_cash(6500);
			send_user(who,"%c%s",';',"Báo động 经验 7000 tiềm năng 260 Ngân lượng 6500 高爆炸弹图纸 1");
