...
void do_look( object who )
{
	string tmp="";
        ...
        send_user( who, "%c%s", ':', sprintf( "%s：\n    听说长白山天池有神兽狻猊出现，国王允诺重赏要得此神兽，我家相公Tiểu Đồ为了我们刚刚出世的孩儿能有机会出山富贵，同着一伙熟识的猎人进山碰运气，但是一轮月圆月缺过去，时至今日也没有任何消息，我日日倚望独望，秋水望穿也不见相公归来，哎，帘卷北风，人比黄花瘦，怎么一个愁字了得，如果他有什么不测…我…我…呜……\n", get_name())
        + tmp +
        ESC"Rời khỏi."
         );
}
...
void do_welcome2( object me, string arg )
{
	...
	case 2:
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,60,"Tiểu Đồ（1）" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Tiểu Đồ（1）");
        	}
        	break;
        case 3:
			...
			who->add_cash(43500);
			send_user(who,"%c%s",';',"Tiểu Đồ（1） 经验 44500 tiềm năng 650 Ngân lượng 43500");
	...
	case 4:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_QIGUO,"齐国" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,61,"Tiểu Đồ（2）" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Tiểu Đồ（2）");
        	}
        	break;
        }
}