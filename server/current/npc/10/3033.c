...
void do_welcome2( object me, string arg )
{
	...
	case 2:
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,20,"Tượng bùn" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Tượng bùn");
        	}
        	break;       
        case 3:
        	if ( who->get_legend(TASK_13,23) && !who->get_legend(TASK_13,24) )
        	{
        		if (TASK_LEGEND_D->check_task_item(who,"Tượng bùn",10) != 1 )
        			return;
			...
			who->add_cash(4600);
			send_user(who,"%c%s",';',"Tượng bùn 经验 5700 tiềm năng 220 Ngân lượng 4600 鹤啸行装图纸 1");
