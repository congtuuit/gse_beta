...
void do_look( object who )
{
	...
	id = getoid(this_object());
		...
		tmp += sprintf(ESC HIY "Chấp nhận yêu cầu\ntalk %x# welcome.1\n",id); 
...
void do_welcome2( object me, string arg )
{
        ...
        case 2:
			...
			who->add_cash(58000);
			send_user(who,"%c%s",';',"打探消息 经验 70000 tiềm năng 880 Ngân lượng 58000");
	...
	case 3:
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_WEIGUO,38,"Bằng chứng sức mạnh" );
        ...
        case 4:
			...
			who->add_cash(60000);
			send_user(who,"%c%s",';',"Bằng chứng sức mạnh 经验 71000 tiềm năng 890 Ngân lượng 60000");
	...
	case 5:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_WEIGUO,"魏国" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_WEIGUO,39,"Chấp nhận yêu cầu" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Chấp nhận yêu cầu");
        	}
        	break;  
        case 6:
			...
			who->add_cash(61000);
			send_user(who,"%c%s",';',"Chấp nhận yêu cầu 经验 72000 tiềm năng 900 Ngân lượng 61000");
	...
	case 7:
			...
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"寻宝历险");
        	}
        	break;  
        case 8:
        	if( who->get_level() >= 25 && who->get_legend(TASK_37,21) && !who->get_legend(TASK_37,22) ) 
        	...
        	if ( who->get_level() >= 25 && who->get_legend(TASK_37,21) && !who->get_legend(TASK_37,22) )
		{
			...
			who->add_cash(1400);
			send_user(who,"%c%s",';',"家庭聚会 经验 1400 tiềm năng 70 Ngân lượng 1400");
