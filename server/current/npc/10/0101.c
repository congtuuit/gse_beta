...
void do_look( object who )
{
	string tmp="";
		...
		tmp += sprintf(ESC HIY "Thông điệp của nhà vua\ntalk %x# welcome.1\n",getoid(this_object())); 
...
void do_welcome2( object me, string arg )
{
        ...
        case 1:
        	if ( who->get_legend(TASK_10,1) && !who->get_legend(TASK_10,2) ) 
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    %s，我早听说过你了，你就是那位消灭山戎族奸细和Gởi quốc tịch的%s！\n    我军本被困在这崇山峻岭中，虽派出多批探子探路，但仍然找不着出路，后找了匹识途的老马带路，才使我军走出了山谷，现在战事已停，我得带领大军速速回国。\n"ESC"Hoàn thành任务\ntalk %x# welcome.2\n"ESC"Rời khỏi.", me->get_name(),who->get_name(),NPC_RANK_D->get_respect_2(who),getoid(me)));
        	break;
        ...
        case 2:
			...
			who->add_cash(4000);
			send_user(who,"%c%s",';',"带信给管仲（3） 经验 4000 tiềm năng 130 Ngân lượng 4000 传风袍图纸 1");
