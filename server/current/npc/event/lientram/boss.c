...
int get_random_picid() {
	int* pic = ({ 8287, 8286,8267,9012,7739,8285, });
	int index = random(sizeof(pic));
	return pic[index];
}
...
void create()
{
	...
	set_char_picid(get_random_picid());
	set_level(100);
	set_max_hp(10000);
	set_max_mp(10000);
	set_ap(1000);
...
void drop_items(object me, object who)
{
	...
	if (rate < rate1)
	{
		...
		{
			nTmp = WEAPON_FILE->get_family_equip(nFamily[random(7)], 80, level, HEAD_TYPE + random(6));
			if (sizeof(nTmp))
			{
	...
	{
		nTmp = WEAPON_FILE->get_family_equip(nFamily[random(7)], 80, 100, HEAD_TYPE + random(6));
		if (sizeof(nTmp))
		{
			item = new(nTmp[random(sizeof(nTmp))]);
			if (!item)
