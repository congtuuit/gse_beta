...
int set_fight_status(int type, int level)
{
	...
	case 1:  	//低物防高敏型
		...
		add_max_hp(get_max_hp() * 500 / 100);	//最大气血
...
void drop_items(object me, object who)
{
	...
	if (map = get_map_object(get_z(who))) {
		map_level = map->get("map_grade");

		if (map_level && map_level >= 35) {
			if (random(100) <= (map_level + 35)) {
				if (p = get_valid_xy(z, x, y, IS_ITEM_BLOCK))
				{
					item = new ("/item/mocnap/item/ManhLongChau_2");
					TEAM_D->drop_item(item, who);
					item->set_user_id(owner);
					item->add_to_scene(z, p / 1000, p % 1000);
					item->set("winner", who);
		...
		if (map_level && map_level >= 29) {
			for (i = 0; i < 3; i++) {
				if (p = get_valid_xy(z, x, y, IS_ITEM_BLOCK))
				{
					item = new ("/npc/event/tower/item/manhghephuyetlong");
					TEAM_D->drop_item(item, who);
					item->set_user_id(owner);
					item->add_to_scene(z, p / 1000, p % 1000);
					item->set("winner", who);
	...
	if (rate <= map_grade) {
		...
		{
		case 1:
			item105 = new (sprintf("item/trangbi110/dhn/%d", random(4)));
			break;
		case 2:
			item105 = new (sprintf("item/trangbi110/ts/%d", random(4)));
			break;
		case 3:
			item105 = new (sprintf("item/trangbi110/cvq/%d", random(4)));
			break;
		case 4:
			item105 = new (sprintf("item/trangbi110/dm/%d", random(4)));
			break;
		case 5:
			item105 = new (sprintf("item/trangbi110/ms/%d", random(4)));
			break;
		case 6:
			item105 = new (sprintf("item/trangbi110/cl/%d", random(4)));
			break;
		case 7:
			item105 = new (sprintf("item/trangbi110/vmc/%d", random(4)));
			break;
		}
		...
		if (item105 && who) {
			if (p = get_valid_xy(z, x, y, IS_ITEM_BLOCK))
			{
				TEAM_D->drop_item(item105, who);
				item105->set_user_id(owner);
				item105->add_to_scene(z, p / 1000, p % 1000);
				item105->set("winner", who);
				item105->set("time", time());
			}
		}
		/*else {
			log_file("thap105.txt",sprintf("%s Lỗi item %s\n",short_time(), item105->get_name()));
		}*/
		
	}

	if (random(100) < map_grade + 10) {
		if (p = get_valid_xy(z, x, y, IS_ITEM_BLOCK))
		{
			item = new(sprintf("/item/product/%d", 64748 + random(6)));
			if (item) {
				TEAM_D->drop_item(item, who);
				item->set_user_id(owner);
				item->add_to_scene(z, p / 1000, p % 1000);
