...
//	}        
		...
		for (i=0;i<sizeof(team);i++)
		{
			send_user(team[i],"%c%c%w%s",':',3,this_object()->get_char_picid(),sprintf("Võ Lâm Bại Hoại Hoàng Kim：\n   Muốn vào không ? \n"ESC"Tiến vào\ntalk %x# visit.4\n",id,id));
		}
//		me->do_visit(2);
        	break;
        case 3:        	
...
//				for(i=0;i<sizeof(nxy);i++)
				...
				map->set("copy_npc_amount", 50);

				if (map->get("copy_npc_die") < 1) {
					for (i = 0; i < 10; i++)
					{
						npc = new("npc/party/scum_fight2");
						npc->set("team_id", tId);
						npc->set_level(level);
...
void scum_die(object map,object who)
{
	...
	who->add_to_scene(get_z(owner), get_x(owner)+random(3), get_y(owner)+random(3), 3);
        send_user(who,"%c%s",'!',"Ngươi bị Võ Lâm Bại Hoại Hoàng Kim đánh trọng thương");
}