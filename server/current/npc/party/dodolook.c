...
void do_welcome_call(object me, string arg)
{
	...
	case 10101:
		...
		p = item->move(who, -1);
		send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s：\n    恭喜你获得了%s。\n" ESC "Rời khỏi.", me->get_name(), item->get_name()));
		item->add_to_user(p);

		break;
	case 102:
		send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s：\n    拥有缘分卡的用户一定要先领取缘分宠物再进行缘分任务～！不然你将会失去缘分对对碰的抽奖机会哦～！缘分任务只为有缘人Thiết lập ，领取缘分任务玩家要求必须是男女双方各一人，双方要处于组队状态。并且，拥有缘分宠物的用户必须和同样拥有缘分宠物的用户一起组队任务才可以噢。当你们Hoàn thành任务后来我这里交还任务，我会送给你们双方一人一套缘分套装哦！就算任务失败也不用担心，可以取消任务任务重新再来，不过中间要间隔一个小时哦！\n" ESC "Rời khỏi.", me->get_name(), ));
		break;
	case 103:
		if (!iStart1 || time() < iStart1)
		{
	...
	case 105:
		...
		send_user(member, "%c%s", '!', "Nhận nhiệm vụ  消灭噬情魔");
	...
	case 107:
		...
		member = nTeam[0];
		if (!inside_screen_2(who, member) || member->get_number() != who->get_save_2("dodolook_task.member") || who->get_number() != member->get_save_2("dodolook_task.member"))
		{
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s：\n    怎么就你一人来呢？你的有缘人呢？还是把你的有缘人带来，再找我领取奖励吧！\n" ESC "Rời khỏi.", me->get_name(), ));
			return;
		}
		...
		send_user(CHAT_D->get_chat(), "%c%s", 0xA3, sprintf(HIY "%s和%s成功Hoàn thành了缘分测试，让我们祝福他们成为真心相待的有缘人！", who->get_name(), member->get_name()));
		//		if ( who->get_save("dodolook_yuanfen") && member->get_save("dodolook_yuanfen") ) //有缘分卡的记录下来
		log_file("dodolook_task.txt", sprintf("%s %d %s(%d) %s(%d) Hoàn thành任务\n", short_time(), "sys/main/main"->get_host(), who->get_name(), who->get_number(), member->get_name(), member->get_number()));
		break;
	case 108:
...
int check_dodolook(object me, object who)
{
	...
	{
		send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s：\n    由于你们队伍中%s没有Hoàn thành上次缘分任务，所以该任务不能领取。\n" ESC "Rời khỏi.", me->get_name(), who->get_name()));
		return 0;
	}
	if (member->get_save_2("dodolook_task.npcid"))
	{
		send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s：\n    由于你们队伍中%s没有Hoàn thành上次缘分任务，所以该任务不能领取。\n" ESC "Rời khỏi.", me->get_name(), member->get_name()));
		return 0;
	}
