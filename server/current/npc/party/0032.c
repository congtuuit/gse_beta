...
void do_look_call(object me,object who)
{
	...
	if ( iTime > iStart1 && iTime < iFinish1 )
	{
		if ( who->get_save_2("51cuanye.time") )	
			send_user(who,"%c%c%w%s",':',3,me->get_char_picid(),sprintf("%s：\n    五一七天乐，战国乐翻天！\n    五一期间我们为玩家准备了丰富的线上活动，参与活动领取“Huân chương lao động thời Chiến quốc”，就有机会兑换金色装备、特殊称谓等珍贵奖品！\n    三大活动将伴您共度黄金周，活动详细信息请查询http:/""/zg.mop.com活动专区。\n"ESC"天道酬勤\ntalk %x# welcome.2\n"ESC"Huân chương lao động thời Chiến quốc兑奖\ntalk %x# welcome.5",me->get_name(),id,id));
		else
...
void do_welcome2( object me, string arg )
{
	...
	if (MAIN_D->get_host_type()==2)
		...
		if ( who->get_level() >= 30 )
       		{
			send_user(who,"%c%c%w%s",':',3,me->get_char_picid(),sprintf("%s：\n    你的等级似乎不太适合来做这件事情噢～不过你可以考虑帮助低级玩家来Hoàn thành这个任务！\n"ESC"Rời khỏi.",me->get_name(),));
			return ;
		}
		if ( who->get_save_2("51cuanye.count") == 0 )
			send_user(who,"%c%c%w%s",':',3,me->get_char_picid(),sprintf("%s：\n    30级以下的玩家注意了！这个任务可是特别为你们设计的哦～不过你们想要顺利到达传说中的东海岛可离不开高级玩家的帮助，还在犹豫什么，赶紧组上他们来接任务吧！活动详情请查询http:/""/zg.mop.com活动专区。\n    拿好这张“Giấy chứng nhận”，去交给东海岛东北方的Thanh tra vượt biên，你就能拿到他给你的奖励了！\n    这个任务每天可重复领取5次哦～\n"ESC"Nhận nhiệm vụ\ntalk %x# welcome.1\n"ESC"Rời khỏi.",me->get_name(),id,));
		else
			send_user(who,"%c%c%w%s",':',3,me->get_char_picid(),sprintf("%s：\n    30级以下的玩家注意了！这个任务可是特别为你们设计的哦～不过你们想要顺利到达传说中的东海岛可离不开高级玩家的帮助，还在犹豫什么，赶紧组上他们来接任务吧！活动详情请查询http:/""/zg.mop.com活动专区。\n    拿好这张“Giấy chứng nhận”，去交给东海岛东北方的Thanh tra vượt biên，你就能拿到他给你的奖励了！\n    这个任务你还可以做%d次哦～\n"ESC"Nhận nhiệm vụ\ntalk %x# welcome.1\n"ESC"Rời khỏi.",me->get_name(),5-who->get_save_2("51cuanye.count"),id,));
        ...
        case 2:
			...
			if ( who->get_save_2("51tiandao.count") == 0 )
				send_user(who,"%c%c%w%s",':',3,me->get_char_picid(),sprintf("%s：\n    一分耕耘，一分收获，只要辛苦劳动就会收获丰硕的果实，天道酬勤就是给勤劳的你专门设计的任务，Hoàn thành任务就可以获得神秘的“Huân chương lao động thời Chiến quốc”。\n    任务很简单，只要你拿着这块Đá Thử Nghiệm，去杀满100只高于自己等级5级之内的怪，你就可以回来找我领取奖励了。这个任务每天可重复领取5次哦～\n    活动详细信息请查询http:/""/zg.mop.com活动专区。\n"ESC"Nhận nhiệm vụ\ntalk %x# welcome.3\n"ESC"Rời khỏi.",me->get_name(),id,));
			else
				send_user(who,"%c%c%w%s",':',3,me->get_char_picid(),sprintf("%s：\n    一分耕耘，一分收获，只要辛苦劳动就会收获丰硕的果实，天道酬勤就是给勤劳的你专门设计的任务，Hoàn thành任务就可以获得神秘的“Huân chương lao động thời Chiến quốc”。\n    任务很简单，只要你拿着这块Đá Thử Nghiệm，去杀满100只高于自己等级5级之内的怪，你就可以回来找我领取奖励了。这个任务你还可以做%d次哦～\n    活动详细信息请查询http:/""/zg.mop.com活动专区。\n"ESC"Nhận nhiệm vụ\ntalk %x# welcome.3\n"ESC"Rời khỏi.",me->get_name(),5-who->get_save_2("51tiandao.count"),id,));
			return ;
		}
		else if ( who->get_save_2("51tiandao.amount") >= 100 )
			send_user(who,"%c%c%w%s",':',3,me->get_char_picid(),sprintf("%s：\n    不赖不赖！这三个Huân chương lao động thời Chiến quốc归你了！\n"ESC"Hoàn thành任务\ntalk %x# welcome.4\n"ESC"Rời khỏi.",me->get_name(),id,));
        	else
        ...
        case 4:
        	if ( who->get_save_2("51tiandao.amount") < 100 )
        		return ;
		if ( !objectp(item = present("Đá Thử Nghiệm", who, 1, MAX_CARRY)) )
		{
			send_user(who,"%c%c%w%s",':',3,me->get_char_picid(),sprintf("%s：\n    你的Đá Thử Nghiệm呢？\n"ESC"Rời khỏi.",me->get_name(),));
			return ;
		}     
		...
		item->add_to_user(p);
        	...
        	break;
        case 5:
		send_user(who,"%c%c%w%s",':',3,me->get_char_picid(),sprintf("%s：\n    不同数量的Huân chương lao động thời Chiến quốc可以换取不同的奖品，数量越多，奖品越珍贵。\n"ESC"1枚奖章兑换奖品\ntalk %x# welcome.1001\n"ESC"5枚奖章兑换奖品\ntalk %x# welcome.1005\n"ESC"20枚奖章兑换奖品\ntalk %x# welcome.1020\n"ESC"50枚奖章兑换奖品\ntalk %x# welcome.1050\n"ESC"100枚奖章兑换奖品\ntalk %x# welcome.1100\n"ESC"300枚奖章兑换奖品\ntalk %x# welcome.1300\n"ESC"30枚奖章兑换一级称号\ntalk %x# welcome.2030\n"ESC"60枚奖章兑换二级称号\ntalk %x# welcome.2060\n"ESC"120枚奖章兑换三级称号\ntalk %x# welcome.2120\n"ESC"Rời khỏi.",me->get_name(),id,id,id,id,id,id,id,id,id,));
        	break;
        case 1001:
        case 1005:
        case 1020:
		...
		if ( USER_INVENTORY_D->count_empty_carry(who) < 2 )
		{
        	...
        	count = flag - 1000;
		send_user(who,"%c%c%w%s",':',3,me->get_char_picid(),sprintf("%s：\n    你确定要用%d枚Huân chương lao động thời Chiến quốc来兑换奖品吗？\n"ESC"确定\ntalk %x# welcome.%d\n"ESC"Rời khỏi.",me->get_name(),count,id, 2000+flag));
        ...
        case 2120:
        	...
        	if ( count == 30 )
        		...
        		tmp = "爆强的战国劳模";
		send_user(who,"%c%c%w%s",':',3,me->get_char_picid(),sprintf("%s：\n    你确定要用%d枚Huân chương lao động thời Chiến quốc来兑换"HIR"“%s”"NOR "的称号吗？\n"ESC"确定\ntalk %x# welcome.%d\n"ESC"Rời khỏi.",me->get_name(),count,tmp,id, 2000+flag));
        	break;
        case 4030:
        case 4060:
        case 4120:
        	...
        	for(i=0,size=sizeof(inv);i<size;i++)
		{
			if ( !objectp(inv[i]) )
				continue;
			if ( inv[i]->get_name() != "Huân chương lao động thời Chiến quốc" )
				continue;
			amount += inv[i]->get_amount();		
		}
		if ( amount < count )
		{
			send_user(who,"%c%c%w%s",':',3,me->get_char_picid(),sprintf("%s：\n    您身上的Huân chương lao động thời Chiến quốc数量不足%d个，建议您多做几次任务后再来找我吧！\n"ESC"Rời khỏi.",me->get_name(),count));
			return ;	
		}
		for(i=0,size=sizeof(inv);i<size;i++)
		{
		...
		if ( count == 1 )
		{
			...
			p = item->move(who,-1);
			item->add_to_user(p);
			send_user(who,"%c%s",';',sprintf("您用%d枚Huân chương lao động thời Chiến quốc兑换了%s",count,item->get_name()));
		}
		else if ( count == 5 )
		{
			if ( rate < 2500 )
