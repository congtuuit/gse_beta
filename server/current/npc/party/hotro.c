...
void do_welcome2( object me, string arg )
{
	...
	if (MAIN_D->get_host_type()==2)
		...
		if ( !objectp(item = present("Giấy chứng nhận", who, 1, MAX_CARRY)) )
		{
			send_user(who,"%c%c%w%s",':',3,me->get_char_picid(),sprintf("%s：\n    你的Giấy chứng nhận呢？\n"ESC"Rời khỏi.",me->get_name(),));
			return ;
		}
		...
		send_user(who,"%c%c%w%s",':',3,me->get_char_picid(),sprintf("%s：\n    五一七天乐，战国乐翻天！\n    没想到你这么快就找到我了，我还以为，我要再等上一万年！\n    你 %d 分钟找到我，将得到%d个Huân chương lao động thời Chiến quốc。\n"ESC"极限穿越\ntalk %x# welcome.2\n"ESC"Rời khỏi.",me->get_name(),i,p,id,));
		break;
	case 2:
		iTime = who->get_save_2("51cuanye.time");
		...
		for(i=0;i<size;i++)
		{
			...
			item->add_to_user(p);
			send_user(nTeam[i],"%c%s",';',sprintf("你Hoàn thành任务，得到%d个Huân chương lao động thời Chiến quốc",amount));
