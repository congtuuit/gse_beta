...
void do_look_2( object who, object me )
{
	...
	{
		national2 = sprintf(ESC HIY"购买国庆疯狂Túi quà lớn\ntalk %x# bonus.43\n",id);
		national = sprintf(ESC HIY"领取猫扑大杂烩赠送Túi quà lớn\ntalk %x# bonus.34\n"
				   ESC HIY"领取校内网网赠送Túi quà lớn\ntalk %x# bonus.34\n"
				   ESC HIY"领取官方赠送新手礼包\ntalk %x# bonus.34\n",id,id,id);
	...
	switch(flag) {
		...
		case 2:
			if(level>0) {
				...
				if(level >1) {
					send_user( who, "%c%s", '!', sprintf(HIY"您的Ngân lượng增加了 %s。",active_bonus[2]));
				}				
				who->delete_save("guess_true_tiger");
...
//		USER_D->user_channel(result);	
		...
		send_user(who,"%c%c%w%s",':',3,me->get_char_picid(),sprintf("%s：\n    到十级了，对武器装备都会有不同的要求了，我决定送你4颗可以给武器装备附加属性的"HIR "晶石"NOR "，如果你按照从低到高的等级顺序，依次将具有相同属性的2颗晶石附加在武器或者防具上，你会看到它们的功能增强不少哦～记住，这些晶石"HIR "只对20级以下"NOR "的装备生效。\n    除了这些晶石外，这位可爱的双鱼小姐在这14天内都会陪伴着你哦。\n"ESC"领取\ntalk %x# bonus.35\n",me->get_name(),id));
        	break;
        case 35:
		if( iTime<iFinish_n && iTime>iStart_n && !who->get_save("wg_national_new") && who->get_level()<30 && who->get_level()>=10 )
		{
...
//        	break;
        case 38:
		send_user(who,"%c%c%w%s",':',3,me->get_char_picid(),sprintf("%s：\n    Túi quà lớn包含物品：一个干将宝石、一张年兽卡、一个鲁班宝石、一条紫金钥匙、一张瞒天过海符、一张保底符、一个战国令、一个芝麻开门、一块济世宝玉、一把尚方宝剑、一个人参王、一个锦囊和一把魔力锤，同时附送一个内含200元宝的大红包。\n    要领取这个Túi quà lớn，我会收取您390元宝，你Xác nhận需要吗？\n"
			ESC"果然物超所值，我要马上购买\ntalk %x# bonus.42\n"
			ESC"我先考虑一下",me->get_name(),id));
        	break;
        case 39:
		send_user(who,"%c%c%w%s",':',3,me->get_char_picid(),sprintf("%s：\n    这个礼包所有物品都不绑定，可以送给朋友，让你的朋友可以和你一起欢度国庆。同时也可以Giao dịch和摆摊出售哦。因为是很珍贵的礼物，所以只会在国庆期间内出售。\n"
			ESC"我明白了",me->get_name()));
        	break;
        case 40:
		send_user(who,"%c%c%w%s",':',3,me->get_char_picid(),sprintf("%s：\n    Túi quà lớn包含物品：一个干将宝石、一张年兽卡、一个鲁班宝石、一条紫金钥匙、一张瞒天过海符、一张保底符、一个战国令、一个芝麻开门、一块济世宝玉、一把尚方宝剑、一个人参王、一个锦囊和一把魔力锤，一个内含200个元宝的红包。\n"
		...
		db_user_pay( who, sprintf( "%s:80\n"
        	...
        	break;
	case 43:
		ssss=":\n    秋色渐浓，喜庆渐现。中秋正刚刚在甜蜜的滋味中度过了，国庆又在热闹的气氛里来临了。在此普天同庆的日子里，大话战国的世界里更是热闹非凡。为什么呢？就因为有我手中这个国庆疯狂Túi quà lớn嘛！特别的日子特惠的价格，每套Túi quà lớn仅售390元宝，绝对物超所值。喂喂喂，不要挤不要抢，排队一个一个来哦。\n";
		national2 = sprintf(ESC HIY"手快有手慢无，马上点击进入购买\ntalk %x# bonus.38\n"
				    ESC"我想了解更详细的情况\ntalk %x# bonus.39\n"
				    ESC"我想看一下Túi quà lớn里有什么东东哦\ntalk %x# bonus.40\n"
				    ESC"等我充值了再说\ntalk %x# bonus.41\n",id,id,id,id);
