...
void do_look_2(object who, object me)
{
	...
	//			ESC"领取神兽\ntalk %x# bonus.1017\n"
				...
				ESC+HIY"补发七夕戒指\ntalk %x# bonus.31\n"
				ESC+HIY"修复锻造异常的装备\ntalk %x# bonus.30\n"
				ESC"Rời khỏi.", id, id, id, id, id, id, id, id, id, id, id, id, ) );
		}*/
}
...
void do_bonus_2(object me, object who, string arg)
{
	...
	if (status == 0)
	{
		...
		case 38:
			if (me->get_level() < 10)
			{
				send_user(me, "%c%c%w%s", ':', 3, who->get_char_picid(), sprintf("%s:\n    您不符合领取条件！！！" ESC "Rời khỏi.", who->get_name()));
				return;
			}
...
void give_bonus(object me, int type)
{
	...
	case 101420:
		if (!item)
			item = new ("item/mop/9002"); // 金Đá Tinh Linh5
