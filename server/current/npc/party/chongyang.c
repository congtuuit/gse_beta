...
void do_bonus_2(object me , string arg)
{
	...
	mixed *mxTime;  
	string *treasure=({"黄菊","茱萸","重阳糕","Cây cúc酒",});
        who = this_player();
        flag = to_int(arg);
        who->set_time( "talk",  0);
        id = getoid(me);
		...
		if( !objectp( item_q4 = present("Cây cúc酒", who, 1, MAX_CARRY*4) ) )
		{
			send_user(who,"%c%s",';',"你的重阳四宝数量不足！");
			return;	
		}
		...
		if ( check_item_amount(who,"Cây cúc酒") < 3 )
		{
			send_user(who,"%c%s",';',"你的重阳四宝数量不足！");
			return;	
		}
		...
		reduce_item_amount(who,"黄菊",3);reduce_item_amount(who,"茱萸",3);reduce_item_amount(who,"重阳糕",3);reduce_item_amount(who,"Cây cúc酒",3);
		...
		item = new ("item/04/0464");//重阳Túi quà lớn
		result = result + sprintf(HIR"%s。",item->get_name());
		if ( item )
		{
			p = item->move(who,-1);
