...
//				else
				...
				send_user(who,"%c%c%w%s",':',3,me->get_char_picid(),sprintf("%s：\n    这是我们的一点心意，你们就收下吧。\n"
				ESC"Rời khỏi.",me->get_name()));
				send_user(who,"%c%s",';',sprintf(HIR"你Hoàn thành了七夕天赐奇缘任务，获得Ngân lượng50000，经验50000，情比金坚戒 "));
			...
			send_user( CHAT_D->get_chat(), "%c%s", 0xA3, sprintf(HIY"恭喜%sHoàn thành七夕天赐奇缘任务，使牛郎织女能够幸福地生活在一起。",who->get_name()) );
		}
        	break;
        }
}
