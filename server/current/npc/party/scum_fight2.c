...
#include <ansi.h>
inherit BADMAN;
#define SCUM_NPC "npc/party/scum_fight2"
...
void create()
{
        set_name("Võ Lâm Bại Hoại Hoàng Kim");
...
{
        object item, leader,map,*member,npc1, tuingoc;
...
//	map = TEAM_D->get_copy_scene(tId,COPY_SCUM);
	...
	if ( map->get("copy_npc_die") < map->get("copy_npc_amount") )
	{
		for (i = 0; i < 3; i++)
		{
			if (p = get_valid_xy(zz, xx, yy, IS_ITEM_BLOCK))
			{
				item = new ("/item/std/0001");
				item->set_value(2000);
				TEAM_D->drop_item(item, who);
				item->set_user_id(owner);
				item->add_to_scene(zz, p / 1000, p % 1000);
				item->set("winner", who);
				item->set("time", time());
			}
		}
		...
		if (random(100) < 8) {
			if (p = get_valid_xy(zz, xx, yy, IS_ITEM_BLOCK))
			{
				tuingoc = new("/item/TuiLinhThach/NgauNhien/1");
				TEAM_D->drop_item(tuingoc, who);
				tuingoc->set_user_id(owner);
				tuingoc->add_to_scene(zz, p / 1000, p % 1000);
				tuingoc->set("winner", who);
				tuingoc->set("time", time());
			}
		}

		generate_copy_npc(tId,map->get_id(),map->get("copy_level"));

		//if (map->get("copy_npc_die")%1==0)
		//{
			...
			for(i=0;i<size;i++)
			{
				if ( get_z(member[i]) != z )
					continue;
				//"sys/sys/test_db"->add_yuanbao(member[i],10);
				send_user( member[i], "%c%s", ';', sprintf(HIY"Còn %d tên Võ Lâm Bại Hoại Hoàng Kim" , map->get("copy_npc_amount") - map->get("copy_npc_die")));
			}
		//}
		...
		for (i = 0; i < size; i++)
		{
			if (get_z(member[i]) != z)
				continue;
			send_user(member[i], "%c%s", ';', HIY"Võ lâm bại hoại đã bị đánh đuổi thành công!!");
		}
