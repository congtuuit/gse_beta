...
void do_look(object who)
{
	...
	if ( host_type == 2)
		...
		send_user(who,"%c%c%w%s",':',3,this_object()->get_char_picid(),sprintf("Như Ý Tiên Tử：\n    您好！我是新手使者。欢迎您来到大话战国发展，为了鼓励您的事业进取心，姐姐决定送给您一个新手Túi quà lớn。\n"ESC"领取如意礼包\ntalk %x# bonus\n"ESC"领取新人王礼包\ntalk %x# welcome.1\n"ESC"竞选技能大师称号\ntalk %x# welcome.2\n"ESC"领取第一帮礼包\ntalk %x# welcome.3\n"ESC"Rời khỏi.",id,id,id,id));
}
...
void do_bonus()
{
	...
	item->add_to_user(p);
	send_user(who,"%c%c%w%s",':',3,me->get_char_picid(),sprintf("Như Ý Tiên Tử：\n    丰富多彩的战国生活等着您呢，尽情去体验吧！等您到了%d级之后，姐姐会再送您一个Túi quà lớn的。",(i+2)*10));
	...
	if ( host_type == 2 || host_type == 4 )	//台湾&猫扑
	        ...
	        switch(flag)
		{
	        case 6:
	        	...
	        	if ( name == "Thục Sơn" )
	        	{
	     	        	send_user(who,"%c%c%w%s",':',3,this_object()->get_char_picid(),sprintf("Như Ý Tiên Tử：\n    你希望得到什么武器？\n"ESC"刀\ntalk %x# welcome.7\n"ESC"剑\ntalk %x# welcome.8\n"ESC"Rời khỏi.",getoid(me),getoid(me)));
	        		return ;	
	        	}
	        	if ( name == "Cấm Vệ Quân" )
	        	{
	        	...
	        	send_user(who,"%c%s",'!',"恭喜Bạn nhận được 了如意仙子的礼物。");
	        	break;    			
		case 7:
		case 8:
		case 9:
...
//		send_user(who,"%c%c%w%s",':',3,me->get_char_picid(),"Như Ý Tiên Tử：\n    不要着急，活动的时间是从内测开始的9月18日至9月24日截止的。届时我们会根据你的技能等级和人物等级作出一个综合实力的评比。待活动时间结束，你觉得自己能成为等级最高的新人王再来找我吧～！");
		...
		{
			send_user(who,"%c%c%w%s",':',3,me->get_char_picid(),"Như Ý Tiên Tử：\n    经过这段时间的考验和你自己的努力，你已经成为万人瞩目的新人王，作为对你的鼓励，这是给你的Túi quà lớn～！");
		...
		who->add_cash(10000);
		send_user(who,"%c%c%w%s",':',3,me->get_char_picid(),"Như Ý Tiên Tử：\n    经过这段时间的考验和你自己的努力，你已经成为万人瞩目的新人王，作为对你的鼓励，这是给你的Túi quà lớn～！");
		send_user(who,"%c%s",';',sprintf("战国新人王称号 乾坤袋 1 Túi quà lớn 1 %s 1 战国号角 1 10000金",name->get_name()));
...
//		send_user(who,"%c%c%w%s",':',3,me->get_char_picid(),"Như Ý Tiên Tử：\n    不用着急，技能师的评比活动时间是从内测开始的9月18日至10月4日截止的。～！等活动时间结束了，你觉得自己能成为生活技能最高的技能师再来找我吧～！");
		...
		TASK_LEGEND_D->give_item(who,"item/std/6002",1);
		TASK_LEGEND_D->give_item(who,"item/04/0405",1);		
		send_user(who,"%c%c%w%s",':',3,me->get_char_picid(),"Như Ý Tiên Tử：\n    经过这段时间的考验和你自己的努力，你已经成为万人瞩目的技能师，作为对你的鼓励，这是给你的Túi quà lớn～！");
        	break;
        case 3:
			...
			if( sizeof_inventory(who, 1, MAX_CARRY) >= MAX_CARRY -2 )
			{
        		...
        		send_user(who,"%c%c%w%s",':',3,me->get_char_picid(),"Như Ý Tiên Tử：\n    经过这段时间的考验和你自己的努力，你的帮派已经成为万人瞩目的第一帮，作为对你的鼓励，这是给你的Túi quà lớn～！\n"ESC"Rời khỏi.\nCLOSE\n");
			...
			who->add_cash(500000);
			send_user(who, "%c%s", ';', "您获得Túi quà lớn一个、乾坤袋一个以及50万金。");
