...
void do_look_2( object who, object me )
{
	...
	int id;
	id = getoid(me);
	cmd1 = sprintf(ESC"交还Dưa hấu获得报酬\ntalk %x# bonus.2\n",id);
	cmd2 = sprintf(ESC"将捉到的坏动物交给Dưa hấu伯伯。\ntalk %x# bonus.8\n",id);
	send_user( who, "%c%c%w%s", ':', 3, me->get_char_picid(),
		me->get_name()+":\n    今年风调雨顺，农作物和水果都获得了大丰收，这是一件所有人都开心的事。但是我却不怎么开心，因为我种植的Dưa hấu实在太多了，一个人根本没办法采集完，很希望有人能帮我。如果你能帮我把Dưa hấu采集回来的话，那么将获得很不错的报酬哦！\n    不过你要小心，时常会有一些坏动物来抢我的Dưa hấu，如果你见到了，把它们捉起来交给我，我就可以教训它们了！\n    我再给你三个神奇的葫芦，当你抓到坏动物的时候，就可以把它们关在里面交给我。这个神奇葫芦你平时看不见，当你抓到坏动物的时候，它自己就会出来的！\n"
		+cmd1
		+cmd2
		+ESC"Rời khỏi." );
}
...
void do_bonus_2(object me , string arg)
{
	...
	mixed *mxTime;  
		...
		send_user(who,"%c%c%w%s",':',3,me->get_char_picid(),sprintf("%s：\n    谢谢你帮我采集Dưa hấu！请选择你所想要的报酬。\n"
        	...
        	break;
        case 3:
        	if( !objectp( item_q = present("Dưa hấu伯伯的Dưa hấu", who, 1, MAX_CARRY*4) ) )
        	{
        		send_user( who, "%c%s", ';', "很抱歉，你的Dưa hấu数量不足哦，想要获得报酬可是要努力采集的！");
        		return;
        	}  
		...
		item_q->add_amount(-1);
		result2 = sprintf(HIR"%s玩家"NOR"在国庆活动中，用Dưa hấu兑换到了"HIR, who->get_name());
...
//		USER_D->user_channel(result);	
        	...
        	break;
        case 4:
		if ( check_item_amount(who,"Dưa hấu伯伯的Dưa hấu") < 5 )
		{
			send_user(who,"%c%s",';',"很抱歉，你的Dưa hấu数量不足哦，想要获得报酬可是要努力采集的！");
			return;	
		}
		...
		reduce_item_amount(who,"Dưa hấu伯伯的Dưa hấu",5);
		who->add_cash(15000);
		...
		result += NOR"，让我们大家表示祝贺！";
        	break;
        case 5:
		if ( check_item_amount(who,"Dưa hấu伯伯的Dưa hấu") < 10 )
		{
			send_user(who,"%c%s",';',"很抱歉，你的Dưa hấu数量不足哦，想要获得报酬可是要努力采集的！");
			return;	
		}
		...
		reduce_item_amount(who,"Dưa hấu伯伯的Dưa hấu",10);
		who->add_cash(30000);
		...
		send_user(who,"%c%c%w%s",':',3,me->get_char_picid(),sprintf("%s：\n    呵呵，请把你抓到的坏动物交给我吧，你现在已经抓到了%d只坏小猴，%d只坏小猪，%d只坏大熊。作为报酬你可以有如下选择。\n"
			...
			ESC"用2只坏小猪换取报酬\ntalk %x# bonus.10\n"
			ESC"用1只坏大熊换取报酬\ntalk %x# bonus.11\n"
			ESC"Rời khỏi.",me->get_name(),amount01,amount02,amount03,id,id,id));
        	break;
        case 9:
