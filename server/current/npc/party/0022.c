...
void do_welcome_call( object me,string arg )
{
	...
	case 3:
		if ( who->get_birthday() < iStart1 )
		{
        	...
        	if ( who->get_level() < 10 )
        	{
			send_user(who,"%c%c%w%s",':',3,me->get_char_picid(),sprintf("%s：\n    胡说！就你的等级还没Rời khỏi.过新手村吧。我在这里好久了都没见过她，你不要欺骗我了。\n"ESC"承认自己撒谎\ntalk %x# welcome.5\n"ESC"Rời khỏi.",me->get_name(),id,));
        		return ;	
        	}
	...
	case 4:
		...
		break;
	case 5:
		send_user(who,"%c%c%w%s",':',3,me->get_char_picid(),sprintf("%s：\n    没有关系，"HIR"等你到了十级可以Rời khỏi.这里的时候再来找我，那时我将会托付于你一件事！"NOR"也许我们真的有缘！看你衣衫褴褛，十分狼狈，不如我送你点好吃的吧！\n"ESC"接受馈赠\ntalk %x# welcome.7\n"ESC"Rời khỏi.",me->get_name(),id,));
		break;
	...
	case 8:
		if ( time() < iStart1 )
        	...
        	send_user(who,"%c%s",'!',"Nhận nhiệm vụ "HIR"拜访魏昭王");
