...
void do_welcome2( object me, string arg )
{
	...
	case 2:
		...
		send_user(who,"%c%s",';',"牛二报恩 经验 160 tiềm năng 10");
...
//        	else 	if ( who->get_legend(TASK_41, 2) && !who->get_legend(TASK_41, 3) )
	...
	case 6:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_NEWBIE_01,"新手村" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_NEWBIE_01,31,"Niệm chú lên con người" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Niệm chú lên con người");
        	}
        	break;
        case 7:
		if ( who->get_legend(TASK_48, 19) && !who->get_legend(TASK_48, 20))
...
//        	if ( !who->get_legend(TASK_41, 13) )
		if ( who->get_legend(TASK_48, 27) && !who->get_legend(TASK_48, 28))
			send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(), sprintf("%s:\n    别看我是个粗人，可是我还是挺讲究生活的精致的，我喜欢在家里养点花花草草的，这样房子里看起来才有生气，虽然家里种了不少的Cây lan、Măng tây、Cây cúc之类的，但是我最喜欢的还是那百合，象征着纯洁。\n    新手村的当铺老板一直是我最敬佩的人，他家正在装修，所以我想送他一些Hoa huệ来Trang trí nhà cửa，我听说巴丘的持扇草人、赤褐蚁或子母蛙都会有Hoa huệ，你去替我找些来送到他家吧。\n    做完任务后，用alt+w打开人物属性界面，再使用新手信物就可以快速回到新手村哦！\n"ESC"Nhận nhiệm vụ\ntalk %x# welcome.9\n"ESC"Rời khỏi.", me->get_name(),getoid(me)));
...
//        	if ( !who->get_legend(TASK_41,13) )
        ...
        case 10:
			...
			who->add_exp(250);
			who->add_potential(10);
			send_user(who,"%c%s",';',"Trang trí nhà cửa 经验 250 tiềm năng 10");
...
//		if ( who->get_level() >= 5 && !who->get_legend(TASK_41, 4) )
		if ( who->get_legend(TASK_48, 20) && !who->get_legend(TASK_48, 21))
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_NEWBIE_01,"新手村" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_NEWBIE_01,109,"Rất nhiều bệnh khác nhau（1）" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Rất nhiều bệnh khác nhau（1）");
        	}
        	break; 
        case 13:
