...
void do_look(object who)
{
	...
	if ((who->get_level() >= 70 && !who->get_legend(TASK_2_01, 4)) ||
		...
		tmp += sprintf(ESC HIY "Hoàn thành nhiệm vụ\ntalk %x# welcome.19\n", id);
	...
	more_look_menu(who, get_name() + ":\nTa chính là đệ nhất diệu thủ Công Thâu Ban ! ngươi tìm ta có chuyện chi ?\n"
		...
		//ESC "Rèn trang bị bằng Đá Cường Hóa\n" + sprintf("talk %x# welcome.60\n", id) +
		ESC "Rời khỏi\nCLOSE\n");
}
...
{
	//mix2
	send_user(this_player(), "%c%c%c%d", 0x25, 2,10, getoid(this_object()));
}

void do_make_2()
{
	//mix3
	send_user(this_player(), "%c%c%d", 0x25, 3, getoid(this_object()));
}
...
void do_welcome2(object me, string arg)
{
	...
	case 2:
		if (who->get_legend(TASK_NEWBIE_03, 13) && !who->get_legend(TASK_NEWBIE_03, 14))
		{
			...
			send_user(who, "%c%s", ';', "买椟还珠 经验 600 tiềm năng 10 Ngân lượng 1000");
	...
	case 3:
		if (who->get_legend(TASK_NEWBIE_03, 14) && !who->get_legend(TASK_NEWBIE_03, 15))
		{
			...
			send_user(who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"买椟还珠(3)");
		}
		break;
	case 4:
	...
	case 5:
		...
		who->delete_save_2("gongsumutou");
		send_user(who, "%c%s", ';', "买椟还珠 经验 650 tiềm năng 50 Ngân lượng 1000 正气散 3");
	...
	case 6:
		if (who->get_legend(TASK_NEWBIE_03, 16) && !who->get_legend(TASK_NEWBIE_03, 17))
		{
			...
			send_user(who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"买椟还珠(4)");
		}
		break;
	case 7:
	...
	case 8:
		...
		who->delete_save_2("gongsupaozi");
		send_user(who, "%c%s", ';', "买椟还珠 经验 800 tiềm năng 30 Ngân lượng 1500 木盒 1");
		send_user(who, "%c%c%c%w%w%s", 0x51, 2, 2, TID_ZHOUGUO, 6, "买椟还珠(5)");
		send_user(who, "%c%c%c%w%w%s", 0x51, 2, 2, TID_ZHOUGUO, 5, "");
		send_user(who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"买椟还珠(5)");
		break;
	case 9:
		if (who->get_legend(TASK_28, 12) && !who->get_legend(TASK_28, 13))
	...
	case 10:
		if (who->get_legend(TASK_28, 12) && !who->get_legend(TASK_28, 13))
		{
			...
			who->add_cash(22500);
			send_user(who, "%c%s", ';', "木箱的秘密（3） 经验 24000 tiềm năng 435 Ngân lượng 22500");
	...
	case 11:
		if (who->get_legend(TASK_28, 13) && !who->get_legend(TASK_28, 14))
		{
			...
			send_user(who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"木箱的秘密（4）");
		}
		break;
	case 12:
		if (who->get_legend(TASK_28, 15) && !who->get_legend(TASK_28, 16))
		{
			...
			who->add_cash(110000);
			send_user(who, "%c%s", ';', sprintf("木箱的秘密（4） 经验 120000 tiềm năng 1500 Ngân lượng 110000 %s 1", tmp->get_name()));
	...
	case 14:
		if (who->get_legend(TASK_40, 1) && !who->get_legend(TASK_40, 2))
		{
			...
			who->add_cash(35000);
			send_user(who, "%c%s", ';', "四面楚歌（3） 经验 36000 tiềm năng 600 Ngân lượng 35000");
	...
	case 15:
		if (who->get_legend(TASK_40, 2) && !who->get_legend(TASK_39, 28))
		{
			...
			send_user(who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"四面楚歌（4）");
		}
		break;
	case 16:
		if (who->get_legend(TASK_39, 29) && !who->get_legend(TASK_39, 30))
		{
			...
			who->add_cash(38000);
			send_user(who, "%c%s", ';', "四面楚歌（4） 经验 37000 tiềm năng 640 Ngân lượng 38000");
	...
	case 17:
		if (who->get_legend(TASK_39, 30) && !who->get_legend(TASK_40, 3))
		{
			...
			send_user(who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"四面楚歌（5）");
		}
		break;
	case 18:
		if (who->get_legend(TASK_2_00, 23) && !who->get_legend(TASK_2_00, 24) && who->get_save("baifang_ms_2") != 1)
		...
		if (who->get_save("baifang_ms_1") == 1 && who->get_save("baifang_ms_2") == 1 && who->get_save("baifang_ms_2") == 1 && who->get_save("baifang_ms_3") == 1 && who->get_save("baifang_ms_4") == 1 && who->get_save("baifang_ms_5") == 1)
		{
			...
			send_user(who, "%c%c%c%w%w%s", 0x51, 2, 2, TID_ZHOUGUO, 21, "Thăm viếng Chu(Hoàn thành)");
		}
		break;
	case 19:
	...
	case 20:
		if (who->get_level() >= 65 && !who->get_legend(TASK_2_01, 4))
		{
			...
			send_user(who, "%c%c%c%w%s", 0x51, 2, 1, TID_ZHOUGUO, "周国");
			send_user(who, "%c%c%c%w%w%s", 0x51, 2, 2, TID_ZHOUGUO, 103, "Bản đồ bí mật");
			send_user(who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Bản đồ bí mật");
		}
		break;
	case 21:
		if (who->get_legend(TASK_2_01, 5) && !who->get_legend(TASK_2_01, 6))
		{
			if (TASK_LEGEND_D->check_task_item(who, "Bản đồ bí mật上部", 1) != 1)
				return;
			if (TASK_LEGEND_D->check_task_item(who, "Bản đồ bí mật中部", 1) != 1)
			{
				TASK_LEGEND_D->give_item(who, "item/98/0241", 1);
				return;
			}
			if (TASK_LEGEND_D->check_task_item(who, "Bản đồ bí mật下部", 1) != 1)
			{
				TASK_LEGEND_D->give_item(who, "item/98/0241", 1);
			...
			who->add_cash(16800);
			send_user(who, "%c%s", ';', "Bản đồ bí mật 经验 16800 tiềm năng 650 Ngân lượng 16800");
	...
	case 50:
		send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(),
			sprintf("%s:\n    Bạn muốn biết gì?\n"
				ESC"晶石是什么？\ntalk %x# welcome.51\n"
				ESC"Làm thế nào để tổng hợp？\ntalk %x# welcome.52\n"
				ESC"Làm thế nào để tinh chế thiết bị？\ntalk %x# welcome.53\n"
				ESC"Không có gì.", me->get_name(), id, id, id));
		break;
	case 51:
		send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(),
			sprintf("%s:\n    在周国天坛的神秘商人那里就能买到最初级的晶石.晶石的种类一共有10种, 不同的晶石拥有不同的属性.只要你带着晶石过来, 我便可以助你强化手中的装备或武器.\n"
				ESC"Cảm ơn bạn.", me->get_name()));
		break;
	case 52:
		send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(),
	...
	case 60:
		...
		send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(),
			sprintf("%s:\n    Ngươi có thể dùng lệnh "HIR "(Alt+G)"NOR " để đưa trang bị cho ta!\n"
				ESC"Rời khỏi", me->get_name()));
		break;
	case 70:
		send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(),
...
int accept_object_callout(object me, object who, object item)
{
	...
	{
		send_user(who, "%c%c%w%s", ':', 3, me->get_char_picid(),
			sprintf("%s:\n    Đoạn Thạch không thể phân giải\n"
				ESC"Rời khỏi", me->get_name()));
		return 0;
	}
