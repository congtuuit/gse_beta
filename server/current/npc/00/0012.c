...
void do_welcome2( object me, string arg )
{
		...
		if ( !who->get_legend(TASK_NEWBIE_00, 17) )
		{
			...
			send_user( who, "%c%s", ':',me->get_name()+":\n    太好啦！！我终于美梦成真了～你真是帮了我一个大忙啊。实在太感谢了。\n"+sprintf(ESC"Hoàn thành任务\ntalk %x# welcome.3\n",getoid(me))+ESC"Rời khỏi.");
		else
			send_user( who, "%c%s", ':',me->get_name()+":\n    请暂时不要打扰我，我在思考一个很深奥的人生哲理。等你拿到Đá Tinh Linh后，再来找我吧！\n"ESC"Rời khỏi.");
                break;
	case 2:
		if (who->get_legend(TASK_NEWBIE_00, 17))	
			return ;
	...
	case 3:
		...
		{
			if( !objectp( item = present("Đá Tinh Linh", who, 1, MAX_CARRY) ) )
			{
				send_user(who,"%c%s",'!',"你的Đá Tinh Linh呢?");
				return;
			}
			...
			who->add_exp(180);
			send_user(who,"%c%s",';',"稻草人的委托 经验 180 tiềm năng 30 ");
