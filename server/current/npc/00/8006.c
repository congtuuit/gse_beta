...
void do_look(object who)
{
	...
	*/
	...
	if ("sys/party/treasure"->is_open() && "sys/party/treasure"->is_valid_time() == 1)
	{
		tmp += ESC HIY"Kho báu thất lạc\n" + cmd4;
	}
...
void do_treasure()
{
	...
	if ("sys/party/treasure"->is_open() && "sys/party/treasure"->is_valid_time() == 1)
	{
		tmp += ESC"Lĩnh cái cuốc\n" + cmd4;
		send_user(player, "%c%s", ':', get_name() + ":\n "HIR "Mỗi thứ Bảy và Chủ nhật 9:00—10:00 và 19:00—20:00"NOR "，Anh có thể tới chỗ tôi để lấy cái cuốc làm kho báu đó，"HIR "Bạn chỉ có thể có mỗi lần một cái, nhưng số lần là không giới hạn."NOR "，Trong vòng một giờ, anh có thể đào kho báu bằng một cây cuốc nhỏ！\n" +
			tmp +
			ESC"Rời khỏi\nCLOSE\n");
	}
