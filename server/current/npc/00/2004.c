...
inherit OFFICER;
mapping mpBox = ([
			382043062: ([ 32*33+4 : ([ "legend2" : 32*33+5,"obj" : "item/98/0154" ,"taskid":TID_CHUGUO,"taskno":34,"taskname":"祖父的遗产（4）(Hoàn thành)"])]),
		]);
// 函数：获取人物造型
...
//	if ( z == 2 && who->get_legend(TASK_NEWBIE_02, 12) && !who->get_save_2("dssouzuo1") )
	if ( (z==2||z==5||z==8) && who->get_legend(TASK_49, 9) && !who->get_save_2("dssouzuo1") )
	{
		...
		if( p )
		{
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_NEWBIE_01,117,"丢失的手镯(已经Hoàn thành)" );					
