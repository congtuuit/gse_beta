...
mapping mpNpc = ([
	"Tần Quốc":([
		...
		"Triệu Quốc" : ([
			1:(["file":"npc/88/2001", "nPos" : ({ 205, 130 })]),
...
void chia_doi_diem(object me, string month);
	...
	if (level > 0)
	{
		...
		if (!z)
			...
			send_user(who, "%c%s", ':', sprintf("%s:\n    Lão tiên ta đã sắp xếp cho ngươi đến "NOR "Chiến trường cấp %d  của Trường Bình Huyết Chiến số "HIR "%d"NOR ", phe tham gia là "HIR "%s"NOR ". Bây giờ chiến trường này đã có đã có "HIR "%d người "NOR "tham gia đăng ký. Ngươi còn muốn giúp gì không?\n"
				...
				ESC"Bạn đã sử dụng 5 Kim Bảo (Tham gia Chiến Trường sẽ nhận được gấp đôi Chiến Tích)\ntalk %x# welcome.17\n"
				ESC"Nhận thưởng Chiến Trường\ntalk %x# welcome.4\n"
		...
		tell_user(who, ECHO"Điểm Chiến Trường của bạn là:");

		data = explode(short_time(), " ");
		data = explode(data[0], "-");
		dateString = data[2];

		if (who->get_save_2("changping.monthScore") != to_int(data[1])) {
			chia_doi_diem(who, data[1]);
		}

		tell_user(who, sprintf(ECHO"%d", who->get_save_2("changping.score")));
...
void do_welcome2(object me, string arg)
{
	...
	case 4:		//领取奖励
		...
		send_user(who, "%c%s", ':', sprintf("%s:\n    Ngươi tổng cộng tham gia "HIR "%d lần "NOR "trận chiến Trường Bình, trong đó thắng lợi "HIR "%d trận "NOR ". Hiện giờ tổng cộng có "HIR "%d điểm "NOR "Chiến Tích, ngươi muốn sử dụng điểm Chiến Tích này để đổi giải thưởng gii?\n"
			...
			ESC"Đổi Kinh nghiệm\ntalk %x# welcome.13\n"
			ESC"Hủy", me->get_name(), x + y, x, who->get_save_2("changping.score"), getoid(me), getoid(me))
		);
		break;
	case 5:		//Hủy đăng ký
...
{
	int i, size, pos, score, win, lose, * nTmp, id, win_country = 0;
	...
	{
		cTmp = "------CHIẾN THẮNG！------";
		//|Với sự chiến đấu anh dũng của các bằng hữu, quân ta đã giành thắng lợi. Mong rằng từ nay trở đi mọi người sẽ gặt hái được nhiều thành quả thắng lợi hơn nữa.";
	...
	{
		cTmp = "------BẠI TRẬN！------";
		//|Mặc dù các bạn đã anh dũng chiến đấu, nhưng quân ta vẫn bại trận. Hi vọng từ nay về sau mọi người hãy cố gắng hơn nữa.";
		cName = "|" + sprintf(HIR"LOSE - BẠI TRẬN！");
	}
	...
	send_user(who, "%c%c%d%d%d%d%s", 0xA5, 6, map->get_2("total_player"), win, lose, score, cTmp);
...
void give_title(object me, object who, int iFlag)
{
	...
	who->set_save_2("changping.expired", time() + (30*24*3600));

	who->delete_title(cTitle1);
	who->add_title(cTitle);
	send_user(who, "%c%s", '!', "Bạn đã nhận được Danh Hiệu mới " + HIR + cName);
	MAILBOX->sys_mail(who->get_id(), who->get_number(), "Chúc mừng bạn nhận được " + HIR + cName + " Danh Hiệu Chiến Trường, thời hạn 30 ngày .");
}
...
void chia_doi_diem(object me, string month) {
	...
	int diem, i;
	int monthx, distand;
	diem = me->get_save_2("changping.score");
	monthx = to_int(month);

	if (diem > 0) {
		if (me->get_save_2("changping.monthScore") > 0) {

			if (monthx > me->get_save_2("changping.monthScore")) {
				distand = monthx - me->get_save_2("changping.monthScore");
		...
		if (me->get_save_2("changping.monthScore") != monthx) {
			me->add_save_2("changping.score", - diem);
			me->add_save_2("changping.totalscore", - diem);
			me->set_save_2("changping.monthScore", monthx);
		}
	}
