...
{
	object item;
	if (objectp(item = present("Một gói hàng từ Viện Hàn", who, 1, MAX_CARRY)) && who->get_quest("valentine.flag") == 1)
	{
		object npc, me = this_object();
		int id = getoid(me), i;
		string cmd = sprintf("talk %x# valentine\n", id);
		string tmp = "";
		tmp = ESC "Hạt giống bí ẩn\n" + cmd;
	...
	else if (!objectp(item = present("Hạt giống bí ẩn", who, 1, MAX_CARRY)) && who->get_quest("valentine.flag") == 3)
	{
		object npc, me = this_object();
		int id = getoid(me), i;
...
void delete_family_skill(object who)
{
	...
	name = keys(skls);
	size = sizeof(name);
	result = sprintf("Ngươi quyết định “%s” 门下！损失：", get_fam_name());
	for (i = 0; i < size; i++)
	{
		skill = to_int(name[i]);
...
void do_valentine_call(object who, object me)
{
	...
	if (objectp(item = present("Một gói hàng từ Viện Hàn", who, 1, MAX_CARRY)) && who->get_quest("valentine.flag") == 1)
	{
		...
		string tmp = "";
		tmp = ESC "Kết thúc nhiệm vụ\n" + cmd;
		send_user(who, "%c%s", ':', get_name() + "：\n    喔～包裹我收下了，谢谢你！看来你也是一个有诚心的小伙子，但是这Hạt giống bí ẩn在几日前就已经被偷走了，如果你很想要得到的话，你考虑清楚后再来找我吧！\n" +
			tmp +
			ESC "Nghỉ\nCLOSE\n");
	}
}
...
void do_valentine5_call(object who, object me)
{
	...
	{
		cWord = sprintf(me->get_name() + "：\n    Hạt giống là thứ anh muốn.！");
		who->set_quest("valentine.name", "Kẻ trộm hoa hồng");
		send_user(who, "%c%s", ':', cWord + "\n" +
			ESC + sprintf("Kết thúc nhiệm vụ。\ntalk %x# valentine6\n", getoid(me)) +
			ESC "Nghỉ。\nCLOSE\n");
	}
}
...
void do_valentine6_call(object who, object me)
{
	...
	if (who->get_quest("valentine.flag") == 2 && who->get_quest("valentine.bonus") == 1)
	{
		...
		{
			send_user(who, "%c%s", '!', "Ngươi không có Hạt giống bí ẩn");
			return;
		}
		item->remove_from_user();
		destruct(item);
...
void do_valentine7_call(object who, object me)
{
	...
	string cmd, cWord, tmp = "";
	object item;
	if (!objectp(item = present("Hạt giống bí ẩn", who, 1, MAX_CARRY)) && who->get_quest("valentine.flag") == 3)
	{
		cWord = sprintf(me->get_name() + "：\n    将这Hạt giống bí ẩn拿去交给月老吧，我想他应该知道要怎样做的！");
		send_user(who, "%c%s", ':', cWord + "\n" +
			ESC + sprintf("chấp nhận nhiệm vụ。\ntalk %x# valentine8\n", getoid(me)) +
			ESC "Nghỉ。\nCLOSE\n");
	}
...
void do_valentine8_call(object who, object me)
{
	...
	if (!objectp(item = present("Hạt giống bí ẩn", who, 1, MAX_CARRY)) && who->get_quest("valentine.flag") == 3)
	{
		object item214_1 = new ("item/04/0421");    //Hạt giống bí ẩn
		string name214_1 = "item/04/0421";
		if (item214_1)
		{
			if (USER_INVENTORY_D->can_carry(who, item214_1) >= 1)
			{
				...
				send_user(who, "%c%s", '!', "得到"HIR"Hạt giống bí ẩn"NOR"一袋");
				...
				send_user(who, "%c%s", '!', "Bạn đã nhận được Valentine Day————Tìm！");
			}
			else
			{
				destruct(item214_1);
