...
return;
	...
	if ( "sys/school/shouxi"->can_betray(who) == 0 )
        ...
        {
                send_user( who, "%c%s", ':', sprintf( "%s：\n    您上次叛师所返回的经验和tiềm năng还没有取回，不能再叛师。\n",
                        me->get_name() ) );
                return;
        }
        if( arg == "?" )
...
//		send_user( who, "%c%s", ':', sprintf( "%s：\n    您这次背叛，积累下%d点tiềm năng和%d金，您可以向我领取。\n",
...
void do_getback( string arg )
{
	int money, pot, get, less;
        ...
        if (arg=="?")
        {
        	result = sprintf("%s：\n    因为这次官方的门派调整所对阁下造成不便，我们特别为您提供了一次tiềm năng和Ngân lượng的补偿机会。记住，补偿的机会只有一次，当你把所有的补偿都领取完毕之后，我不会再给予你任何的补偿，一定要好好把握应用。\n"
                	ESC "确定\ntalk %x# getback\n"
                	ESC "取消。\nCLOSE\n",
	...
	if (less>0)
		result = sprintf("%s：\n    您领取了%dtiềm năng%d金，尚余%dtiềm năng%d金没有领取。\n", me->get_name(), get, get*20, less, less*20);
	else
		result = sprintf("%s：\n    您领取了%dtiềm năng%d金，全部领取完毕。\n", me->get_name(), get, get*20);
	send_user( who, "%c%s", ':', result);
}
