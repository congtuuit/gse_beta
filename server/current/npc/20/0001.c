...
void do_look( object who )
{
	string tmp="";
        ...
        send_user( who, "%c%s", ':', sprintf( "%s：\n    跟这里的所有人一样，我的军旅生涯是从被抓壮丁开始的。从我Rời khỏi.村子那天起，我的女人就注定成了活寡妇。\n", get_name())
        + tmp +
        ESC"Rời khỏi."
         );
}
...
void do_welcome2( object me, string arg )
{
	...
	case 2:
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_HANGUO,46,"Tình yêu（1）" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Tình yêu（1）");
        	}
        	break;       
        case 3:
			...
			who->add_cash(20000);
			send_user(who,"%c%s",';',"Tình yêu（1） 经验 20000 tiềm năng 290 Ngân lượng 20000");
	...
	case 4:
			...
			send_user( who, "%c%c%c%w%s", 0x51, 2, 1,TID_HANGUO,"韩国" );
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_HANGUO,47,"Tình yêu（2）" );
			send_user( who, "%c%s", '!', "Nhận nhiệm vụ  "HIY"Tình yêu（2）");
        	}
        	break;  
        }
}