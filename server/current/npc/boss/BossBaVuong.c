...
//        set("no_sk04237",1);	//玄龙决对其没有效果
        ...
        set( "birthday", time() );
        if( !clonep(me) ) return;
	CHAT_D->rumor_channel( 0, CHAT+"Nghe nói Bá Vương Đệ Nhất Sư đã xuất hiện tại Sở Phương Thành 2" ); 
        set_2( "talk", ([
                "fight"		: (: do_fight:),
        ]));  
	log_file("boss.txt",short_time()+" Bá Vương Đệ Nhất Sư xuất hiện\n");
}
	...
	{		
		if ( gone_time(saytime) >=60 )
		{
			...
			if ( random(2) )
			{
				...
				{
					case 0:
						SAY_CMD->say(me, "Loài người ngu xuẩn.Các ngươi sắp chết rồi đấy！！"); 
						break;
					case 1:
						SAY_CMD->say(me, "Con người nhỏ yếu! Hãy chịu chết đi!"); 
						break;
					case 2:
						SAY_CMD->say(me, enemy->get_name()+"，Hãy nhận lấy hậu quả khi dám thách thức bổn vương！！"); 
						break;
					default:
						SAY_CMD->say(me, "Con người thật quá nhỏ bé！Thật là thất vọng！"); 
						break;
				}
			}
		}
...
int perform_action( object who ) 
{
	...
	if ( lowhp == 0 && get_hp() * 100 / get_max_hp() < 40 )	//生命值为40％以下
	{
		...
		if ( summon == 0 )
			summon_guard();
		set_lowhp_status();
		SAY_CMD->say(me, "Loài người ngu xuẩn！Các ngươi phải trả giá vì dám chọc tức bổn đại vương！"); 	
	}
...
void win_bonus_callout( object me, object who )
{
	...
	log_file("boss.txt",short_time()+" Bá Vương Đệ Nhất Sư đã bị tiêu diệt\n");
...
void drop_items( object me, object who )
{
	...
	USER_D->user_channel(sprintf(HIR "Đệ Nhất Võ Lâm Cao Thủ "HIY "%s"  HIC" đã tiêu diệt "HIY "Bá Vương Đệ Nhất Sư "HIC" và lấy được nhiều vật phẩm đáng giá. ", who->get_name() ));
	...
	if ( rate < rate1 )
	{
		...
		{
			item = new( "/item/ppvacanh/phiphong/nam/lv150" );
			if ( !item )
				return;
			TEAM_D->drop_item(item,who);
			item->set_user_id(owner);
		...
		if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )	//玉灵液
		{
			item = new( "/item/ppvacanh/phiphong/nu/lv150" );
			if ( !item )
				return;
			TEAM_D->drop_item(item,who);
			item->set_user_id(owner);
		...
		if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )	//玉灵液
		{
			item = new( "/item/ppvacanh/canh/nam/lv150" );
			if ( !item )
				return;
			TEAM_D->drop_item(item,who);
			item->set_user_id(owner);
		...
		{
			item = new( "/item/ppvacanh/canh/nu/lv150" );
			if ( !item )
				return;
			TEAM_D->drop_item(item,who);
			item->set_user_id(owner);
	...
	else if ( rate < 2500  * rate1 / 100)		//紫参丹
	{
	         for(i=0;i<2;i++)
	         {
	                if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )
	                {
	                        item = new( "/item/tienvang/tienvang500" );
	                        TEAM_D->drop_item(item,who);
				item->set_user_id(owner);
	                        item->add_to_scene(z, p / 1000, p % 1000);
	                        item->set("winner", who);
	...
	else if ( rate < 4400  * rate1 / 100 )		//无暇宝石矿 2个
	{
	         for(i=0;i<3;i++)
	         {
	                if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )
	                {
	                        item = new( "item/tienvang/tienvang200");
	                        TEAM_D->drop_item(item,who);
				item->set_user_id(owner);
	                        item->add_to_scene(z, p / 1000, p % 1000);
	                        item->set("winner", who);
	...
	else if ( rate < 6100  * rate1 / 100 )		//完美宝石矿 1个
	{
			for(i=0;i<4;i++)
	         { 
                if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )
                {
                        item = new( "item/tienvang/tienvang100");
                        TEAM_D->drop_item(item,who);
			item->set_user_id(owner);
                        item->add_to_scene(z, p / 1000, p % 1000);
                        item->set("winner", who);
...
{
	send_user( who, "%c%c%w%s", ':',3,9966,"Bá Vương Đệ Nhất Sư:\n    Con người nhỏ bé, các người vẫn và sẽ không bao giờ đủ tuổi để có tư cách nói chuyện với ta đâu！！\n"+
		sprintf(ESC"Ta khiêu chiến ngươi...\ntalk %x# fight\n",getoid(this_object())) +
		ESC"Rời khỏi");
}
