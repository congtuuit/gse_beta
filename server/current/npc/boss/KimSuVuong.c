...
int flag;
int saytime, hptime, hptime2, silent, silent2, silent3, summon, RelaxTime,
number = 1;
object* huohai = ({});
string* nFamily = ({ "Đào Hoa Nguyên", "Thục Sơn", "Cấm Vệ Quân", "Đường Môn",
                    "Mao Sơn", "Côn Luân", "Vân Mộng Cốc" });
// 函数：是否不能进城
...
void create() {
    ...
    set_head_color(0x00FF00FF);

    setup();
    set_char_type(OFFICER_TYPE);
    set("birthday", time());
    if (!clonep(me)) return;
    CHAT_D->rumor_channel(
        0, CHAT + "Nghe nói Kim Sư Vương xuất hiện ở Tứ Linh Thần Động.");
    set_2("talk", (["fight":(: do_fight:), ]));
    set("guard", ({}));
    log_file("boss.txt", short_time() + " Kim Sư Vương xuất hiện\n");
}

int can_fight(object who) {
    object owner;
    string leader, id;
    if (get_char_type() == OFFICER_TYPE) return 0;
    if (who->is_die()) {
        set_enemy(0);
        return 0;
    }
...
void heart_beat_walk() {
    ...
    if (objectp(enemy)) {
        if (gone_time(saytime) >= 60) {
            ...
            if (random(10) > 1) {
                switch (random(3)) {
                case 0:
                    SAY_CMD->say(me, "Các ngươi đang quấy rối ta đấy...");
                    break;
                case 1:
                    SAY_CMD->say(me,
                        enemy->get_name() + ", hãy quay về nơi của người đi.");
                    break;
                case 2:
                    SAY_CMD->say(me, "Hãy xem sức mạnh của ta đây!!!");
                    break;
                default:
                    SAY_CMD->say(me,
                        "Các ngươi không muốn sống nữa, thì ta cho các ngươi "
                        "toại nguyện....");
                    break;
                }
            }
        }
        if (enemy->is_die()) set_enemy(0);
    }
    else if (RelaxTime == 0) {
        RelaxTime = time();
    }
    ...
    if (gone_time(hptime) >= 10) {
        ...
        if (objectp(enemy)) {
            ...
            if (z == z0 && (p = distance_between(me, enemy)) < 20 && p > 1) {
                ...
                y0 = get_y(me);
                for (i = 0; i < 10; i++) {
                    if (p = get_valid_xy(z, abs(x + random(3) - 1),
                        abs(y + random(3) - 1), IS_CHAR_BLOCK)) {
                        x1 = p / 1000;
                        y1 = p % 1000;
                        break;
                    }
    ...
    if (objectp(enemy = get_enemy()) && gone_time(leiTime) > 40) {
        ...
        if (lowhp && random(100) < 60)
            return "skill/11/11016"->perform_action_npc(me, 0);
    }
...
void summon_guard() {
    ...
    for (i = 0; i < 4; i++)  // 4个游魂祸害
    {
        ...
        send_user(get_scene_object_2(npc, USER_TYPE), "%c%d%w%c%c%w%c%c%w%c%c%c",
            0x6f, getoid(npc), 41311, 1, OVER_BODY, 41312, 1, OVER_BODY,
            41313, 1, UNDER_FOOT, PF_ONCE);
    }
}
// 函数：特技攻击对手
int perform_action(object who) {
    ...
    if (gone_time(silent2) >= 30) {
        silent2 = time();
        SAY_CMD->say(me,
            who->get_name() + ", Hãy đỡ đòn này của ta, đỡ nổi không ?");
        ...
        if (random100() < 50) {
            ...
            for (i = 0, size = sizeof(huohai); i < size; i++) {
                ...
                char = get_range_object(z, x, y, 2, USER_TYPE) +
                    get_range_object(z, x, y, 2, CHAR_TYPE) - ({ huohai[i] });
                for (j = 0; j < sizeof(char); j++)
                    if (who = char[j]) {
                        if (who->is_npc()) continue;
                        send_user(get_scene_object_2(huohai[i], USER_TYPE),
                            "%c%w%w%w%c%c%c", 0x4f, get_x(huohai[i]),
                            get_y(huohai[i]), 9902, 1, OVER_BODY, PF_ONCE);
                        send_user(who, "%c%d%w%c%d%w%c", 0x48, getoid(who), damage,
                            get_d(who), 0, HIT_BLADE, 1);
...
void win_bonus_callout(object me, object who) {
    ...
    CHAT_D->rumor_channel(
        0, CHAT + sprintf("Nghe nói Kim Sư Vương đã bị %s tiêu diệt rồi",
            who->get_name()));
    log_file("boss.txt", short_time() + " Kim Sư Vương bị tiêu diệt\n");
    ...
    //		who->get_save_2("cantuong.ktl")>=1 ) send_user( who, "%c%c%c%w%w%s",
    //0x51, 1, 2, TID_CANTUONG, 1, "Tìm kiếm Thần Binh Phổ(Hoàn thành)" );
    //		send_user(who,"%c%s",';',"Đã tiêu diệt Âm Gian Bạn Đồ cần cho nhiệm
    //vụ nâng cấp Tuyệt thế thần binh !");
    //	}
    ...
    for (i = 0; i < size; i++)
        if (team[i]) {
            ...
            exp2 = me->correct_exp_bonus(level, level2, exp2) *
                team[i]->get_online_rate() / 100;
...
void drop_items(object me, object who) {
    ...
    rate1 = me->correct_drop_rate(me->get_level() - who->get_level()) *
    ...
    if (rate < rate1) {
        if (p = get_valid_xy(
            z, x, y,
            IS_ITEM_BLOCK))  // 70级或80级（二选一）的随机门派金色装备1件
        {
            nTmp = WEAPON_FILE->get_family_equip(
                nFamily[random(7)], 0, random(2) ? 70 : 80, HEAD_TYPE + random(6));
            if (sizeof(nTmp)) {
    ...
    if (!p) return;
    rate = random(10000);
    if (rate < 500 * rate1 / 100)
    {
        nTmp = WEAPON_FILE->get_family_equip(nFamily[random(7)], 0, 80,
            HEAD_TYPE + random(6));
        if (sizeof(nTmp)) {
            item = new (nTmp[random(sizeof(nTmp))]);
            if (!item) return;
