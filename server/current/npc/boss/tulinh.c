...
void create()
{
	...
	set("birthday", time());
	if (!clonep(me)) return;
	CHAT_D->rumor_channel(0, CHAT + "Nghe nói Thú Hồn Tứ Linh lại xuất hiện!!");
	set_2("talk", ([
		"fight":(: do_fight:),
	]));
	log_file("boss.txt", short_time() + " BOSS tứ linh xuất hiện\n");
...
void drop_items(object me, object who)
{
	...
	if ("/sys/sys/title"->check_title_code("E500") < 1) {
		who->set_save_2("TuLinh.total", who->get_save_2("TuLinh.total") + 1);

		if (who->get_save_2("TuLinh.total") > 3) {
			who->add_title("E500");
			who->show_title("E500");

			who->delete_save_2("TuLinh.total");
			who->delete_save_2("TuLinh");
		}
	}
