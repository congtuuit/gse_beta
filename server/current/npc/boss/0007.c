	...
	if (objectp(enemy))
	{
		if (gone_time(saytime) >= 60)
		{
			...
			if (random(2))
			{
				...
				{
				case 0:
					SAY_CMD->say(me, "贫道术法天下无双！就算是太上老君也奈何我不得！");
					break;
				case 1:
					SAY_CMD->say(me, "这些尸魔的诅咒可将你们直接打下十八层地狱！");
					break;
				default:
					SAY_CMD->say(me, sprintf("哈哈～～！%s你也敢与贫道斗法！？回去多修炼几劫再来！", enemy->get_name()));
					break;
				}
			}
		}
...
int perform_action(object who)
{
	...
	if (lowhp == 0 && get_hp() * 100 / get_max_hp() < 40)	//生命值为40％以下
	{
		lowhp = 1;
		summon_guard();
		SAY_CMD->say(me, "可恶……本道长可不会就这样被打败的！");
		return 0;
	}
...
void win_bonus_callout(object me, object who)
{
	...
	for (i = 0; i < size; i++) if (team[i])
	{
		...
		//	"sys/sys/test_db"->add_yuanbao(team[i],55/size);
	}
}
...
{
	send_user(who, "%c%c%w%s", ':', 3, 9968, "Khu Thi Đạo Trưởng:\n    生死去来，棚头傀儡。一线断时，落落磊磊。嘻嘻，人的生命不过是贫道手中的木偶，就像这些僵尸一样，偶线一断，什么都完结了。\n" +
