...
void create()
{
	...
	set("birthday", time());
	if (!clonep(me)) return;
	CHAT_D->rumor_channel(0, CHAT + HIM"Nghe nói Phi Tặc Vương lại xuất hiện trên tầng 4 của địa ngục！");
	set_2("talk", ([
		"fight":(: do_fight:),
	]));
	...
	if (objectp(enemy))
	{
		if (gone_time(saytime) >= 60)
		{
			...
			if (random(2))
			{
				...
				{
				case 0:
					SAY_CMD->say(me, "Ngươi có muốn ân điển của thiên đàng？Đánh ta trước đã！！");
					break;
				case 1:
					SAY_CMD->say(me, "Thiên uy，một người yếu đuối như ngươi có thể có được nó！");
					break;
				default:
					SAY_CMD->say(me, enemy->get_name() + "Đến và chấp nhận đi thử thách đi！！");
					break;
				}
			}
		}
...
void drop_items(object me, object who)
{
	...
	for (i = 0; i < 10; i++) {
		if (p = get_valid_xy(z, x, y, IS_ITEM_BLOCK))
		{
			item = new(sprintf("/item/sell/%d032", random(3)));
			if (!item)
				return;
...
{
	send_user(who, "%c%c%w%s", ':', 3, 0202, "Phi Tặc Vương:\n    Con người ngu ngốc！Sao ngươi dám bước vào vương quốc của ta！！Xem ra ngươi chán sống rồi！！\n" +
