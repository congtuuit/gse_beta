...
//        set("no_sk04237",1);	//玄龙决对其没有效果
        ...
        set( "birthday", time() );
        if( !clonep(me) ) return;
	CHAT_D->rumor_channel( 0, CHAT+"Nghe nói Bá Vương Sư lại xuất hiện ở Sở Phương Thành 2!" ); 
        set_2( "talk", ([
                "fight"		: (: do_fight:),
        ]));  
	log_file("boss.txt",short_time()+" Bá Vương Sư xuất hiện\n");
}
	...
	{		
		if ( gone_time(saytime) >=60 )
		{
			...
			if ( random(2) )
			{
				...
				{
					case 0:
						SAY_CMD->say(me, "Ta là Bá Vương Lão Nhị..người cả gan dám chọc tức ta！！"); 
						break;
					case 1:
						SAY_CMD->say(me, "Con người nhỏ yếu! Hãy chịu chết đi!"); 
						break;
					case 2:
						SAY_CMD->say(me, enemy->get_name()+"，Hãy nhận lấy！！"); 
						break;
					default:
						SAY_CMD->say(me, "Bá Vương Lão Nhị ta không phải là dễ xơi mô nhe！！"); 
						break;
				}
			}
		}
...
int perform_action( object who ) 
{
	...
	if ( lowhp == 0 && get_hp() * 100 / get_max_hp() < 40 )	//生命值为40％以下
	{
		...
		if ( summon == 0 )
			summon_guard();
		set_lowhp_status();
		SAY_CMD->say(me, "Loài người ngu xuẩn！Các ngươi phải trả giá vì đã chọc tức bổn vương！"); 	
	}
...
void win_bonus_callout( object me, object who )
{
	...
	CHAT_D->rumor_channel( 0, CHAT+"Nghe nói Bá Vương Đệ Nhị Sư đã bị một hiệp sĩ vô danh tiêu diệt" ); 
	log_file("boss.txt",short_time()+" Bá Vương Đệ Nhị Sư đã bị tiêu diệt\n");
	...
	exp = 50000;
        for( i = 0; i < size; i ++ ) if( team[i] )
        {
				...
				team[i]->add_title("P013");
				team[i]->add_title("P014");
				team[i]->add_title("P015");
				team[i]->add_title("P016");
				team[i]->add_title("P017");
				team[i]->add_title("P018");
...
void drop_items( object me, object who )
{
	...
	if ( rate < rate1 )
	{
		...
		{
			item = new( "/item/tienvang/tienvang10" );
			if ( !item )
				return;
			TEAM_D->drop_item(item,who);
			item->set_user_id(owner);
	...
	else if ( rate < 100  * rate1 / 100)		//紫参丹
	{
	         for(i=0;i<1;i++)
	         {
	                if( p = get_valid_xy(z, x, y, IS_ITEM_BLOCK) )
...
{
	send_user( who, "%c%c%w%s", ':',3,9970,"Bá Vương Đệ Nhị Sư:\n    Ngươi có đủ tự tin chắc chắn rằng ngươi đủ tuổi để nói chuyện với ta chưa！！\n"+
		sprintf(ESC"Ta nổi giận rồi..Giết\ntalk %x# fight\n",getoid(this_object())) +
		ESC"Rời khỏi");
}
