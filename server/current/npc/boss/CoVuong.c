...
void create()
{
	...
	set("birthday", time());
	if (!clonep(me)) return;
	CHAT_D->rumor_channel(0, CHAT + "Nghe nói Cờ Vương Đại Sư đã xuất hiện tại Bạch Mã Trang");
	set_2("talk", ([
		"fight":(: do_fight:),
	]));
	log_file("boss.txt", short_time() + " Cờ Vương Đại Sư xuất hiện\n");
}
	...
	if (objectp(enemy))
	{
		if (gone_time(saytime) >= 40)
		{
			...
			if (random(2))
			{
				...
				{
				case 0:
					SAY_CMD->say(me, "Ngươi có bản lĩnh thì hãy thi đấu 1 ván xem nào！");
					break;
				case 1:
					SAY_CMD->say(me, "Tuổi trẻ chưa trải sự đời, cần luyện tập thêm rồi hay quay lại tìm ta nhé!");
					break;
				case 2:
					SAY_CMD->say(me, enemy->get_name() + "，Woww, đúng là tuổi trả tài cao！ Hãy xem...");
					break;
				default:
					SAY_CMD->say(me, "Ngươi cần luyện tập thêm！Thật là thất vọng！");
					break;
				}
			}
		}
...
void win_bonus_callout(object me, object who)
{
	...
	log_file("boss.txt", short_time() + " Cờ Vương Đại Sư đã bị tiêu diệt\n");
	...
	for (i = 0; i < size; i++) if (team[i])
	{
		...
		//team[i]->add_title("P019");
		//team[i]->show_title("P019");
		...
		write_user(team[i], sprintf(ECHO "Tiêu diệt Cờ Vương Đại Sư nhận được %d Kim Bảo !", (knbAmount) / 10));
...
void drop_items(object me, object who)
{
	...
	USER_D->user_channel(sprintf(HIR "Võ Lâm Cao Thủ "HIY "%s"  HIC" đã đánh bại "HIY "Cờ Vương Đại Sư "HIC"nhận được Kim Bảo và cùng nhiều vật phẩm đáng giá. ", who->get_name()));
...
{
	send_user(who, "%c%c%w%s", ':', 3, 9966, "Cờ Vương Đại Sư:\n    Ngươi đã đủ khả năng chưa, ngươi còn trẻ, còn rất nhiều thời gian！！\n" +
		sprintf(ESC"Ta muốn khiêu chiến...\ntalk %x# fight\n", getoid(this_object())) +
		ESC"Rời khỏi");
}
