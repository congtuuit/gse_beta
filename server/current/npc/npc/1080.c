...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_43, 26) && !who->get_legend(TASK_43, 27) && random(100) < 18)
	{
		...
		send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,59,"Biểu tượng quyền lực(Hoàn thành)" );
	...
	if( who->get_legend(TASK_45, 1) && !who->get_legend(TASK_45, 2) && who->get_save("lilangtaijmzs") < 18 )
	{
		...
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,68,"Ấm và lạnh（3）(Hoàn thành)" );
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                else if( p < 7183 * rate / 100 )    // Ngân lượng
                {
