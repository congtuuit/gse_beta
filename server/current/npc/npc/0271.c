...
void check_legend_task( object who )
{
	...
	{
		if ( objectp( item = present("Rượu vang đỏ", who, 1, MAX_CARRY) ) && item->is_legend_item() == 1 ) 
		{
		...
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_WEIGUO,4,"Rượu cho Lão Đinh(Hoàn thành)" );
			who->set_save_2("haopengyou",10);
			who->set_legend(TASK_07, 2);
		}

		USER_TASK_D->send_task_intro(who,2,TID_WEIGUO,4);
		send_user(who,"%c%s",'!',sprintf(HIY"Rượu vang đỏ %d/10",i));		
	}
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                else if( p < 5290 * rate / 100 )    // Ngân lượng
                {
