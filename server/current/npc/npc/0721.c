...
void check_legend_task( object who )
{
	...
	{
		send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_HANGUO,41,"Gia vị nấu cá(Hoàn thành)" );
	...
	if( who->get_legend(TASK_22, 24) && !who->get_legend(TASK_22, 25) && random(100) < 18 )
	{
		...
		send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_HANGUO,42,"Mồi câu vàng(Hoàn thành)" );
	...
	if( who->get_legend(TASK_23, 1) && !who->get_legend(TASK_23, 2) )
	{
		if ( (i=who->add_save_2("hgdjjmitan",1)) >= 10 )
		{
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_HANGUO,44,"Thảm họa rò rỉ(Hoàn thành)" );
		}
	...
	{
		send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_WEIGUO,29,"Tình yêu（4）(Hoàn thành)" );
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                else if( p < 9255 * rate / 100 )    // Ngân lượng
                {
