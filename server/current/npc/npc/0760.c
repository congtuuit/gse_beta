...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_24, 4) && !who->get_legend(TASK_24, 5) )
	{
		...
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,14,"Quán Trung Yêu(Hoàn thành)" );
	...
	if( who->get_legend(TASK_35, 27) && !who->get_legend(TASK_35, 28) && who->get_save("qgsrguanzy") < 18 )
	{
		...
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,50,"Quán Trung Yêu Tiên trong nồi(Hoàn thành)" );
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                else if( p < 5909 * rate / 100 )    // Ngân lượng
                {
