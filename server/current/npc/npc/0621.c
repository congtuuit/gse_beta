...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_16, 3) && !who->get_legend(TASK_16, 4) && who->get_save_2("zgjianbing") < 20)
	{
		...
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,7,"Chó đường phố(Hoàn thành)" );
	...
	if( who->get_legend(TASK_19, 26) && !who->get_legend(TASK_19, 27) && random(100) < 18)
	{
		...
		send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHAOGUO,37,"Món quà phù hợp(Hoàn thành)" );
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                else if( p < 5690 * rate / 100 )    // Ngân lượng
                {
