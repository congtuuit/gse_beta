...
void check_legend_task( object who )
{
	...
	{
		if ( objectp( item = present("Tượng bùn", who, 1, MAX_CARRY) ) && item->is_legend_item() == 1 ) 
		{
		...
		if ( (i=who->add_save_2("gdsnidiao",1)) >= 10 )
		{
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,20,"Tượng bùn(Hoàn thành)" );
		}
		USER_TASK_D->send_task_intro(who,2,TID_YANGUO,20);
		send_user(who,"%c%s",'!',sprintf(HIY"Tượng bùn %d/10",i));		
	}	
	if( who->get_legend(TASK_13, 28) && !who->get_legend(TASK_13, 29) && who->get_save_2("jjdaodunbing") < 20)
	{
		...
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,22,"Nguy hiểm tìm ẩn(Hoàn thành)" );
	...
	if( who->get_legend(TASK_15, 9) && !who->get_legend(TASK_15, 10) && random(100) < 18)
	{
		...
		if ( (i=who->add_save_2("frlingzhi",1)) >= 3 )
		{
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,31,"夫人的怪病（2）(Hoàn thành)" );
		}
	...
	if( who->get_legend(TASK_15, 15) && !who->get_legend(TASK_15, 16) && random(100) < 25)
	{
		...
		if ( (i=who->add_save_2("cqfajia",1)) >= 10 )
		{
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,33,"Cửa hàng trang sức(Hoàn thành)" );
		}
	...
	if( who->get_legend(TASK_15, 18) && !who->get_legend(TASK_15, 19) && random(100) < 34)
	{
		...
		if ( (i=who->add_save_2("cflajiao",1)) >= 20 )
		{
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,34,"Người giàu có(Hoàn thành)" );
		}
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                else if( p < 5595 * rate / 100 )    // Ngân lượng
                {
