...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_07, 13) && !who->get_legend(TASK_07, 14) && random(100) < 50)
	{
		...
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_WEIGUO,8,"Thảm họa của Ngụy quản gia（1）(Hoàn thành)" );
	...
	if( who->get_legend(TASK_07, 22) && !who->get_legend(TASK_07, 23) && random(100) < 28 && who->get_save_2("ssyluobo") < 4)
	{
		...
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_WEIGUO,26,"Bữa tối cầu kỳ(Hoàn thành)" );
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                else if( p < 5360 * rate / 100 )    // Ngân lượng
                {
