...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_31, 27) && !who->get_legend(TASK_31, 28) && random(100) < 25)
	{
		...
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,42,"Hỗ trợ(Hoàn thành)" );
	...
	{
		if ( objectp( item = present("Dao đơn", who, 1, MAX_CARRY) ) && item->is_legend_item() == 1 ) 
		{
		...
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,43,"Dao đơn(Hoàn thành)" );
			who->set_legend(TASK_32, 2);
		}

		USER_TASK_D->send_task_intro(who,2,TID_YANGUO,43);
		send_user(who,"%c%s",'!',sprintf(HIY"Dao đơn %d/8",i));		
	}
	if( who->get_legend(TASK_32, 14) && !who->get_legend(TASK_32, 15) && random(100) < 25)
	{
		...
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,45,"Thu thập táo tàu(Hoàn thành)" );
	...
	if( who->get_legend(TASK_32, 17) && !who->get_legend(TASK_32, 18) && who->get_save("ygswhaiguj") < 8 )
	{
		...
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,46,"Sức mạnh của trụy lạc(Hoàn thành)" );
	...
	if( who->get_legend(TASK_32, 23) && !who->get_legend(TASK_32, 24) && who->get_save("ywhaiguj") < 5 )
	{
		...
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,48,"Giết nó(Hoàn thành)" );
	...
	if( who->get_legend(TASK_43, 20) && !who->get_legend(TASK_43, 21) )
	{
		...
		if ( who->get_save("ygdjjnangua")>=8 && who->get_save("ygdjjluobo")>=12 && who->get_save("ygdjjjidan")>=7 )
		{
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,50,"Nguồn cung cấp doanh trại(Hoàn thành)" );
		}
	}
}
// 函数：掉宝奖励
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                else if( p < 6659 * rate / 100 )    // Ngân lượng
                {
