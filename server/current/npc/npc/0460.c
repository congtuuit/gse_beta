...
void check_legend_task( object who )
{
	...
	{
		if ( objectp( item = present("Đá ma thuật", who, 1, MAX_CARRY) ) && item->is_legend_item() == 1 ) 
		{
		...
		if ( (i=who->add_save_2("moqingshi",1)) >= 10 )
		{
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QINGUO,7,"Đá ma thuật(Hoàn thành)" );
		}
		USER_TASK_D->send_task_intro(who,2,TID_QINGUO,7);
		send_user(who,"%c%s",'!',sprintf(HIY"Đá ma thuật %d/10",i));		
	}	
	if( who->get_legend(TASK_10, 20) && !who->get_legend(TASK_10, 21) && who->get_save_2("tpzuangding") < 30)
	{
		if ( (i=who->add_save_2("tpzuangding",1)) >= 30 )
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QINGUO,1,"Chạy trốn(Hoàn thành)" );
	...
	if( who->get_legend(TASK_23, 22) && !who->get_legend(TASK_23, 23) )
	{
		...
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_HANGUO,46,"Tình yêu（1）(Hoàn thành)" );
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                else if( p < 5524 * rate / 100 )    // Ngân lượng
                {
