...
void check_legend_task( object who )
{
	...
	{
		if ( objectp( item = present("Hoa nhài", who, 1, MAX_CARRY) ) && item->is_legend_item() == 1 ) 
		{
		...
		if ( (i=who->add_save_2("cgbxmolihua",1)) >= 20 )
		{
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_CHUGUO,29,"Trà Jasmine（2）(Hoàn thành)" );
		}
		USER_TASK_D->send_task_intro(who,2,TID_CHUGUO,29);
		send_user(who,"%c%s",'!',sprintf(HIY"Hoa nhài %d/20",i));		
	}
	if( who->get_legend(TASK_24, 17) && !who->get_legend(TASK_24, 18) && random(100) < 18)
	{
		...
		send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,16,"Tìm kiếm con dấu chính thức(Hoàn thành)" );
	...
	if( who->get_legend(TASK_25, 11) && !who->get_legend(TASK_25, 12) && who->get_save_2("hzgdoubing") < 20 )
	{
		...
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_HANGUO,19,"Chứng minh bản thân(Hoàn thành)" );
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                else if( p < 9242 * rate / 100 )    // Ngân lượng
                {
