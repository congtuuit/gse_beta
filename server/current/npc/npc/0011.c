...
void check_legend_task( object who ) 
{
	...
	if ( who->get_legend(TASK_49, 18) && !who->get_legend(TASK_49, 19) )
	{
		...
		if ( i >= 3 )
		{
			who->set_legend(TASK_49, 19);
	                send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_NEWBIE_01,46,"Một con thỏ nhỏ(Hoàn thành)" );
			i = 3;
		}
	...
	if( who->get_legend(TASK_40, 26) && !who->get_legend(TASK_40, 27) && who->get_save("ydlbxiaoyt") < 5 )
	{
		...
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_NEWBIE_01,25,"Đau đầu（2）(Hoàn thành)" );
...
/*              else if( p < 4323 * rate / 100 )    // Ngân lượng
                {
