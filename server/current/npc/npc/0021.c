...
void check_legend_task( object who ) 
{
			...
			p =item->move(who, -1);
				...
				return;
	                ...
	                else
	                {
	                	...
	                	send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_NEWBIE_01,107,"修理店老板的铁锤(Hoàn thành)" );
	...
	if ( who->get_legend(TASK_48, 18) && !who->get_legend(TASK_48, 19) && who->get_save("new_chy") < 8 )
	{
		...
		if ( i >= 8 )
		{
			who->set_legend(TASK_48, 19);
	                send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_NEWBIE_01,108,"Niệm chú lên con người(Hoàn thành)" );
			i = 8;
		}
		send_user(who,"%c%s",'!',sprintf("Niệm chú lên con người Hoàn thành %d/8",i));
	...
	if ( who->get_legend(TASK_49, 15) && !who->get_legend(TASK_49, 16) )
	{
		...
		if ( i >= 3 )
		{
			who->set_legend(TASK_49, 16);
	                send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_NEWBIE_01,45,"Rắc rối do Kiến gây ra(Hoàn thành)" );
			i = 3;
		}
		send_user(who,"%c%s",'!',sprintf("Kiến Nhỏ Hoàn thành %d/3",i));
	...
	if ( who->get_legend(TASK_48,23) && !who->get_legend(TASK_48,24) && random(100)<40 )
	{
		...
		send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_NEWBIE_01,110,"Rất nhiều bệnh khác nhau（2）(Hoàn thành)" );
...
/*              else if( p < 4740 * rate / 100 )    // Ngân lượng
                {
