...
void check_legend_task( object who )
{
	...
	if ( who->get_legend(TASK_28,23) && !who->get_legend(TASK_28,24) && random(100) < 18 )
	{
		...
		if ( p )
		{
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_WEIGUO,30,"Ngọc ở mộ cổ(Hoàn thành)" );	
	...
	if( who->get_legend(TASK_29, 13) && !who->get_legend(TASK_29, 14) && who->get_save("wgswshihu") < 10 )
	{
		...
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_WEIGUO,34,"Điều kiện để làm bảo vệ(Hoàn thành)" );
	...
	if( who->get_legend(TASK_29, 18) && !who->get_legend(TASK_29, 19) && who->get_save("qzgshihu") < 30 )
	{
		...
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,27,"Làm bài kiểm tra(Hoàn thành)" );
	...
	if( who->get_legend(TASK_30, 24) && !who->get_legend(TASK_30, 25) && who->get_save("qgsrshihu") < 30 )
	{
		...
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,42,"Đơn độc trong mộ cổ(Hoàn thành)" );
...
/*              else if( p < 6416 * rate / 100 )    // Ngân lượng
                {
