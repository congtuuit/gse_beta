...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_07, 16) && !who->get_legend(TASK_07, 17) && who->get_save_2("wgjzhuyao") < 20)
	{
		...
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_WEIGUO,9,"Thảm họa của Ngụy quản gia（2）(Hoàn thành)" );
	...
	if( who->get_legend(TASK_07, 25) && !who->get_legend(TASK_07, 26) && random(100) < 23)
	{
		...
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_CHUGUO,11,"Ba lông(Hoàn thành)" );
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                else if( p < 9220 * rate / 100 )    // Ngân lượng
                {
