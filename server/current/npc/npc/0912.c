...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_31, 18) && !who->get_legend(TASK_31, 19) && who->get_save("cfxieling") < 7 )
	{
		...
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,39,"Quái vật(Hoàn thành)" );
	...
	{
		if ( objectp( item = present("Vải thiều", who, 1, MAX_CARRY) ) && item->is_legend_item() == 1 ) 
		{
		...
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YANGUO,53,"Việc vặt（2）(Hoàn thành)" );
			who->set_legend(TASK_32, 7);
		}

		USER_TASK_D->send_task_intro(who,2,TID_YANGUO,53);
		send_user(who,"%c%s",'!',sprintf(HIY"Vải thiều %d/10",i));		
	}
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                else if( p < 6695 * rate / 100 )    // Ngân lượng
                {
