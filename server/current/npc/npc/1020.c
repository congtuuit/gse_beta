...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_36, 3) && !who->get_legend(TASK_36, 4) )
	{
		...
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,51,"完美宝石（2）(Hoàn thành)" );
	...
	if( who->get_legend(TASK_36, 6) && !who->get_legend(TASK_36, 7) )
	{
		...
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,52,"完美宝石（3）(Hoàn thành)" );
	...
	if( who->get_legend(TASK_35, 27) && !who->get_legend(TASK_35, 28) && who->get_save("qgsrguanzx") < 6 )
	{
		...
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,50,"Ác ma trong lọ与Quán Trung Tiên(Hoàn thành)" );
	...
	if( who->get_legend(TASK_36, 11) && !who->get_legend(TASK_36, 12) && who->get_save("qgdjjguanzx") < 8 )
	{
		...
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,53,"Không sợ(Hoàn thành)" );
	...
	if( who->get_legend(TASK_36, 20) && !who->get_legend(TASK_36, 21) )
	{
		...
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,56,"Cuộc thám hiểm(Hoàn thành)" );
	...
	if( who->get_legend(TASK_36, 23) && !who->get_legend(TASK_36, 24) && who->get_save("qgjguanzx") < 10 )
	{
		...
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,57,"Kẻ phản bội(Hoàn thành)" );
	...
	if( who->get_legend(TASK_36, 26) && !who->get_legend(TASK_36, 27) )
	{
		...
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QIGUO,58,"Nổi loạn của Kỳ(Hoàn thành)" );
	...
	if( who->get_legend(TASK_37, 10) && !who->get_legend(TASK_37, 11) )
	{
		...
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_DONGHAIDAO,2,"Tiểu Đồ（4）(Hoàn thành)" );
...
//      p = random(100);  size = ( p > 40 ) ? 3 : 10;  total = 2;  total2 = 3;
        ...
        for( i = 0; i < size; i ++ )
        {
                ...
                else if( p < 7012 * rate / 100 )    // Ngân lượng
                {
