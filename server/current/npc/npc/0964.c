...
void check_legend_task( object who )
{
	...
	if( who->get_legend(TASK_33, 7) && !who->get_legend(TASK_33, 8) && who->get_save("cgswtujing") < 6 )
	{
		...
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_CHUGUO,35,"Cơ thể bận rộn(Hoàn thành)" );
	...
	{
		if ( objectp( item = present("Trà Quan Vũ", who, 1, MAX_CARRY) ) && item->is_legend_item() == 1 ) 
		{
		...
		if ( (i=who->add_save("cgswtiegy",1)) >= 20 )
		{
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_CHUGUO,36,"Trà Quan Vũ(Hoàn thành)" );
		}
		USER_TASK_D->send_task_intro(who,2,TID_CHUGUO,36);
		send_user(who,"%c%s",'!',sprintf(HIY"Trà Quan Vũ %d/20",i));		
	}
	if( who->get_legend(TASK_33, 13) && !who->get_legend(TASK_33, 14) && random(100) < 30 )
	{
		if ( objectp( item = present("Hoa hồng", who, 1, MAX_CARRY) ) && item->is_legend_item() == 1 ) 
		{
		...
		if ( (i=who->add_save("cgswmeigui",1)) >= 11 )
		{
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_CHUGUO,37,"Hoa hồng(Hoàn thành)" );
		}
		USER_TASK_D->send_task_intro(who,2,TID_CHUGUO,37);
		send_user(who,"%c%s",'!',sprintf(HIY"Hoa hồng %d/11",i));		
	}
	if( who->get_legend(TASK_33, 19) && !who->get_legend(TASK_33, 20) )
	{
		...
		if (  who->get_save("cgbxmuz") && who->get_save("cgbxpaoz") )
		{
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_CHUGUO,39,"Giữ khoảng cách(Hoàn thành)" );
		}
	...
	if( who->get_legend(TASK_33, 27) && !who->get_legend(TASK_33, 28) && who->get_save("lidetujing") < 30 )
	{
		...
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_WEIGUO,38,"Bằng chứng sức mạnh(Hoàn thành)" );
	...
	if( who->get_legend(TASK_35, 17) && !who->get_legend(TASK_35, 18) && who->get_save("wlhtujing1") < 30 )
	{
		...
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_WEIGUO,58,"Dũng cảm đột nhập vào hang động(Hoàn thành)" );
	...
	if( who->get_legend(TASK_39, 28) && !who->get_legend(TASK_39, 29) )
	{
		...
		if (  who->get_save("gsbtieku") && who->get_save("gsbmutou") >=5 )
		{
			...
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_ZHOUGUO,8,"四面楚歌（4）(Hoàn thành)" );
		}
	}
	if( who->get_legend(TASK_43, 23) && !who->get_legend(TASK_43, 24) && who->get_save("qggjtuj") < 5 )
	{
		...
		{
			send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_QINGUO,60,"Bóng tối trước bình minh(Hoàn thành)" );
	...
	if( who->get_legend(TASK_45, 26) && !who->get_legend(TASK_45, 27) && random(100) < 18)
	{
		...
		send_user( who, "%c%c%c%w%w%s", 0x51, 2, 2,TID_YUNMENZE,2,"Theo y tế （4）(Hoàn thành)" );
...
/*              else if( p < 6968 * rate / 100 )    // Ngân lượng
                {
