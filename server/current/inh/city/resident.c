...
void do_enter( object who, object me, string arg )
{
        ...
        else if( arg == "?" )
        {
                ...
                send_user( who, "%c%s", ':', "您Chấp nhận要加入" + name1 + "吗？\n" 
                        ESC "是的。\n" + cmd +
                        ESC "我再想想……\nCLOSE\n" );
        }
        else if( city = CITY_D->get_city_object(me) )
        {
                ...
                who->log_legend( sprintf("您入籍%s。", name2) );
                send_user( who, "%c%s", ':', sprintf( "%s：\n欢迎您成为%s的一份子。\n", me->get_name(), name2 ) );
...
void do_leave( object who, object me, string arg )
{
        ...
        if( arg == "?" )
        {
                ...
                send_user( who, "%c%s", ':', "您Chấp nhận要脱离" + name1 + "吗？\n" 
                        ESC "是的。\n" + cmd +
                        ESC "我再想想……\nCLOSE\n" );
        }
        else if( city = CITY_D->get_city_object(me) )
        {
                ...
                who->log_legend( sprintf("您脱离%s。", name1) );
                send_user( who, "%c%s", ':', sprintf( "%s：\n您的户籍已经注销了，现在您同%s没有任何关系了。\n", me->get_name(), name1 ) );
