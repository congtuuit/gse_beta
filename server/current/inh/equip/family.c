...
//			item->set("mp%",range_value(4*level,4,40));
			...
			item->set("ap%",range_value(4*level,4,40));
			item->set("hp*", range_value(2 * level, 3, 40));
//			item->set("!%",range_value(7*level,7,70)*100);
...
//			item->set("!%",range_value(10*level,10,100)*100);
			item->set("t!%",range_value(1*level,1,40));
			item->set("?%",range_value(4*level,4,40)*100);
			break;	
		case "Côn Luân":			
			item->set("cp%",range_value(4*level,4,40));
//			item->set("hp%",range_value(4*level,4,40));
			item->set("mp%",range_value(4*level,2,40));
//			item->set("c!%",range_value(4*level,4,40));
			break;												
		case "Mao Sơn":			
			item->set("hp%",range_value(4*level,4,40));
...
int get_use_effect_callout( object me, object item )
{
        ...
        {
                send_user(me, "%c%s", '!', "Người đeo mặt nạ xuất hiện và hành động của bạn bị ảnh hưởng。");
                return 0;
        }
	if( me->is_store_open() )
	{
                send_user(me, "%c%s", '!', "Không có thời gian");
                return 0;
        }	
...
//	        send_user( me, "%c%w%c%w", 0x50, 61, 0xff, 0 );              
        ...
        {
                write_user(me, ECHO "……Ngừng sử dụng.");
                return 0;
        }
}
