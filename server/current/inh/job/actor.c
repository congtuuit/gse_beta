...
void do_dance( object me, object who, string arg )
{
        ...
        if( me == who )
        {
                send_user(me, "%c%s", '!', "您不能对自己跳舞。");
                return;
        }
        if( who->is_die() )
        {
                send_user(me, "%c%s", '!', "您不能对死人跳舞。");
                return;
        }
...
void do_dance_finish( object me, object who, string arg )
{
        ...
        write_user( me, ECHO "您的艺人经验 +10, 忠实 +1, 勇敢 +10, 智谋 +10, 声望 +10, 国家贡献 +1, 经验 %+d, tiềm năng +1。", level + 10 );
}
