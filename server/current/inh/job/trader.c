...
void do_look( object who, object me )
{
	...
	if (time>5) info = "Sớm thôi，Tôi sẽ đưa một lô hàng trở lại trên kệ.。";
	else
	if (time>=2) info = "Chờ chút，Tôi sẽ đưa một lô hàng trở lại trên kệ.。";
	else info = "Tôi sẽ trở lại trên kệ sớm.。";
        if( who->is_businessman() || who->get_save_2("mastertask.type")==11 || who->get_save_2("orgtask3.type") )
                me->more_look_menu( who, sprintf( "%s：\n    "+"/quest/word"->get_normal_word(me)+"\n    这位%s，刚上市的特产，您来点什么吧？(买入物价: %d  卖出: %d) "+info+"\n",
                        ...
                        ESC "Mua bán hàng hóa。\n" + cmd1 +
//                        ESC "砍价。\n" + cmd2 +
                        ESC "Rời đi。\nCLOSE\n" );
        else    me->more_look_menu( who, me->get_name() + "：\nBây giờ là một doanh nhân là ngon.，Có thể đóng góp vào cửa hoặc bang phái，và bạn có thể làm cho tài sản nhỏ của riêng bạn.。\n" );
}
...
void do_cheque( object who, object me, string arg )
{
        ...
        &&      who->is_businessman() )
        {
                ...
                if( arg == "?" )
                {
                        ...
                        {
                                send_user(who, "%c%s", '!', "Bạn đã có vé của bạn.。");
                                return;
                        }
                        if( present("银票", who, 1, MAX_CARRY) )
                        {
                                send_user(who, "%c%s", '!', "Bạn đã có một vé bạc.。");
                                return;
                        }                        
                        if( sizeof_inventory(who, 1, MAX_CARRY) >= MAX_CARRY )
                        {
                                send_user(who, "%c%s", '!', "Anh đang mang quá nhiều.。");
                                return;
                        }

                        cmd = sprintf( ESC "talk %x# cheque.%%s\n", id );
                        send_user( who, "%c%c%d%s", '?', 1, who->get_cash(), "Bạn muốn nhận được bao nhiêu vé tiền chính?？Vui lòng nhập khoản tiền cụ thể：\n" + cmd );
                }
                else if( arg == "@" )
                {
                        if( !( obj = present("钱票", who, 1, MAX_CARRY) ) )
                        {
                                send_user(who, "%c%s", '!', "Bạn đã không nhận được vé tiền từ các thương gia.。");
                                return;
                        }
                        ...
                        if( gold2 > 0 )
                        {
                                ...
                                write_user( who, ECHO "Bạn cung cấp cho các vé tiền%s，%sThuế %+d，Đóng góp quốc gia của bạn %+d, Ngân lượng %+d。",
                        ...
                        else    // 亏本
                        {
                                ...
                                write_user( who, ECHO "Bạn cung cấp cho các vé tiền%s，Tiền của bạn %+d。", me->get_name(), gold );
                        }
                }
...
void do_good( object who, object me, string arg )
{
        ...
        if( arg == "?" )
        {
                ...
                send_user( who, "%c%s", ':', "您Chấp nhận要游说商人以影响物价指数吗？(消耗 100 商人经验)\n" 
                        ESC "是的。\n" + cmd +
                        ESC "我再想想……\nCLOSE\n" );
                return;
        }
...
void do_stir_up( object who, object me, string arg )
{
        ...
        &&    ( city = CITY_D->get_city_object(who) ) )
        {
                ...
                if( arg == "?" )
                {
                        ...
                        send_user( who, "%c%s", ':', "您Chấp nhận要造谣吗？(消耗 20 体力)\n" 
                                ESC "是的。\n" + cmd +
                                ESC "我再想想……\nCLOSE\n" );
                }
                else if( arg == "@" )
