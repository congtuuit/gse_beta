...
private static int DflSkill;		//默认Sử dụng技能(对其他人Sử dụng)
private static int DflSkill1;		//默认Sử dụng技能(对己Sử dụng)
...
int set_default_skill(int iSki)
{
	...
	if ( oSki->get_perform_type() == ON_ME_P )	//对己Sử dụng的法术
		DflSkill1=iSki;	
	else
		DflSkill=iSki;
