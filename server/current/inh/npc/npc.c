        ...
        case FIGHT_STAT: // 普通攻击, 特殊技(动作)
            ...
            if (who = me->get_enemy_3())
            {
                if (stringp(file = me->get_perform_file())
                    && load_object(file)
                    && file->perform_done_npc(me, who) == 2)    // 特技攻击(Hoàn thành)
                    ;

                else if (stringp(file = me->get_perform_2_file())
                    && load_object(file)
                    && file->perform_done_npc(me, who) == 2)    // 特技攻击(循环)(Hoàn thành)
                    ;
                ...
                else
                {
                    ...
                    ret = CHAR_FIGHT_D->attack_done(me, who, HIT_NORMAL);
