...
//                send_user( team[i], "%c%w", 0x0d, MUSIC_TASK );
                if (team[i]==who)
                	write_user( team[i], ECHO "您击毙%s，经验 %+d, tiềm năng %+d。", me->get_name(), exp2, pot2 );
                else
                	write_user( team[i], ECHO "您参与击毙%s的使命，经验 %+d, tiềm năng %+d。", me->get_name(), exp2, pot2 );
...
void drop_items( object me, object who )
{
	...
	rate = rate * who->get_online_rate() / 100;
        ...
        else if( p < 96 * rate / 100 )    // Ngân lượng
        {
