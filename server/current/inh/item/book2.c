...
int get_use_effect_callout(object me, object item)
{
	...
	int skill, level, exp, i, pf, level2, maxlevel, myLevel;
	object file, perform;
	...
	//tell_user(me, "level skill %d", me->get_skill(skill));
	//tell_user(me, "level %d", me->get_level());
	...
	myLevel = me->get_level();

	if (level > 90)
	{
		send_user(me, "%c%s", '!', book + "Thật không đáng để đọc.");
		return 0;
	}
