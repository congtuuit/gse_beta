	...
	if (count > 0)
	{
		...
		if (nPlayer)
		{
			if (map->get("win") == "Tần Quốc")
				send_user(nPlayer, "%c%c%d%s", 0x43, 10, 0, sprintf(HIR"Chiến trường kết thúc Tần Quốc giành thắng lợi, rời khỏi chiến trường sau %d giây.", count));
			else if (map->get("win") == "Triệu Quốc")
				send_user(nPlayer, "%c%c%d%s", 0x43, 10, 0, sprintf(HIR"Chiến trường kết thúc Triệu Quốc giành thắng lợi, rời khỏi chiến trường sau %d giây.", count));
		}
