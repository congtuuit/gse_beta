	...
	for (i = 0, size = sizeof(inv); i < size; i++)
		if (item = inv[i])
		{
			...
			case ITEM_TYPE_EQUIP:
				Inventory += ({sprintf("%s;%d;%d;%d;%d;%s;[%s]%d,%d",
			...
			case ITEM_TYPE_POISON_2:
				Inventory += ({sprintf("%s;%d;%d;%d;[%s]%d,%d",
			...
			case ITEM_TYPE_CHEQUE:
				Inventory += ({sprintf("%s;%d;%d;%d;%d;[%s]%d,%d",
			...
			case ITEM_TYPE_DIAMOND_3:
				...
				break;
			default:
				Inventory += ({sprintf("%s;%d;%d;[%s]%d,%d", file, amount, index, id, locktime, lockutil)});
				break;
			}
		}
}
...
void restore_inventory()
{
	...
	if (arrayp(Inventory) && sizeof(Inventory))
		for (i = 0, size = sizeof(Inventory); i < size; i++)
		{
			...
			{
			case ITEM_TYPE_EQUIP:
				if (sscanf(Inventory[i], "%s;%d;%d;%d;%d;%s;[%s]%d,%d", file, amount, index, use, maxuse, prop, id, locktime, lockutil) != 9)
					continue;
			...
			case ITEM_TYPE_TALISMAN:
				if (file->get_name() == "Đồng tâm khóa" && !me->get_save_2("marry"))
					continue;
				if (sscanf(Inventory[i], "%s;%d;%d;%d;%d;%d;[%s]%d,%d", file, amount, index, use, maxuse, point, id, locktime, lockutil) != 9)
					continue;
			...
			/*
							if( gone_time(locktime) > lockutil )    // 计算是否锁定
							{
