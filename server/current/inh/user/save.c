...
void prepare_save()
{
        ...
        else if ((name = me->get_org_name()) != "" && objectp(npc = CITY_ZHOU->get_2(sprintf("org.%s", name))))
        {
                me->set_start_place(sprintf("%d:(%d,%d)%d", get_z(npc), get_x(npc) + random(11) - 5, get_y(npc) + random(11) - 5, 3));
        }
        else if (!stringp(xyz = me->get_start_place()) || sscanf(xyz, "%d:(%d,%d)%d", z, x, y, d) != 4 || z == 0)
        {
...
int save()
{
        ...
        if (me->get("is_gm"))
                return 0;
        prepare_save();
        return db_user_save(me, me->get_id());
}
