...
varargs int set_exp(int exp, int bil_exp)
{
	...
	if (Level < 10 && UpgradeExp >= USER_EXP_D->get_level_exp(Reincarnation, Level))
	{
		...
		TEACHER_D->add_shifu_exp(me, 1);
		"/sys/user/huynhde"->phanthuong_huynhde(me);
		...
		else
		{
			if (me->get_org_name() != "")
			{
				tell_user(me, HIR "Cập nhật điểm bang phái");
			}
		}

		"/quest/zg_dict"->player_can_be_gala(me, 1);
	}
	else if (Level < 11 && UpgradeExp >= USER_EXP_D->get_level_exp(Reincarnation, Level))
	{
		...
		{
			notify("Có thể thăng cấp bằng cách nhấn Thăng Cấp trong bảng thông tin nhân vật");
			status |= 32;
...
int Upgrade()
{
	...
	if (Level < get_max_char_level() && UpgradeExp >= sub_exp)
	{
		...
		TEACHER_D->add_shifu_exp(me, sub_exp);
		"/sys/user/huynhde"->phanthuong_huynhde(me);
...
int add_exp(int exp)
{
	...
	if (me->get("doubleTapKich") == 1)
	{
		exp += exp;
	}

	if (me->get("title_exp") > 0)
	{
		exp = exp + ((exp / 100) * 5);
	}

	set_upgrade_exp(UpgradeExp + exp);
...
// 函数：获取最大tiềm năng
int get_max_potential() { return range_value(Level * 2, 1, MAX_NUMBER) * 1000; } // 注意新登录情况
...
// 函数：获取ＰＫ标志
int get_pk() { return range_value(PK, 0, 9999); } // Cảnh báo : phòng ngựa tình huống xảy ra
...
int set_pk(int flag)
{
	...
	{
		send_user(me, "%c%c%w%s", ':', 3, 0, "Bởi vì ngươi nghiệp chứng nặng nề, giết người không gớm tay, nên ngươi phải chịu hình phạt !\n" ESC "Xác nhận\nCLOSE\n");
	}
	else if (old_pk > 0 && PK == 0)
	{
		send_user(me, "%c%c%w%s", ':', 3, 0, "Ngươi đã sám hồi rồi, tốt lắm ! hãy đến gần hơn với Hoà bình nha !\n" ESC "Xác nhận\nCLOSE\n");
	}
	else if (PK > old_pk)
	{
		set_effect(me, EFFECT_USER_PK, 400);
		me->set_attack_mode(0);
	}
	// if (PK>=5) me->set_attack_mode(0);
	if (PK < 5 && me->get_attack_mode() == 0)
	{
		//               me->set_char_type(PK_TYPE);    // Trạng thái chủ động PK
		me->set_char_type(PLAYER_TYPE_2); // Trạng thái chủ động PK
	}
	else
		switch (PK)
		{
		case 0:
			...
			break;
		case 1..2:
			me->set_char_type(PK_TYPE_1);
			break;
		case 3..4:
			me->set_char_type(PK_TYPE_3);
			break;
		default:
			me->set_char_type(PK_TYPE_5);
			break;
		}

	return range_value(PK, 0, 9999); // Cảnh báo : phòng ngựa tình huống xảy ra
}
