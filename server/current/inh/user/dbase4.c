...
mixed get_pawn(string key)
{
        if (strchr(key, '.') == -1)
                ...
                return _get(mpPawn, explode(key, "."));
}
...
{
        if (strchr(key, '.') == -1)
                return mpPawn[key] = value;
        else
                return _set(mpPawn, explode(key, "."), value);
}
...
{
        if (strchr(key, '.') == -1)
                map_delete(mpPawn, key);
        else
                _delete(mpPawn, explode(key, "."));
}
