...
void add_friend(object who, int type)
{
	...
	else if (who->get_save_2("baishi.tudi.id") == me->get_number())
		note->Name = sprintf("%s (Sư phụ)", who->get_name());
	else if (who->get_save_2("baishi.shifu.id") == me->get_number())
	...
	result = sprintf("%-6s%-10s%s", note->Family, note->Org, note->Name);
...
void refresh_friend()
{
	...
	send_user(me, "%c%c%d%c%c%c%c%w%s", 0x46, 1, 0, 0, 0, 2, 8, 100, "");
}
...
void add_black(string id, string name)
{
	...
	if (Black != 0)
	{
		...
		if (size >= 50)
		{
			notify("Danh sách đen không thể vượt quá 50 người.");
			return;
		}
	}
