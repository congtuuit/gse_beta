...
string set_occupation(string name)
{
        if (!name || name == "")
                ...
                send_user(this_object(), "%c%c%s", 0x3d, 51, name);
...
string set_title(string name)
{
        if (!name || name == "")
                ...
                send_user(this_object(), "%c%c%s", 0x3d, 52, name);
        return Title = name;
}
...
int set_gift_ra(int gift)
{
        ...
        if (GiftRA != gift && get_login_flag() > 1)
                send_user(me, "%c%c%d", 0x3d, 60, gift);
        return GiftRA = gift;
}
...
{
        object me = this_object();
        if (GiftZY != gift && get_login_flag() > 1)
                send_user(me, "%c%c%d", 0x3d, 61, gift);
        return GiftZY = gift;
}
...
{
        object me = this_object();
        if (GiftZS != gift && get_login_flag() > 1)
                send_user(me, "%c%c%d", 0x3d, 62, gift);
        return GiftZS = gift;
}
...
{
        object me = this_object();
        if (GiftCX != gift && get_login_flag() > 1)
                send_user(me, "%c%c%d", 0x3d, 63, gift);
        return GiftCX = gift;
}
...
{
        object me = this_object();
        if (GiftYG != gift && get_login_flag() > 1)
                send_user(me, "%c%c%d", 0x3d, 64, gift);
        return GiftYG = gift;
}
...
{
        object me = this_object();
        if (GiftZM != gift && get_login_flag() > 1)
                send_user(me, "%c%c%d", 0x3d, 65, gift);
        return GiftZM = gift;
}
...
{
        object me = this_object();
        if (Repute != point && get_login_flag() > 1)
                send_user(me, "%c%c%d", 0x3d, 66, point);
        return Repute = point;
}
...
{
        object me = this_object();
        if (Contribute != point && get_login_flag() > 1)
                send_user(me, "%c%c%d", 0x3d, 67, point);
...
{
        object city;
        if (city = CITY_D->get_city_object(this_object()))
                city->add_contribute(point);
...
int set_strength(int point)
{
        ...
        if (point > MAX_STRENGTH)
                point = MAX_STRENGTH;
        if (Strength != point && get_login_flag() > 1)
                send_user(me, "%c%c%c", 0x3d, 68, point);
...
int set_occupation_level(int level)
{
        ...
        if (undefinedp(Jobs[name]))
                job = new(class OccupationData);
        else
                job = Jobs[name];
