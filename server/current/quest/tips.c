...
mapping MapTipsTable = ([
	 ...
	 //"1003" : "请将" HIY "百合仙子的信件" NOR "交给" HIY "神仙爷爷" NOR "。这样你便可以Hoàn thành百合仙子的委托了。\n",
...
void enter_scene_tip(object who, object map)
{
	...
	if (level1 > level)
	{
		...
		if (sizeof(cTmp))
			send_user(who, "%c%s", ';', "Vì suy nghĩ cho sự an toàn của bạn, đề nghị bạn đi thích hợp đẳng cấp hiện tại " + HIR + cTmp + NOR + " Vùng luyện cấp.");
	}
	else
	{
		if (who->get_level() < 40)
		{
			who->set_attack_mode(1);
		}
	}

	// check pk 39
	/*if (who->get_level() + 19 < level1 && who->get_level() > 30 && who->get_level() < 40) {
		notify(HIY"Khu vực này có thể nguy hiểm cho bạn!");
		who->set_attack_mode(0);
		tell_user(who, HIR"Bạn có thể bị người khác tấn công, không thể đánh trả!");
	}*/

	if (get_z(who) == 893)
	{
		who->set_attack_mode(0);
		tell_user(who, ECHO "Đã bật chế độ tấn công!");
	}
