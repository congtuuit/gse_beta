...
//每次回答正确以后给予的经验和tiềm năng奖励
void exp_potential_bonus(object player)
{
	int level,exp_bonus,potential_bonus;
	int times = player->get_save_2("question.number");//现在Hoàn thành的题目数。
	...
	if ( player->get_online_rate() == 50 )//三小时到五小时
	{
		...
		send_user( player, "%c%s", ';', sprintf("由于你正确作答，获得了%d经验，%dtiềm năng的奖励！",exp_bonus,potential_bonus)); 
	}
	...
	{
		send_user(player,"%c%s",';',"不健康游戏时间无经验和tiềm năng。");	
	}
...
void feed_back(object player, object npc,int fail_tags)
{
	...
	times = player->get_save_2("question.number");//现在Hoàn thành的题目数。
