...
string get_random_fam_level_equip(string family, int gender, int level)
{
	int level3, weapon, armor, rand;
	string file;
        ...
        if (level<20) level3 = 20;
       	else
        	level3 = ( level / 10 ) * 10;
        if (level3>120) level3 = 120;	
        switch(family)
        {
        case "":
        	weapon = 1;
	...
	case 0:
	       ...
	       default: file = sprintf( "/item/70/1%03d", level3 );  break;
		}		
		break;
	case 1:
		armor = weapon ;
