...
int main(object me, object who, int x, int y, string arg)
{
	...
	if (who->is_yutu() && who->get_name() == "Cương Thi") {
		if (!objectp(item = present("Bùa Hàng Ma", me, 1, MAX_CARRY * 4)))
		{
			printf(ECHO "Phải có Bùa Hàng Ma mới bắt được %s.", who->get_name());
			return 1;
		}
	}

	if (!who->is_yutu() && !objectp(item = present("Dây Càn Khôn", me, 1, MAX_CARRY * 4)))
	{
		printf(ECHO "Phải có Dây Càn Khôn mới được sử dụng “" PF_NAME "”.");
		return 1;
	}

	if (who->is_yutu() && who->get_name() == "Thỏ Ngọc") {
		if (!objectp(item = present("Dây Bắt Thỏ", me, 1, MAX_CARRY * 4)))
		{
			printf(ECHO "Phải có Dây Bắt Thỏ mới được sử dụng “" PF_NAME "”.");
			return 1;
		}
	}
	...
	if (who->is_yutu() == 1 && who->get_name() == "Cương Thi") {
		rate = 90;
	}

	if (random(100) > rate && who->is_yutu() == 1 && who->get_name() == "Cương Thi")
	{
	...
	rate = 75 + (100 - who->get_hp() * 100 / who->get_max_hp()) / 2;

	if (USER_BEAST_D->capture_beast(me, who) != 1 && random(100) > rate)
	{
