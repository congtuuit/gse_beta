...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return HIR + PF_NAME +"\n Võ công yêu cầu: 44 cấp\n Vật phẩm tiêu hao: 4 lá Bùa Giấy\n    Triệu hồi Hắc Ám Thần Long tấn công đối thủ, tăng thêm 100 điểm Nội Công và tăng 5%% khả năng ra đòn Bạo Kích.";
        cur_level = (level - SKILL_LEVEL)/19 + 1;

        dame = ADD_AP + (me->get_mp() / 100 * 10);
        
        result = sprintf(HIC" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Vật phẩm tiêu hao: 4 lá Bùa Giấy\n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n    Triệu hồi Hắc Ám Thần Long tấn công đối thủ, tăng thêm %d (+ chuyển hóa 10%% pháp lực thành sát thương) điểm Nội Công và tăng %d%% khả năng ra đòn Bạo Kích.\n " HIC"Cấp tiếp theo:\n "NOR"Võ công yêu cầu: " HIR "Cấp %d " NOR"\n    Tăng thêm %d điểm Nội Công，và tăng thêm %d%% khả năng Bạo kích.",
        	PF_NAME, cur_level, SKILL_LEVEL, SUB_MP, TIME_INTERVAL, dame, (ADD_RATE)/100, cur_level*19+SKILL_LEVEL, dame + 60, (ADD_RATE)/100 + 5);
        return result;
}
...
int cast_done( object me )
{
    ...
    dame = ADD_AP + (me->get_max_mp() / 100 * 20);

	rate = me->get_double_rate_2();
	me->set_double_rate_2(rate+ADD_RATE);
        CHAR_FIGHT_D->cast_done(me, who, dame);    // 小心准备 me
        if ( who && !who->is_die() )
