...
#define SUB_EFFECT      ( 50 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 5 * 10 )
#define SUB_EFFECT2      ( 30 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 5 * 5 )
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return sprintf(HIR" %s \n Võ công yêu cầu: Cấp 5\ Bùa chú ăn mòn hộ giáp, giảm 50 điểm Ngoại Kháng và giảm 30 điểm Nội Kháng của đối thủ. Hiệu quả duy trì liên tục trong 2 phút.", PF_NAME);
