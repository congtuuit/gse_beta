...
#define TIME_INTERVAL   20 - ( me->get_skill(THIS_SKILL)- SKILL_LEVEL) / 10 * 2
...
string get_desc( object me )
{
        ...
        int cur_level, timeInterval;
        string result;
        if (level<SKILL_LEVEL)
        	return HIR + PF_NAME +"\n Võ công yêu cầu: 22 cấp\Vật phẩm tiêu hao: 1 lá Bùa Giấy\nn    Phát ra nội công hàn băng để tấn công mục tiêu từ xa, tăng thêm 30 điểm Nội Công khi sát thương và làm mục tiêu bị Chậm Chạp trong 6 giây";
        cur_level = (level - SKILL_LEVEL)/10 + 1;

        timeInterval = TIME_INTERVAL;
        if(timeInterval < 12) { timeInterval = 12;}
        result = sprintf(HIC" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Vật phẩm tiêu hao: 1 lá Bùa Giấy\n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n    Phát ra nội công hàn băng để tấn công mục tiêu từ xa, tăng thêm %d điểm Nội Công khi sát thương và làm mục tiêu bị Chậm Chạp trong 6 giây.\n " HIC"Cấp tiếp theo:\n "NOR"Võ công yêu cầu: " HIR "Cấp %d " NOR"\n    Tăng thêm %d điểm Nội Công.",
        	PF_NAME, cur_level, SKILL_LEVEL, SUB_MP, timeInterval, ADD_AP, cur_level*10+SKILL_LEVEL, ADD_AP + 15 );
        return result;
}
...
void can_perform( object me )
{
	...
	name = sprintf(" %s (Cấp %d )", PF_NAME, (me->get_skill(THIS_SKILL)-SKILL_LEVEL)/10+1);
        ...
        timeInterval = TIME_INTERVAL;
        if(timeInterval < 12) { timeInterval = 12;}
        interval = timeInterval - gone_time( me->get_perform(PF_START) );
        if( interval < 0 )
