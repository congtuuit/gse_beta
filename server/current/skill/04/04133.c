...
#define INIT_PP         me->get_pf_value(THIS_SKILL, SKILL_LEVEL_SQUARE, 122, 60)    // 警告：me 得准备好
...
/*
        if( p = get_valid_xy(z, x0 + random(3) - 1, y0 + random(3) - 1, IS_CHAR_BLOCK) )
        {
                ...
                cp = me->get_cp();
                pp = me->get_pp();
                npc->add_max_hp( (cp + pp) * 8000000000000 / 100 );
                npc->add_ap( cp * 80 / 10000 );
                npc->add_dp( pp * 2 / 5000 );
                npc->add_cp( cp * 50 / 10000 );
                npc->add_pp( pp * 40 / 10000 );
                npc->add_sp( cp / 10000 );
