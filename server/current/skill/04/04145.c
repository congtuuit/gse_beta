...
int cast_done(object me)
{
	...
	string arg;
	int z, x, y;
	int level, ret, flag = 0, damage, addEffect;
	...
	addEffect = me->get_max_hp() / 100;

	damage = ADD_EFFECT;

	damage = (damage / 10) + addEffect;

	if (who->is_user()) {
		//damage = damage + ((who->get_hp() / 1000) * 5);
		damage = damage + (who->get_max_hp() / 1000) + (me->get_skill(0432) * 50);
	}

	if (-who->get("pf.fall") > damage) return 2;
	...
	who->set("pf.fall", -damage);    // 小心准备 me
	if (who->is_user()) who->set_save_2("pf.fall", -damage);
