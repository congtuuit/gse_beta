...
#define SUB_MP          ( 30 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 10 * 20 )
#define ADD_AP          ( 70 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 10 * 25 )
#define ADD_CP          ( 40 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 10 * 15 )
#define ADD_DAMAGE      ( 70 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 10 * 4 )
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return HIR + PF_NAME +"\n Võ công yêu cầu: 18 cấp\Vật phẩm tiêu hao: 3 lá Bùa Giấy\nn    Triệu hồi Hoả Long Liệt Diễm vô tình thiêu đốt kẻ địch, tăng thêm 30 điểm Nội Công khi sát thương và làm mục tiêu bị Thiêu Đốt trong 10 giây, mỗi 1 giây mất 30 điểm Khí Huyết";
