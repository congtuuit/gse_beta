...
string get_desc(object me)
{
	...
	if (level < SKILL_LEVEL)
		return sprintf(HIR " %s \n Võ công yêu cầu: Cấp 6\n Côn pháp, tăng thêm 20 điểm Ngoại Công và sát thương 8 hướng xung quanh bản thân.", PF_NAME);
	cur_level = (level - SKILL_LEVEL) / 10 + 1;
	result = sprintf(HIC " %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Nguyên khí tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n Côn pháp tăng thêm %d điểm Ngoại Công và sát thương 8 hướng xung quanh bản thân. Sau khi luyện tập Phong Ma Côn Pháp đạt cấp 61 thì có thể tấn công 25 mục tiêu xung quanh.\n " HIC "Cấp tiếp theo:\n " NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Tăng thêm %d điểm Ngoại Công.",
...
int perform_done(object me, object who)
{
	...
	if (me->get_skill(THIS_SKILL) >= 61)
		char = get_range_object_2(me, 4, USER_TYPE) + get_range_object_2(me, 4, CHAR_TYPE) - ({me});
	else
...
int perform_done_npc(object me, object who)
{
	...
	char = get_range_object_2(me, 1, USER_TYPE) - ({me});
