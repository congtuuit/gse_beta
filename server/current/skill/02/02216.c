...
#define ADD_AP		( 500 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 10 * 140 )
...
string get_desc(object me)
{
	...
	int cur_level, percent;
	string result;
	if (level < SKILL_LEVEL)
		return HIR + PF_NAME + "\n Võ công yêu cầu: 35 cấp\n    Tuyệt kĩ Đao pháp tối thượng, tăng thêm 500 (+1% điểm ngoại công) điểm sát thương khi tấn công tất cả mục tiêu trong phạm vi tùy ý trước mặt.";
	cur_level = (level - SKILL_LEVEL) / 10 + 1;
	percent = me->get_ap() / 100;
	result = sprintf(HIC" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n    Tuyệt kĩ Đao pháp tối thượng, tăng thêm %d (+%d) điểm sát thương khi tấn công tất cả mục tiêu trong phạm vi tùy ý trước mặt.\n " HIC"Cấp tiếp theo:\n "NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Tăng thêm %d sát thương.",
		PF_NAME, cur_level, SKILL_LEVEL, SUB_MP, TIME_INTERVAL, ADD_AP, percent, cur_level * 10 + SKILL_LEVEL, ADD_AP + 250);
	return result;
}
...
int cast_done(object me)
{
	...
	string arg;
	int z, x, y, dx, dy, p;
	int level, ap, damage, damage2, i, size, count, percent;
	int* dxs, * dys;
	...
	for (i = 0, size = sizeof(char); i < size; i++) if (who = char[i])
	{
		...
		percent = me->get_ap() / 100;
		CHAR_FIGHT_D->attack_done(me, who, HIT_BLADE, ADD_AP + percent);
		if (who && !who->is_die())
