...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return sprintf(HIR" %s \n Võ công yêu cầu: Cấp 3\n Pháp thuật diệt trừ ma đạo, tăng thêm 35 (+2%% máu tối đa của bản thân) điểm Nội Công khi sát thương 1 mục tiêu bất kì trong tầm nhìn.", PF_NAME);
...
int main( object me, object who, int x, int y, string arg )
{
        ...
        me->to_front_xy(get_x(who), get_y(who)); //Thiết lập me的面向坐标.
...
int cast_done( object me )
{
	...
	send_user( get_scene_object_2(who, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(who), 27112, 1, OVER_BODY, PF_ONCE );
        ...
        dame = me->get_max_hp() / 100 * 2;

        CHAR_FIGHT_D->cast_done(me, who, ADD_AP + dame);    // 小心准备 me
        if ( who && !who->is_die() )
