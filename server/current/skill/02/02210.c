...
#define THIS_PERFORM2    02516

#define SKILL_LEVEL     1
#define TIME_INTERVAL   1
#define SUB_MP          6 +  (me->get_skill(THIS_SKILL) - SKILL_LEVEL)/10*3
#define ADD_AP          100 + (me->get_skill(THIS_SKILL) - SKILL_LEVEL)/10*10
...
string get_desc(object me)
{
	...
	result = sprintf(HIC" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n    Chiêu thức đao pháp cơ bản, gia tăng %d sát thương khi tấn công một mục tiêu.\n " HIC"Cấp tiếp theo:\n "NOR"Võ công yêu cầu: " HIR "Cấp %d " NOR"\n    Tăng thêm %d sát thương.",
...
int main(object me, object who, int x, int y, string arg)
{
	...
	send_user(me, "%c%w%c%w", 0x50, THIS_PERFORM2, 0xff, 0);
	me->set_perform(PF_START, time());
