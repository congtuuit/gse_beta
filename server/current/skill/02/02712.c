...
#define RANGE_SKILL_LEVEL     5
#define TIME_INTERVAL   2
#define SUB_MP          ( 20 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 15 * 15 )
#define ADD_CP          ( 100 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 15 * 10 )
...
string get_desc(object me)
{
	...
	if (level < SKILL_LEVEL)
		return sprintf(HIR " %s \n Võ công yêu cầu: 25 cấp\n Pháp Lực tiêu hao: 20 điểm\n Phạm vi: 6x6 \n Tái sử dụng sau: 2 giây\n Thần lực hiệu triệu Thiên Lôi trong 10 giây liên tục dùng lôi điện gây sát thương.Tăng 50 Nội Công sát thương với tất cả mục tiêu và có 30%% cơ hội làm choáng 1 giây.", PF_NAME);
...
void into_effect(object me, string arg)
{
	...
	char = get_range_object(z, x, y, RANGE_SKILL_LEVEL, USER_TYPE) + get_range_object(z, x, y, RANGE_SKILL_LEVEL, CHAR_TYPE) - ({ me });
	count = 0;
	for (i = 0, size = sizeof(char); i < size; i++) if (who = char[i])
	{
		if (!who->can_be_fighted(me) || !me->can_fight(who)) continue;
		count++;
		if (count <= numberEffect)
			send_user(get_scene_object_2(who, USER_TYPE), "%c%d%w%c%c%w%c%c%c", 0x6f, getoid(who),
				42321, 3, OVER_BODY, 42321, numberEffect, OVER_BODY, PF_ONCE);
