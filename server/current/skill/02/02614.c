...
#define SUB_EFFECT      5 + (me->get_skill(THIS_SKILL) - SKILL_LEVEL) /10
...
string get_desc( object me )
{
        ...
        if (level<SKILL_LEVEL)
        	return sprintf(HIR " %s \n Võ công yêu cầu: Cấp 12\n Ám khí xuyên thấu hộ giáp của đối thủ, giảm 20%% điểm Ngoại Kháng trong 20 giây và làm mục tiêu mất tổng cộng 1000 điểm Khí Huyết trong 12 giây.", PF_NAME);
        ...
        result = sprintf(HIC" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n Ám khí xuyên thấu hộ giáp của đối thủ, giảm %d %% điểm Ngoại Kháng trong 20 giây và làm mục tiêu mất tổng cộng %d điểm Khí Huyết trong 12 giây.\n " HIC"Cấp tiếp theo:\n "NOR "Võ công yêu cầu: "HIR "Cấp %d \n" NOR "    Ám khí xuyên thấu hộ giáp của đối thủ, giảm %d %% điểm Ngoại Kháng trong 300 giây và làm mục tiêu mất tổng cộng %d điểm Khí Huyết trong 12 giây.", 
        	PF_NAME, cur_level, SKILL_LEVEL, SUB_MP, TIME_INTERVAL, SUB_EFFECT, SUB_BLOOD, cur_level*10+SKILL_LEVEL, SUB_EFFECT + 1, SUB_BLOOD + 18);
        return result;
}
...
int cast_done( object me )
{
        ...
        int z, x, y;
        int level, ret;
        int interval, sub_effect, sub_dp;
	...
	if (ret>0)
	{
        ...
        sub_effect = who->get_dp() / 100 * SUB_EFFECT;
...
int cast_done_npc( object me )
{
	...
	if (ret>0)
	{
        ...
        sub_effect = who->get_dp() / 100 * SUB_EFFECT + SUB_BLOOD;
