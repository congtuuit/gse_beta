...
#define TIME_INTERVAL   13
#define SUB_MP          26 + (me->get_skill(THIS_SKILL) - SKILL_LEVEL)/10*6
#define ADD_DAMAGE	( 30 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 10 * 7 )
...
string get_desc(object me)
{
	...
	if (level < SKILL_LEVEL)
		return HIR + PF_NAME + "\n Võ công yêu cầu: 18 cấp\n    Truy sát đao thức, chỉ trong chớp mắt di chuyển đến sau lưng mục tiêu gây choáng 3 giây, tăng 10 điểm sát thương và tăng 30% độ chính xác.";
	cur_level = (level - SKILL_LEVEL) / 10 + 1;
	result = sprintf(HIC" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n    Truy sát đao thức, chỉ trong chớp mắt di chuyển đến sau lưng mục tiêu gây choáng 3 giây, tăng %d điểm sát thương và tăng %d%% chính xác.\n " HIC"Cấp tiếp theo:\n "NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Tăng thêm %d điểm sát thương và tăng %d%% độ chính xác.",
		PF_NAME, cur_level, SKILL_LEVEL, SUB_MP, TIME_INTERVAL, ADD_AP, ADD_DAMAGE, cur_level * 10 + SKILL_LEVEL, ADD_AP + 10, ADD_DAMAGE + 5);
	return result;
}
...
int main(object me, object who, int x, int y, string arg)
{
	...
	CHAR_FIGHT_D->attack_done(me, who, HIT_BLADE, ADD_AP, 10, ADD_DAMAGE);
