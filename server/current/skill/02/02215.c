...
#define PERCENT         (2 + (me->get_skill(THIS_SKILL) - SKILL_LEVEL)/10)
...
string get_desc(object me)
{
	...
	if (level < SKILL_LEVEL)
		return HIR + PF_NAME + "\n Võ công yêu cầu: 22 cấp\n    Đao thế như núi lở nặng ngàn cân, tăng thêm 30 sát thương và 2% máu còn lại của mục tiêu khi tấn công và làm cho đối thủ bị Choáng trong 3 giây.";
	cur_level = (level - SKILL_LEVEL) / 10 + 1;
	result = sprintf(HIC" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Pháp Lực tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n    Đao thế như núi lở nặng ngàn cân, tăng thêm %d sát thương và %d máu còn lại của mục tiêu khi tấn công, làm cho đối thủ bị Choáng trong 3 giây.\n " HIC"Cấp tiếp theo:\n "NOR"Võ công yêu cầu: " HIR "Cấp %d " NOR"\n    Tăng thêm %d điểm Ngoại Công,Thời gian tái sử dụng giảm còn %d giây.",
		PF_NAME, cur_level, SKILL_LEVEL, SUB_MP, TIME_INTERVAL, ADD_AP2, PERCENT, cur_level * 10 + SKILL_LEVEL, ADD_AP2 + 10, TIME_INTERVAL - 1);
	return result;
}
...
int main(object me, object who, int x, int y, string arg)
{
	...
	if (who->is_user()) {
		bonus = (who->get_hp()/100 * PERCENT) + ADD_AP2;
		ret = CHAR_FIGHT_D->attack_done(me, who, HIT_BLADE, bonus);
	}
	else {
		ret = CHAR_FIGHT_D->attack_done(me, who, HIT_BLADE, ADD_AP2);
	}
