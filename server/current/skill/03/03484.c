...
#define SUB_MP          0
#define SUB_YUAN        4
#define ADD_TIME	10 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 15 * 2
#define ADD_AP		30 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 15 * 40
#define ADD_DP		60 + ( me->get_skill(THIS_SKILL)-SKILL_LEVEL) / 15 * 70
...
string get_desc( object me )
{
        ...
        result = sprintf(HIC" %s (Cấp %d )\n " NOR "Võ công yêu cầu: Cấp %d \n Nguyên khí tiêu hao: %d điểm\n Tái sử dụng sau: %d giây\n    Trong %d giây tăng thêm %d điểm Ngoại Công và %d điểm Ngoại Kháng.\n " HIC"Cấp tiếp theo:\n "NOR "Võ công yêu cầu: " HIR "Cấp %d " NOR "\n    Trong %d giây tăng thêm %d điểm Ngoại Công và %d điểm Ngoại Kháng.Thời gian tái sủ dụng giảm còn %d giây.",
        	PF_NAME, cur_level, SKILL_LEVEL, SUB_YUAN, TIME_INTERVAL, ADD_TIME, ADD_AP, ADD_DP, cur_level*15+SKILL_LEVEL, ADD_TIME + 2, ADD_AP + 40, ADD_DP + 70, TIME_INTERVAL - 5 );
	return result;
}
