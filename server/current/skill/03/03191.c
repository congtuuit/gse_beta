...
#define ADD_AP		50 + (me->get_skill(THIS_SKILL) - SKILL_LEVEL)/5*15
...
string get_desc(object me)
{
	...
	if (level < SKILL_LEVEL)
		return HIR + PF_NAME + "\n Võ công yêu cầu: 10 cấp\n  Tự đưa bản thân vào trạng thái Cuồng.Tăng 10% Ngoại Công hiện tại và tăng thêm 400 điểm Ngoại Công, tuy nhiên Ngoại Kháng sẽ giảm xuống 50%. Nếu lượng máu giảm sẽ tăng thêm chỉ số ngoại công, tối đa 50%%.";
...
int main(object me, object who, int x, int y, string arg)
{
	...
	if (me->get_perform(PF_FLAG))    // 关闭
	{
		...
		set_effect(me, EFFECT_PROGRESS2, watting);
		me->set_progress_file(__FILE__);
		send_user(me, "%c%c%w%s", 0x5a, 0, watting, "Thi triển Cuồng Bạo……");
		return 1;
	}
...
int into_effect(object me, string arg)
{
	...
	{
		me->set_2("03191Root", me->get_ap() + ADD_AP);
