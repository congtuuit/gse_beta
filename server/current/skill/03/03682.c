...
#define PF_NAME        "Diệu Thủ Không Không"
...
string get_desc( object me )
{
        ...
        return sprintf( "［" PF_NAME "］Rình coi mục tiêu mang theo vật phẩm nào rồi ăn cắp luôn。 Pháp lực tiêu hao：%d， Tái sử dụng sau %d giây，%s%d cấp có thể dùng\n",
...
//      send_user( get_scene_object_2(who, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(who), 36821, 1, OVER_BODY, PF_ONCE );
        ...
        if( random(100) < range_value(rate, 0, 100) )
        {
                ...
                &&     !item->is_locked() )
                {
                        ...
                        if( item->is_combined() )
                        {
                                send_user(me, "%c%s", '!', sprintf( "Bạn ăn cắp thành công đạt được %s。", item->get_name() ) );
        ...
        {
        	write_user(who, ECHO "Bạn cảm thấy dường như ba lô nhẹ lại……");
        	write_user(me, ECHO "Kỹ năng sử dụng thất bại……");
        }
