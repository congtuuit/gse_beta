...
int get_use_effect_callout(object who, object gift)
{
	...
	{
		//So tri duoc khoa
		move_item_to_user(who, 2, "/item/test2/block/0007");

	}
	else if (rand < 200)
	{
		//Xiem
		move_item_to_user(who, 1, "/item/sgnv/xiem");
	}
	else if (rand < 300)
	{
		//So phuc duoc khoa
		move_item_to_user(who, 2, "/item/test2/block/0009");
	}
	else if (rand < 400)
	{
		//Tui phap bao
		move_item_to_user(who, 1, "/item/test2/TuiPhapBao");
	}
	else if (rand < 500)
	{
		//An xa phu
		move_item_to_user(who, 1, "/item/test2/AXP");
	}
	...
	else if (rand < 600)
	{
		...
		if (p = item->move(who, -1))
		{
			...
			USER_D->user_channel("Chúc mừng "HIY + who->get_name() + NOR" mở túi quà Tết Trung Thu nhận được " + HIR + item->get_name() + ".\n");
	...
	{
		//Exp
		who->add_exp(who->get_level() * 100 * random(50));
	}
	else 
	{
		//Tiem nang
		who->add_potential(200 + random(1000));
	}
