...
string get_desc()
{
	return "Phong bao lì xì đầu năm mới Chiến Quốc, sử dụng sẽ nhận được các phần quà ý nghĩa.";
}
...
int get_use_effect_callout(object me, object gift)
{
	...
	{
		me->add_exp(300000);
		send_user(me, "%c%s", ';', sprintf("Bạn đã nhận được %d kinh nghiệm, xin chúc mừng !", 300000));
	}
	...
	{
		me->add_cash(75000);
		send_user(me, "%c%s", ';', sprintf("Bạn đã nhận được %d ngân lượng, xin chúc mừng !", 75000));
	}
	else
	{
	...
	CHAT_D->sys_channel(0, "Bằng hữu " + me->get_name() + " đã mở Túi Quà May Mắn và nhận được "HIR""+ item->get_name()+NOR" !");
	return 0;
}
