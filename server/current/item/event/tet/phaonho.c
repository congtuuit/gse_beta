...
string get_desc()
{
	...
	return  "Là loại Pháo mà mỗi năm tết đến thường dùng để chúc mừng năm mới, \ncó thể dùng nó đuổi Niên Thú, mỗi lần dùng có thể gây ra ít nhất " + dame + " sát thương.";
}
...
int get_use_effect_callout(object me, object who)
{
	...
	for (i = 0; i < sizeof(char); i++) if (who = char[i])
	{
		...
		if (who->is_npc())
		{
			...
			if (damage >= who->get_max_damage()) who->init_enemy_damage(me, damage);
		}
