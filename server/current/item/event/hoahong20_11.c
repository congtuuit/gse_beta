...
int get_use_effect_callout( object me, object who )
{
	...
	if ( !who->have_title("B003") && !who->get_save_2("baishi.tudi") )
	{
...
//	send_user( get_scene_object_2(who, USER_TYPE), "%c%d%w%c%c%c", 0x6f, getoid(who), 5915, 3, OVER_BODY, PF_ONCE );
	...
	if ( random(100) < 7 )
	{
		pize = new("/item/event/hoahongdacbiet20_11");
		if ( !pize )
			return 0;
	}
	else if ( random(100) < 15 )
	{
		nTmp = WEAPON_FILE->get_family_equip( me->get_fam_name(),me->get_gender(),30,HEAD_TYPE+random(6) );
	...
	{
		pize = new(sprintf("/item/ride/TuiThanThu/00%d",1+random(6)));
		if ( !pize )
			return 0;
	}
