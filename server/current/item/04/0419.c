...
string get_desc() 
{ 
        return "可右击打开的Túi quà lớn，会带给你意外的惊喜。"; 
}
// 函数：复位处理
void reset()
{
...
int get_use_effect_callout( object who,object gift) 
{
	...
	if ( item )
	{
		...
		send_user(who,"%c%s",'!',"恭喜您！您打开Túi quà lớn得到了"+item->get_name());
		p = item->move2(who);
		item->add_to_user(p);	
	}
	else if (  cash )
	{
		who->add_cash(cash);
		send_user(who,"%c%s",'!',sprintf("恭喜您！您打开Túi quà lớn得到了%d金",cash));
	}
	else if (  exp )
	{
		who->add_exp(exp);
		send_user(who,"%c%s",'!',sprintf("恭喜您！您打开Túi quà lớn得到了%d点经验",exp));
	}
	else if (  pot )
	{
		who->add_potential(pot);
		send_user(who,"%c%s",'!',sprintf("恭喜您！您打开Túi quà lớn得到了%d点tiềm năng",pot));
	}
	return 0;
}
