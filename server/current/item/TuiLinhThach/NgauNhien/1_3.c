...
void create()
{
        set_name( "Túi Tinh Thạch cấp 1-3" );
        set_picid_1(7106);
        set_picid_2(7106);
        set_value(50);
}
...
string get_desc() 
{ 
        return "Sau khi sử dụng có thể nhận được một Tinh Thạch cấp 1-3";
}
...
int get_use_effect_callout( object who,object gift) 
{
	...
	item->set_level(1 + random(3));
	p = item->move(who,-1);
	item->add_to_user(p);
	CHAT_D->sys_channel( 0, sprintf(who->get_name()+" đã mở Túi Tinh Thạch cấp 1-3 và nhận được %s ",item->get_name()));
	return 0;
}