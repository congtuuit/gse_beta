...
string get_desc() 
{ 
        return "Chỉ có thể hóa giải các trang bị đã bị Nhặt Phong Ấn về trạng thái ban đầu (để có thể giao dịch, trao đổi cho người khác)."; 
}
...
int move_item_callout(object me, object item, object equip, int type)
{
	...
	if (equip->get_bind() != 4)
	{
		result = "Vật phẩm này không cần giải phong ấn nữa!";
	...
	if (type!=1)
	{
		result = sprintf("Bạn có chắc chắn muốn giải phong ấn "+ HIY"%s"+ NOR" không?\n"
			ESC "%c\n" ESC "\nmove ! %x# %d\n"
