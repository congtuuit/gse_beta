...
string get_desc()
{
        return "元始天尊的法器，传闻由千年寒铁铸造而成，能锁住世间万物。此乾坤锁不可Giao dịch。\n能将物品锁住14天。\n在这期间，此物品将不能被Giao dịch、买卖、丢弃。\n使用方法：将其拖放于所须加锁物品之上。\n";
}
...
int move_item_callout(object me, object item, object equip, int type)
{
	...
	if (type==1)
	{
		...
		result = sprintf("%s将被强行加锁14天，在这期间，此物品将不能被Giao dịch、买卖、丢弃。\n"
			ESC "%c\n" ESC "\nmove ! %x# %d\n"
			, equip->get_name(), 1, getoid(item), get_d(equip) );
		send_user(me, "%c%c%s", 0x59, 1, result);
		return 1;
...
int pet_item_callout(object me, object item, object pet, int type)
{
	...
	if (type==1)
	{
		...
		result = sprintf("%s将被强行加锁14天，在这期间，此宠物将不能被Giao dịch、买卖、放生。\n"
			ESC "%c\n" ESC "\npet %x# lock 1\n"
			, pet->get_name(), 1, getoid(pet) );
		send_user(me, "%c%c%s", 0x59, 1, result);
		return 1;
