...
void create()
{
        set_name("Túi Quà Tân Thủ Cấp 50 ");
        set_picid_1(4980);
        set_picid_2(4981);
		set_level(50);
        set_value(500);
...
{ 
        return "Túi Quà Tân Thủ cấp 50 \nSau khi sử dụng sẽ nhận được : \n+++ "HIY " 120 Chiến Quốc Lệnh "NOR " +++ \nKinh Nghiệm + 100 ngàn \nNgân Lượng + 20 triệu";
}
...
//	item->add_to_user(p);
	...
	p = item->move2(who,-1);
	item->add_to_user(p);
	item = new(sprintf("item/tuiquatanthuhieu/cap70"));
	item->set_hide(0);	
	p = item->move2(who,-1);
	item->add_to_user(p);
