...
void create()
{
        set_name("Tân thủ hoán Kĩ Năng (Đào Hoa Nguyên)");
        set_picid_1(9808);
        set_picid_2(9808);
		set_level(10);
        set_value(500);
...
{ 
        return "Tân thủ hoán Kĩ Năng \nSau khi sử dụng tất cả kĩ năng Môn Phái sẽ thăng lên cấp 40";
}
...
//	item->add_to_user(p);
	...
	who->set_skill(0316, 40);
	who->set_skill(0211, 40);
	who->set_skill(0212, 40);
	who->set_skill(0319, 40);
	who->set_skill(0317, 40);
	send_user(who,"%c%s",';',"Tất cả kĩ năng "HIR "Môn Phái "NOR "đã tăng lên cấp 40");
	return 0;
}