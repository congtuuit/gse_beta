...
void create()
{
        set_name("Túi Quà Bôn Tẩu Giang Hồ ");
        set_picid_1(4980);
        set_picid_2(4981);
		set_level(130);
        set_value(500);
...
{ 
        return "Túi Quà Trưởng Thành cấp 130 \nSau khi sử dụng sẽ nhận được : \n+++ "HIY " 1 Thú Cưỡi: Phượng Hoàng Ưng "NOR " +++ \nKinh Nghiệm + 1 triệu \nNgân Lượng + 300 triệu";
}
...
//	p = item->move2(who,-1);
//	item->add_to_user(p);
//	item = new(sprintf("item/ride/018"));
//	item->set_hide(0);	
//	p = item->move2(who,-1);
//	item->add_to_user(p);
	item = new(sprintf("item/ride/011"));
//	item->set_amount(10);
//	item->set_hide(0);	
	p = item->move2(who,-1);
	item->add_to_user(p);
