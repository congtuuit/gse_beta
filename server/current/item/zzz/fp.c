...
int get_use_effect_callout( object who,object gift) 
{
	...
	gift->add_amount(-1);
		...
		p = item->move2(who);
		item->add_to_user(p);
	send_user(who,"%c%s",';',sprintf("Bạn nhận được %d lượng, %s, %s, %s, 10 Sơ Trị Dược, 10 Sơ Phục Dược.", cash, item1->get_name(), item2->get_name(), item3->get_name()));	
