...
int get_number() { return 50; }

void create()
{
	set_name("Túi 50 Vô Song");
	set_picid_1(9995);
	set_picid_2(9995);
	set_value(3);
	set_amount(1);
...
int get_use_effect_callout(object who, object gift)
{
	...
	if (sl1)
	{
		...
		else
		{
			...
			p = item1->move2(who, -1);
			item1->add_to_user(p);
			USER_D->user_channel(sprintf("Bằng hữu "HIR "%s" NOR " mở Túi 50 Vô Song nhận được "HIR "%s "NOR " . ", who->get_name(), item1->get_name()));
		}
	}
