...
void create()
{
        set_name("Túi Qùa Top Thực Lực (3)");
        set_picid_1(4980);
        set_picid_2(4981);
        set_value(1000);
...
int get_use_effect_callout(object who, object gift)
{
	...
	//tuilinhthach2
	item3 = new("/item/TuiLinhThach/NgauNhien/1");
	k = item3->move(who, -1);
	item3->add_to_user(k);

	result = HIY + sprintf("Chúc mừng %s ( %d ) sử dụng "HIR"Túi Qùa Top Thực Lực (3)"HIY" nhận được"HIG" x1 Sơ Cấp Đoạn Thạch"HIY", "HIG" x50 Vô Song Điểm Phiếu"HIY", "HIG" 1x Túi Linh Thạch (1)"HIY".", who->get_name(), who->get_number());
