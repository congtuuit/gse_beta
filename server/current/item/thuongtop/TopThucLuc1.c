...
void create()
{
        set_name("Túi Qùa Top Thực Lực (1)");
        set_picid_1(4980);
        set_picid_2(4981);
        set_value(1000);
}
...
int get_use_effect_callout( object who,object gift) 
{
	...
	//tuilinhthach3
	item3 = new("/item/TuiLinhThach/NgauNhien/3"); 
	k = item3->move(who,-1);
	item3->add_to_user(k);
	
	result = HIY + sprintf("Chúc mừng %s ( %d ) sử dụng "HIR"Túi Qùa Top Thực Lực (1)"HIY" nhận được "HIG"x2 Trung Cấp Đoạn Thạch"HIY", "HIG" x1 Sơ Cấp Đoạn Thạch"HIY", "HIG" x200 Vô Song Điểm Phiếu"HIY", "HIG" 1x Túi Linh Thạch (3)"HIY".", who->get_name(), who->get_number());
