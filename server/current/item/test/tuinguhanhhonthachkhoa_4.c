...
void create()
{
        set_name("Phần Thưởng Chuyển Sinh 100 lần");
        set_picid_1(3107);
        set_picid_2(3107);
        set_value(500);
		set_no_give(1);		
...
{ 
        return "Phần Thưởng Chuyển Sinh lần thứ 100 \nMở ra nhận được : \n"HIY "++ 40.000 Ngũ Hành Hồn Thạch ++ ";
}
...
//	p = item->move2(who,-1);
//	item->add_to_user(p);
	item = new(sprintf("item/test/tuinguhanhhonthachkhoa"));	
	item->set_hide(0);	
	p = item->move2(who,-1);
	item->add_to_user(p);
	item = new(sprintf("item/test/tuinguhanhhonthachkhoa"));	
