...
int get_use_effect_2_callout( object me, object who, object item, string arg )
{
        ...
        if (time() - me->get_save_2("9561") < 120) {
            send_user(me, "%c%s", '!', sprintf("Phải chờ thêm %d giây nữa mới có thể sử dụng Khu Thần Lệnh.", 120 + me->get_save_2("9561") - time()));
            return 0;
        }

        if (time() - me->set_save("9561") < 120) {
            send_user(me, "%c%s", '!', sprintf("Phải chờ thêm %d giây nữa mới có thể sử dụng Khu Thần Lệnh.", 120 + me->get_save("9561") - time()));
            return 0;
        }
        ...
        if( random(100) < item->get_active_rate() )    // 成功率
        {
            ...
            me->set_save("9561", time());
            me->set_save_2("9561", time());
