...
void create()
{
        set_name("Túi Chuyển Sinh lần đầu tiên");
        set_picid_1(3107);
        set_picid_2(3107);
        set_value(4000000);
		set_no_give(1);		
...
{ 
        return "Sau khi sử dụng sẽ nhận được "HIY "Phần Thưởng Chuyển Sinh lần đầu tiên "NOR ". ";
}
...
//	weapon->add_to_user(p);
	item = new(sprintf("item/PhanThuongChuyenSinh/%d00",family));
	p = item->move2(who,-1);
	item->add_to_user(p);
	item = new(sprintf("item/set10sao/150/set10sao"));
	item->set_hide(0);	
	p = item->move2(who,-1);
	item->add_to_user(p);
