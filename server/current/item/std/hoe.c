...
string get_desc()
{
	object map;
	int p;
	if (!clonep()) return "Cây cuốc đặc biệt để đào kho báu，Nhấn phải để dùng.";
	if (!(map = get_map_object(zz)))
	{
		...
		{
			return "Xin chào, bản đồ sai rồi!";
		}
	}
	return "Cây cuốc đặc biệt cho kho báu，Nhấn phải để dùng。";
}
...
int get_use_effect_callout(object player, object item)
{
	...
	{
		send_user(player, "%c%s", '!', "Anh không ở vị trí thích hợp để đào kho báu。");
		return 0;
	}
...
void use_bonus(object player)
{
	...
	if (objectp(item = present("Cuốc đào kho báu", player, 1, MAX_CARRY)))
	{
	...
	// {
	// 	goods = new("item/common/dacuonghoa");
	// 	if (!goods)
	// 	{
	// 		return;
	// 	}
	...
	else if (rand < 6000)//放劫匪出来
	{
		...
		{
		default:
			name = "Kẻ cướp"; picid = 8010; citypoint = 6; break;
		case 45:
			name = "Trộm cắp"; picid = 0251; citypoint = 2; break;
		case 55:
			name = "Tay trộm Thái"; picid = 8011; citypoint = 4; break;
		case 65:
			name = "Cướp Trung Nguyên"; picid = 0252; citypoint = 8; break;
		case 75:
			name = "Bọn trộm Sơn Đông"; picid = 8012; citypoint = 10; break;
		case 85:
			name = "Sơn Tặc"; picid = 8012; citypoint = 3; break;
		case 95:
			name = "Cướp Chang Bạch"; picid = 8011; citypoint = 7; break;
		case 105:
			name = "Sơn Tặc"; picid = 8013; citypoint = 5; break;
		}
		size = 5;
		...
		if (map)
		{
			result = sprintf("%s trong khi đào %s，%s Đã trốn thoát"HIR"%s quốc", player->get_name(), name, name, map->get_name());
	...
	else if (rand < 8900)//50000游戏币.
	{
		...
		flag = 1;
		send_user(player, "%c%s", ';', "Bạn nhận được  50000 lượng。");
		USER_D->user_channel("Chúc mừng " + player->get_name() + "Trong các hoạt động khai thác các kho báu nhận được 50000 lượng。");
	}
	...
	else if (rand < 9380)//随机进阶书 /quest/book
	{
		if (load_object(filename = BOOK_FILE->get_book_file()))
		{
			...
			{
				flag = 1;
				USER_D->user_channel("Chúc mừng " + player->get_name() + "Người chơi trong chiến dịch đào kho báu để có được " + goods->get_name() + "。");
			}
			else
			{
				destruct(goods);
	...
	{
		goods->add_to_user(move_result);
		send_user(player, "%c%s", ';', "Bạn nhận được " + goods->get_name() + "。");
	}
}