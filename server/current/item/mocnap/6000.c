...
int get_use_effect_callout(object who, object gift)
{
	...
	item = new("/item/mocnap/item/tui8tr");
	k = item->move(who, -1);
	item->add_to_user(k);

	log_file("sdmocnap.txt", sprintf("%s : %s (%d) %s sử dụng Tui diem 6000\n", short_time(time()), who->get_name(), who->get_number(), who->get_id()));
	return 0;
}