...
{
	return "[Có thể sử dụng] Phần quà nhận được từ mốc Donate.\n Sau khi sử dụng sẽ nhận được Cao Cấp Kết Dính, Túi Ngọc 1-5, Túi Đoạn Thạch 11";
}
...
int get_use_effect_callout(object who, object gift)
{
	...
	{
		send_user(who, "%c%s", '!', "Hành trang của bạn không đủ 4 chỗ");
		return 0;
	}
	...
	item1 = new("/item/mocnap/item/ManhLongChau_2");
	i = item1->move(who, -1);
	item1->add_to_user(i);

	item2 = new("/item/mocnap/item/3032");
	l = item2->move(who, -1);
	item2->add_to_user(l);
	...
	log_file("sdmocnap.txt", sprintf("%s : %s (%d) %s sử dụng Tui diem 36000\n", short_time(time()), who->get_name(), who->get_number(), who->get_id()));
	return 0;
}