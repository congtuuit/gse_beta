...
{
	return "[Có thể sử dụng] Phần quà nhận được từ mốc Donate.\n Sau khi sử dụng sẽ nhận được Đoạn Thạch 7 Sao\n Túi Ngọc 5 và 20 triệu ngân lượng";
}
...
int get_use_effect_callout(object who, object gift)
{
	...
	item2 = new("/item/mocnap/item/doanThach7");
	l = item2->move(who, -1);
	item2->add_to_user(l);

	item3 = new("/item/mocnap/item/5");
	m = item3->move(who, -1);
	item3->add_to_user(m);

	item = new("/item/mocnap/item/tui20tr");
	k = item->move(who, -1);
	item->add_to_user(k);

	log_file("sdmocnap.txt", sprintf("%s : %s (%d) %s sử dụng Tui diem 60000\n", short_time(time()), who->get_name(), who->get_number(), who->get_id()));
	return 0;
}